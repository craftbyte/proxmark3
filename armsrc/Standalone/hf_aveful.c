...
// - Any other Ultralight clones that have no auth and MAX_DEFAULT_BLOCKS (16)
// blocks
...
int get_block_count(iso14a_card_select_t card, uint8_t version[],
...
uint16_t get_ev1_counter(iso14a_card_select_t card, uint8_t counter,
                         uint8_t *response);
uint16_t get_ev1_tearing(iso14a_card_select_t card, uint8_t counter,
                         uint8_t *response);
    ...
    for (;;) {
      ...
      if (button_pressed != BUTTON_NO_CLICK || data_available())
        ...
        if (!iso14443a_select_card(NULL, &card, NULL, true, 0, true)) {
          ...
          if (card.sak == SAK && card.atqa[0] == ATQA0 &&
              card.atqa[1] == ATQA1 && card.uidlen == 7) {
        ...
        // Get version and re-select card as UL EV0s like to shut off after a
        // 0x60
        ...
        for (int i = 0; i < block_count; i++) {
          ...
          // We're skipping 14 blocks (56 bytes) here, as that "[...] has
          // version/signature/counter data here" according to comments on
          // data_mfu_bin2eml When converting a bin, it's almost all 0 other
          // than one 0x0F byte, and functionality seems to be unaffected if
          // that byte is set to 0x00.
        ...
        Dbprintf("Starting simulation, press pm3-button to stop and go back to "
                 "search state.");
