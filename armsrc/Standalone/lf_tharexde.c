 ...
 * (lf_em4x50_simulate.eml). If reader sends password different from dump file
 * password, it is saved in file lf_em4x50_passwords.log in flash memory.
 *
...
static bool get_input_data_from_file(uint32_t *tag, char *inputfile) {
  ...
  if (exists_in_spiffs(inputfile)) {
    ...
    rdv40_spiffs_read_as_filetype(inputfile, mem, size,
    ...
    for (int i = 0; i < now; i++) {
      for (int j = 0; j < 4; j++) {
        tag[i] |=
            (hex2int(mem[2 * j + 9 * i]) << 4 | hex2int(mem[2 * j + 1 + 9 * i]))
            << ((3 - j) * 8);
      }
    }
  ...
  return ((now == EM4X50_NO_WORDS) &&
          (tag[EM4X50_DEVICE_SERIAL] != tag[EM4X50_DEVICE_ID]));
}
...
void RunMod(void) {
  ...
  for (;;) {
    ...
    // reset timer
    AT91C_BASE_TC1->TC_CCR =
        AT91C_TC_CLKEN | AT91C_TC_SWTRG; // re-enable timer and wait for TC0
    ...
    AT91C_BASE_TC0->TC_CCR =
        AT91C_TC_CLKEN | AT91C_TC_SWTRG; // reset and re-enable timer
  }
