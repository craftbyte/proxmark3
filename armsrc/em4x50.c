...
// Sam7s has several timers, we will use the source TIMER_CLOCK1 (aka
// AT91C_TC_CLKS_TIMER_DIV1_CLOCK) TIMER_CLOCK1 = MCK/2, MCK is running at 48
// MHz, Timer is running at 48/2 = 24 MHz EM4x50 units (T0) have duration of 8
// microseconds (us), which is 1/125000 per second (carrier) T0 = TIMER_CLOCK1 /
// 125000 = 192
#ifndef T0
#define T0 192
#endif
...
// timeout values (empirical) for simulation mode (may vary with regard to
// reader)
...
static bool extract_parities(uint64_t word, uint32_t *data) {
  ...
  *data = 0x0;
  ...
  if ((row_parities == row_parities_calculated) &&
      (col_parities == col_parities_calculated))
    return true;
...
static bool get_signalproperties(void) {
  ...
  for (int i = 0; i < EM4X50_T_TAG_WAITING_FOR_SIGNAL; i++) {
    ...
    if ((i > SIGNAL_IGNORE_FIRST_SAMPLES) &&
        (AT91C_BASE_SSC->SSC_RHR > noise)) {
      signal_found = true;
      break;
    }
  }
...
static bool find_single_listen_window(void) {
  ...
  while (cnt_pulses < EM4X50_T_WAITING_FOR_SNGLLIW) {
    ...
    if (check_pulse_length(get_pulse_length(), 3 * EM4X50_T_TAG_FULL_PERIOD)) {
      ...
      if (check_pulse_length(get_pulse_length(),
...
static bool find_em4x50_tag(void) { return find_single_listen_window(); }
...
void em4x50_brute(em4x50_data_t *etd) {
  ...
  LEDsoff();
  lf_finalize();
  reply_ng(CMD_LF_EM4X50_BRUTE, bsuccess ? PM3_SUCCESS : PM3_EFAILED,
           (uint8_t *)(&pwd), sizeof(pwd));
}
...
void em4x50_chk(uint8_t *filename) {
  ...
  rdv40_spiffs_read_as_filetype((char *)filename, pwds, size,
...
void em4x50_read(em4x50_data_t *etd) {
  ...
  LOW(GPIO_SSC_DOUT);
  lf_finalize();
  reply_ng(CMD_LF_EM4X50_READ, status, (uint8_t *)words,
...
void em4x50_info(em4x50_data_t *etd) {
  ...
  LEDsoff();
  lf_finalize();
  reply_ng(CMD_LF_EM4X50_INFO, status, (uint8_t *)words,
           EM4X50_TAG_MAX_NO_BYTES);
}
  ...
  if (request_receive_mode() == PM3_SUCCESS) {
    ...
    if (tearoff_hook() == PM3_ETEAROFF) { // tearoff occurred
      ...
      if (check_ack(false)) {
        ...
        // now EM4x50 needs T0 * EM4X50_T_TAG_TWEE (EEPROM write time = 3.2ms =
        // 50 * 64 periods) for saving data and should return with ACK
        for (int i = 0; i < 50; i++) {
          WaitUS(EM4X50_T_TAG_FULL_PERIOD * CYCLES2MUSEC);
        }
...
void em4x50_write(em4x50_data_t *etd) {
  ...
  LEDsoff();
  lf_finalize();
  reply_ng(CMD_LF_EM4X50_WRITE, status, (uint8_t *)words,
           EM4X50_TAG_MAX_NO_BYTES);
}
