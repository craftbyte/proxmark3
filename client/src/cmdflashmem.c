...
// Following example RSA-1024 keypair, for test purposes  (from
// common/polarssl/rsa.c) private key - Exponent D
#define RSA_D                                                                  \
...
int rdv4_get_signature(rdv40_validation_t *out) {
  ...
  memcpy(out, (rdv40_validation_t *)resp.data.asBytes,
...
int rdv4_validate(rdv40_validation_t *mem) {
  ...
  // Verify (public key)
  int is_verified =
      mbedtls_rsa_pkcs1_verify(&rsa, NULL, NULL, MBEDTLS_RSA_PUBLIC,
                               MBEDTLS_MD_SHA1, 20, sha_hash, mem->signature);
...
static int rdv4_sign_write(uint8_t *signature, uint8_t slen) {
  ...
  clearCommandBuffer();
  PacketResponseNG resp;
  SendCommandOLD(CMD_FLASHMEM_WRITE, FLASH_MEM_SIGNATURE_OFFSET,
                 FLASH_MEM_SIGNATURE_LEN, 0, signature, slen);
  if (!WaitForResponseTimeout(CMD_ACK, &resp, 2000)) {
    ...
    if (!resp.oldarg[0]) {
      ...
      PrintAndLogEx(SUCCESS,
                    "Writing signature ( "_GREEN_("ok") " ) at offset %u",
...
static int CmdFlashmemSpiBaudrate(const char *Cmd) {
  ...
  SendCommandNG(CMD_FLASHMEM_SET_SPIBAUDRATE, (uint8_t *)&baudrate,
                sizeof(uint32_t));
  return PM3_SUCCESS;
}
...
static int CmdFlashMemLoad(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "mem load",
                ...
                "mem load -f myfile                 -> upload file myfile "
                "values at default offset 0\n"
                "mem load -f myfile -o 1024         -> upload file myfile "
                "values at offset 1024\n"
  ...
  void *argtable[] = {
      ...
      arg_lit0("m", "mifare,mfc",
               "upload 6 bytes keys (mifare key dictionary)"),
  ...
  CLIParamStrToBuf(arg_get_str(ctx, 5), (uint8_t *)filename, FILE_PATH_SIZE,
                   &fnlen);
  CLIParserFree(ctx);
  ...
  while (bytes_remaining > 0) {
    ...
    SendCommandOLD(CMD_FLASHMEM_WRITE, offset + bytes_sent, bytes_in_packet, 0,
                   data + bytes_sent, bytes_in_packet);
  ...
  free(data);
  PrintAndLogEx(SUCCESS,
                "Wrote "_GREEN_("%zu") " bytes to offset "_GREEN_("%u"),
                datalen, offset);
  return PM3_SUCCESS;
}
...
static int CmdFlashMemDump(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "mem dump",
                ...
                "mem dump -f myfile                           -> download all "
                "flashmem to file\n"
                "mem dump --view -o 262015 --len 128          -> display 128 "
                "bytes from offset 262015 (RSA sig)\n"
                "mem dump --view -f myfile -o 241664 --len 58 -> display 58 "
                "bytes from offset 241664 and save to file");
  ...
  int fnlen = 0;
  char filename[FILE_PATH_SIZE] = {0};
  CLIParamStrToBuf(arg_get_str(ctx, 4), (uint8_t *)filename, FILE_PATH_SIZE,
                   &fnlen);
  CLIParserFree(ctx);
  ...
  PrintAndLogEx(INFO, "downloading "_YELLOW_("%u") " bytes from flash memory",
                len);
...
static int CmdFlashMemInfo(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "mem info",
                "Collect signature and verify it from flash memory", "mem info"
                //                  "mem info -s"
  );
  ...
  PrintAndLogEx(INFO, "ID................... %s",
                sprint_hex_inrow(mem.flashid, sizeof(mem.flashid)));
  PrintAndLogEx(INFO, "SHA1................. %s",
                sprint_hex_inrow(sha_hash, sizeof(sha_hash)));
  PrintAndLogEx(NORMAL, "");
  ...
  PrintAndLogEx((is_keyok) ? SUCCESS : FAILED, "RSA key validation...  ( %s )",
  ...
  if (shall_sign) {
    ...
    int is_signed =
        mbedtls_rsa_pkcs1_sign(&rsa, NULL, NULL, MBEDTLS_RSA_PRIVATE,
                               MBEDTLS_MD_SHA1, 20, sha_hash, sign);
    PrintAndLogEx((is_signed == 0) ? SUCCESS : FAILED,
                  "RSA signing.......... ( %s )",
  ...
  int is_verified =
      mbedtls_rsa_pkcs1_verify(&rsa, NULL, NULL, MBEDTLS_RSA_PUBLIC,
                               MBEDTLS_MD_SHA1, 20, sha_hash, from_device);
  mbedtls_rsa_free(&rsa);

  PrintAndLogEx((is_verified == 0) ? SUCCESS : FAILED,
                "RSA verification.....  ( %s )",
...
static command_t CommandTable[] = {
    ...
    {"baudrate", CmdFlashmemSpiBaudrate, IfPm3Flash,
     "Set Flash memory Spi baudrate"},
    {"spiffs", CmdFlashMemSpiFFS, IfPm3Flash,
     "High level SPI FileSystem Flash manipulation"},
