  ...
  /*
      ...
      if (IfPm3Felica()) {
          if (readFelicaUid(false) == PM3_SUCCESS) {
              PrintAndLogEx(NORMAL, "\nValid " _GREEN_("ISO18092 / FeliCa tag")
     " found\n"); res = PM3_SUCCESS;
          }
      }
  */
...
int CmdHFTune(const char *Cmd) {
  ...
  PrintAndLogEx(INFO,
                "Measuring HF antenna, click " _GREEN_(
...
int CmdHFSniff(const char *Cmd) {
  ...
  CLIParserInit(
      &ctx, "hf sniff",
      "The high frequency sniffer will assign all available memory on device "
      "for sniffed data.\n"
      "Use `data samples` to download from device and `data plot` to visualize "
      "it.\n"
      "Press button to quit the sniffing.",
      "hf sniff\n"
      "hf sniff --sp 1000 --st 0   -> skip 1000 pairs, skip 0 triggers");
  void *argtable[] = {
      arg_param_begin, arg_u64_0(NULL, "sp", "<dec>", "skip sample pairs"),
      arg_u64_0(NULL, "st", "<dec>", "skip number of triggers"), arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
  ...
  for (;;) {
    ...
    if (WaitForResponseTimeout(CMD_HF_SNIFF, &resp, 1000)) {
      ...
      if (resp.status == PM3_SUCCESS) {
        ...
        // At the moment, sniff takes all free memory in bigbuff. If this
        // changes, we can't start from beginning idx 0 but from that
        // hi-to-start-of-allocated.
...
int handle_hf_plot(void) {
  ...
  if (!GetFromDevice(FPGA_MEM, buf, FPGA_TRACE_SIZE, 0, NULL, 0, &response,
                     4000, true)) {
...
int CmdHFPlot(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf plot",
                ...
                "This can be used after any hf command and will show the last "
                "few milliseconds of the HF signal.\n"
                "Note: If the last hf command terminated because of a timeout "
                "you will most probably see nothing.\n");
  void *argtable[] = {arg_param_begin, arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
  CLIParserFree(ctx);
...
static command_t CommandTable[] = {
    ...
    {"--------", CmdHelp, AlwaysAvailable,
     "----------------------- " _CYAN_(
         "High Frequency") " -----------------------"},
    {"14a", CmdHF14A, AlwaysAvailable,
     "{ ISO14443A RFIDs...                  }"},
    {"14b", CmdHF14B, AlwaysAvailable,
     "{ ISO14443B RFIDs...                  }"},
    {"15", CmdHF15, AlwaysAvailable, "{ ISO15693 RFIDs...                   }"},
    //    {"cryptorf",    CmdHFCryptoRF,    AlwaysAvailable, "{ CryptoRF
    //    RFIDs...                   }"},
    {"epa", CmdHFEPA, AlwaysAvailable,
     "{ German Identification Card...       }"},
    {"emrtd", CmdHFeMRTD, AlwaysAvailable,
     "{ Machine Readable Travel Document... }"},
    {"felica", CmdHFFelica, AlwaysAvailable,
     "{ ISO18092 / FeliCa RFIDs...          }"},
    {"fido", CmdHFFido, AlwaysAvailable,
     "{ FIDO and FIDO2 authenticators...    }"},
    {"jooki", CmdHF_Jooki, AlwaysAvailable,
     "{ Jooki RFIDs...                      }"},
    {"iclass", CmdHFiClass, AlwaysAvailable,
     "{ ICLASS RFIDs...                     }"},
    {"legic", CmdHFLegic, AlwaysAvailable,
     "{ LEGIC RFIDs...                      }"},
    {"lto", CmdHFLTO, AlwaysAvailable,
     "{ LTO Cartridge Memory RFIDs...       }"},
    {"mf", CmdHFMF, AlwaysAvailable, "{ MIFARE RFIDs...                     }"},
    {"mfp", CmdHFMFP, AlwaysAvailable,
     "{ MIFARE Plus RFIDs...                }"},
    {"mfu", CmdHFMFUltra, AlwaysAvailable,
     "{ MIFARE Ultralight RFIDs...          }"},
    {"mfdes", CmdHFMFDes, AlwaysAvailable,
     "{ MIFARE Desfire RFIDs...             }"},
    {"st", CmdHF_ST, AlwaysAvailable,
     "{ ST Rothult RFIDs...                 }"},
    {"thinfilm", CmdHFThinfilm, AlwaysAvailable,
     "{ Thinfilm RFIDs...                   }"},
    {"topaz", CmdHFTopaz, AlwaysAvailable,
     "{ TOPAZ (NFC Type 1) RFIDs...         }"},
    {"waveshare", CmdHFWaveshare, AlwaysAvailable,
     "{ Waveshare NFC ePaper...             }"},
    {"-----------", CmdHelp, AlwaysAvailable,
     "--------------------- " _CYAN_("General") " ---------------------"},
    {"help", CmdHelp, AlwaysAvailable, "This help"},
    {"list", CmdTraceList, AlwaysAvailable,
