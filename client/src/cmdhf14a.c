...
static const manufactureName manufactureMapping[] = {
    ...
    {0x64, "Associacao do Laboratorio de Sistemas Integraveis Tecnologico - "
           "LSI-TEC Brazil"},
...
static int hf_14a_config_example(void) {
  PrintAndLogEx(NORMAL, "\nExamples to revive Gen2/DirectWrite magic cards "
                        "failing at anticollision:");
  PrintAndLogEx(NORMAL, _CYAN_("    MFC 1k 4b UID") ":");
  PrintAndLogEx(NORMAL, _YELLOW_("          hf 14a config --atqa force --bcc "
                                 "ignore --cl2 skip --rats skip"));
  PrintAndLogEx(NORMAL, _YELLOW_("          hf mf wrbl 0 A FFFFFFFFFFFF "
  ...
  PrintAndLogEx(NORMAL, _YELLOW_("          hf 14a config --atqa force --bcc "
                                 "ignore --cl2 skip --rats skip"));
  PrintAndLogEx(NORMAL, _YELLOW_("          hf mf wrbl 0 A FFFFFFFFFFFF "
                                 "11223344441802006263646566676869"));
  PrintAndLogEx(NORMAL, _YELLOW_("          hf 14a config --std"));
  PrintAndLogEx(NORMAL, _CYAN_("    MFC 1k 7b UID") ":");
  PrintAndLogEx(NORMAL, _YELLOW_("          hf 14a config --atqa force --bcc "
                                 "ignore --cl2 force --cl3 skip --rats skip"));
  PrintAndLogEx(NORMAL, _YELLOW_("          hf mf wrbl 0 A FFFFFFFFFFFF "
                                 "04112233445566084400626364656667"));
  PrintAndLogEx(NORMAL, _YELLOW_("          hf 14a config --std"));
  PrintAndLogEx(NORMAL, _CYAN_("    MFC 4k 7b UID") ":");
  PrintAndLogEx(NORMAL, _YELLOW_("          hf 14a config --atqa forcce --bcc "
                                 "ignore --cl2 force --cl3 skip --rats skip"));
  PrintAndLogEx(NORMAL, _YELLOW_("          hf mf wrbl 0 A FFFFFFFFFFFF "
                                 "04112233445566184200626364656667"));
  PrintAndLogEx(NORMAL, _YELLOW_("          hf 14a config --std"));
  PrintAndLogEx(NORMAL, _CYAN_("    MFUL ") "/" _CYAN_(" MFUL EV1 ") "/" _CYAN_(
                            " MFULC") ":");
  PrintAndLogEx(NORMAL, _YELLOW_("          hf 14a config --atqa force --bcc "
                                 "ignore --cl2 force --cl3 skip -rats skip"));
...
static int CmdHf14AConfig(const char *Cmd) {
  ...
  CLIParserInit(
      &ctx, "hf 14a config", "Configure 14a settings (use with caution)",
      "hf 14a config              -> Print current configuration\n"
      "hf 14a config --std        -> Reset default configuration (follow "
      "standard)\n"
  ...
  void *argtable[] = {
      arg_param_begin,
      arg_str0(NULL, "atqa", "<std|force|skip>",
               "Configure ATQA<>anticollision behavior"),
      ...
      arg_lit0("v", "verbose",
               "verbose output, also prints examples for reviving Gen2 cards"),
      arg_param_end};
  ...
  hf14a_config config = {.forceanticol = atqa,
                         .forcebcc = bcc,
                         .forcecl2 = cl2,
                         .forcecl3 = cl3,
                         .forcerats = rats};
...
int Hf14443_4aGetCardData(iso14a_card_select_t *card) {
  ...
  PrintAndLogEx(SUCCESS, " UID: " _GREEN_("%s"),
                sprint_hex(card->uid, card->uidlen));
  PrintAndLogEx(SUCCESS, "ATQA: %02x %02x", card->atqa[1], card->atqa[0]);
  PrintAndLogEx(SUCCESS, " SAK: %02x [%" PRIu64 "]", card->sak, resp.oldarg[0]);
  if (card->ats_len < 3) { // a valid ATS consists of at least the length byte
                           // (TL) and 2 CRC bytes
    PrintAndLogEx(INFO, "E-> Error ATS length(%d) : %s", card->ats_len,
                  sprint_hex(card->ats, card->ats_len));
    return 1;
  }
...
static int CmdHF14AReader(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf 14a reader", "Reader for ISO 14443A based tags",
      ...
      */
      ...
      if (select_status == 3) {
        if (!(silent && continuous)) {
          PrintAndLogEx(
              INFO, "Card doesn't support standard iso14443-3 anticollision");
          PrintAndLogEx(SUCCESS, "ATQA: %02x %02x", card.atqa[1], card.atqa[0]);
        }
        DropField();
        res = PM3_ESOFT;
      ...
      PrintAndLogEx(SUCCESS, " UID: " _GREEN_("%s"),
                    sprint_hex(card.uid, card.uidlen));
      if (!(silent && continuous)) {
        PrintAndLogEx(SUCCESS, "ATQA: " _GREEN_("%02x %02x"), card.atqa[1],
                      card.atqa[0]);
        PrintAndLogEx(SUCCESS, " SAK: " _GREEN_("%02x [%" PRIu64 "]"), card.sak,
                      resp.oldarg[0]);

        if (card.ats_len >= 3) { // a valid ATS consists of at least the length
                                 // byte (TL) and 2 CRC bytes
          PrintAndLogEx(SUCCESS, " ATS: " _GREEN_("%s"),
                        sprint_hex(card.ats, card.ats_len));
        }
      }
      if (!disconnectAfter) {
...
static int CmdHF14AInfo(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf 14a info",
                "This command makes more extensive tests against a ISO14443a "
                "tag in order to collect information",
  ...
  void *argtable[] = {
      ...
      arg_lit0("s", "aidsearch",
               "checks if AIDs from aidlist.json is present on the card and "
               "prints information about found AIDs"),
      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdHF14ACUIDs(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf 14a cuids", "Collect n>0 ISO14443-a UIDs in one go",
...
int CmdHF14ASim(const char *Cmd) {
  ...
  if (uid_len > 0) {
    ...
    PrintAndLogEx(
        SUCCESS,
        "Emulating " _YELLOW_("ISO/IEC 14443 type A tag") " with " _GREEN_(
            "%d byte UID (%s)"),
        uid_len, sprint_hex(uid, uid_len));
    useUIDfromEML = false;
  }
  ...
  if (resp.status == PM3_EOPABORTED &&
      ((flags & FLAG_NR_AR_ATTACK) == FLAG_NR_AR_ATTACK))
...
int CmdHF14ASniff(const char *Cmd) {
  ...
  void *argtable[] = {
      ...
      arg_lit0("r", "reader",
               "triggered by first 7-bit request from reader (REQ,WUP,...)"),
      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
...
int ExchangeRAW14a(uint8_t *datain, int datainlen, bool activateField,
                   bool leaveSignalON, uint8_t *dataout, int maxdataoutlen,
                   int *dataoutlen, bool silentMode) {
  ...
  *dataoutlen = 0;
  ...
  if (activateField) {
    ...
    SendCommandMIX(CMD_HF_ISO14443A_READER,
                   ISO14A_CONNECT | ISO14A_NO_DISCONNECT, 0, 0, NULL, 0);
    ...
    if (resp.oldarg[0] != 1 && resp.oldarg[0] != 2) {
      if (!silentMode)
        PrintAndLogEx(ERR, "Card not in iso14443-4. res=%" PRId64 ".",
                      resp.oldarg[0]);
      return 1;
    }

    if (resp.oldarg[0] == 2) { // 0: couldn't read, 1: OK, with ATS, 2: OK, no
                               // ATS, 3: proprietary Anticollision
      // get ATS
      uint8_t rats[] = {0xE0, 0x80}; // FSDI=8 (FSD=256), CID=0
      SendCommandMIX(CMD_HF_ISO14443A_READER,
                     ISO14A_RAW | ISO14A_APPEND_CRC | ISO14A_NO_DISCONNECT, 2,
                     0, rats, sizeof(rats));
      if (!WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
        if (!silentMode)
          PrintAndLogEx(ERR, "Proxmark3 connection timeout.");
  ...
  SendCommandOLD(CMD_HF_ISO14443A_READER, ISO14A_RAW | ISO14A_APPEND_CRC | cmdc,
                 (datainlen & 0xFFFF) + 2, 0, data, (datainlen & 0xFFFF) + 2);
  ...
  if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
    ...
    *dataoutlen = iLen - 2;
    ...
    if (maxdataoutlen && *dataoutlen > maxdataoutlen) {
      if (!silentMode)
        PrintAndLogEx(ERR, "Buffer too small(%d). Needs %d bytes", *dataoutlen,
                      maxdataoutlen);
      return 2;
    }
    ...
    if (recv[0] != data[0]) {
      if (!silentMode)
        PrintAndLogEx(ERR,
                      "iso14443-4 framing error. Card send %2x must be %2x",
                      dataout[0], data[0]);
      return 2;
    }
...
static int SelectCard14443_4(bool disconnect, iso14a_card_select_t *card) {
  ...
  // Anticollision + SELECT card
  SendCommandMIX(CMD_HF_ISO14443A_READER, ISO14A_CONNECT | ISO14A_NO_DISCONNECT,
                 0, 0, NULL, 0);
  if (!WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
    PrintAndLogEx(ERR, "Proxmark3 connection timeout.");
    return 1;
  ...
  if (resp.oldarg[0] == 2) { // 0: couldn't read, 1: OK, with ATS, 2: OK, no
    ...
    uint8_t rats[] = {0xE0, 0x80}; // FSDI=8 (FSD=256), CID=0
    SendCommandMIX(CMD_HF_ISO14443A_READER,
                   ISO14A_RAW | ISO14A_APPEND_CRC | ISO14A_NO_DISCONNECT,
                   sizeof(rats), 0, rats, sizeof(rats));
    if (!WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
      PrintAndLogEx(ERR, "Proxmark3 connection timeout.");
      return 1;
    }
...
static int CmdExchangeAPDU(bool chainingin, uint8_t *datain, int datainlen,
                           bool activateField, uint8_t *dataout,
                           int maxdataoutlen, int *dataoutlen,
                           bool *chainingout) {
  *chainingout = false;
  ...
  // "Command APDU" length should be 5+255+1, but javacard's APDU buffer might
  // be smaller - 133 bytes
  // https://stackoverflow.com/questions/32994936/safe-max-java-card-apdu-data-command-and-respond-size
  // here length PM3_CMD_DATA_SIZE=512
  // timeout must be authomatically set by "get ATS"
  if (datain)
    SendCommandOLD(CMD_HF_ISO14443A_READER,
                   ISO14A_APDU | ISO14A_NO_DISCONNECT | cmdc,
                   (datainlen & 0xFFFF), 0, datain, datainlen & 0xFFFF);
  else
    SendCommandMIX(CMD_HF_ISO14443A_READER,
                   ISO14A_APDU | ISO14A_NO_DISCONNECT | cmdc, 0, 0, NULL, 0);
  ...
  if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
    ...
    *dataoutlen += dlen;
    ...
    if (maxdataoutlen && *dataoutlen > maxdataoutlen) {
      PrintAndLogEx(ERR, "APDU: Buffer too small(%d). Needs %d bytes",
                    *dataoutlen, maxdataoutlen);
      return 2;
    }
...
int ExchangeAPDU14a(uint8_t *datain, int datainlen, bool activateField,
                    bool leaveSignalON, uint8_t *dataout, int maxdataoutlen,
                    int *dataoutlen) {
  *dataoutlen = 0;
  bool chaining = false;
  ...
  if (APDUInFramingEnable && ((frameLength && (datainlen > frameLength - 3)) ||
                              (datainlen > PM3_CMD_DATA_SIZE - 3))) {
    int clen = 0;
    ...
    do {
      ...
      *dataoutlen = 0;
      res = CmdExchangeAPDU(chainBlockNotLast, &datain[clen], vlen,
                            vActivateField, dataout, maxdataoutlen, dataoutlen,
                            &chaining);
      if (res) {
        if (!leaveSignalON)
          DropField();
      ...
      if ((*dataoutlen == 0) &&
          (*dataoutlen != 0 ||
           chaining != chainBlockNotLast)) { // *dataoutlen!=0. 'A && (!A || B)'
      ...
      if (*dataoutlen) {
        if (clen != datainlen)
          PrintAndLogEx(ERR,
                        "APDU: I-block/R-block sequence error. Data len=%d, "
                        "Sent=%d, Last packet len=%d",
                        datainlen, clen, *dataoutlen);
        break;
      }
    } while (clen < datainlen);
  } else {
    res = CmdExchangeAPDU(false, datain, datainlen, activateField, dataout,
                          maxdataoutlen, dataoutlen, &chaining);
    if (res) {
      if (!leaveSignalON)
        DropField();
  ...
  while (chaining) {
    ...
    res = CmdExchangeAPDU(false, NULL, 0, false, &dataout[*dataoutlen],
                          maxdataoutlen, dataoutlen, &chaining);
...
static int CmdHF14AAPDU(const char *Cmd) {
  ...
  CLIParserInit(
      &ctx, "hf 14a apdu",
      "Sends an ISO 7816-4 APDU via ISO 14443-4 block transmission protocol "
      "(T=CL). works with all apdu types from ISO 7816-4:2013",
      "hf 14a apdu -st 00A404000E325041592E5359532E444446303100\n"
      "hf 14a apdu -sd 00A404000E325041592E5359532E444446303100        -> "
      "decode apdu\n"
      "hf 14a apdu -sm 00A40400 325041592E5359532E4444463031 -l 256    -> "
      "encode standard apdu\n"
      "hf 14a apdu -sm 00A40400 325041592E5359532E4444463031 -el 65536 -> "
  ...
  void *argtable[] = {
      ...
      arg_str0("m", "make", "<head (CLA INS P1 P2) hex>",
               "make apdu with head from this field and data from data field. "
               "Must be 4 bytes length: <CLA INS P1 P2>"),
      arg_lit0("e", "extended",
               "make extended length apdu if `m` parameter included"),
      arg_int0("l", "le", "<Le (int)>",
               "Le apdu parameter if `m` parameter included"),
      arg_strx1(NULL, NULL, "<APDU (hex) | data (hex)>",
                "data if `m` parameter included"),
      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, false);
  ...
  if (makeAPDU && headerlen != 4) {
    PrintAndLogEx(ERR, "header length must be 4 bytes instead of %d",
                  headerlen);
    CLIParserFree(ctx);
    return PM3_EINVARG;
  }
  ...
  PrintAndLogEx(SUCCESS, "( " _YELLOW_("%s%s%s") " )",
                activateField ? "select" : "", leaveSignalON ? ", keep" : "",
                decodeTLV ? ", TLV" : "");
  ...
  int res = ExchangeAPDU14a(data, datalen, activateField, leaveSignalON, data,
                            PM3_CMD_DATA_SIZE, &datalen);

  if (res)
    return res;

  PrintAndLogEx(SUCCESS, "<<< %s | %s", sprint_hex_inrow(data, datalen),
                sprint_ascii(data, datalen));
  PrintAndLogEx(SUCCESS, "<<< status: %02x %02x - %s", data[datalen - 2],
                data[datalen - 1],
...
#define MAX_TIMEOUT 40542464 // = (2^32-1) * (8*16) / 13560000Hz * 1000ms/s
    ...
    if (timeout > MAX_TIMEOUT) {
      timeout = MAX_TIMEOUT;
      PrintAndLogEx(INFO, "Set timeout to 40542 seconds (11.26 hours). The max "
                          "we can wait for response");
    }
    argtimeout =
        13560000 / 1000 / (8 * 16) *
        timeout; // timeout in ETUs (time to transfer 1 bit, approx. 9.4 us)
  }
  ...
  clearCommandBuffer();
  SendCommandOLD(CMD_HF_ISO14443A_READER, flags,
                 (datalen & 0xFFFF) | ((uint32_t)(numbits << 16)), argtimeout,
                 data, datalen & 0xFFFF);
...
static int waitCmd(bool i_select, uint32_t timeout) {
  ...
  if (WaitForResponseTimeout(CMD_ACK, &resp, timeout + 1500)) {
    ...
    if (i_select == false && len >= 3) {
      ...
      char s[16];
      sprintf(s, (crc) ? _GREEN_("%02X %02X") : _RED_("%02X %02X"),
...
static int CmdHF14AAntiFuzz(const char *Cmd) {
  ...
  void *argtable[] = {arg_param_begin, arg_lit0("4", NULL, "4 byte uid"),
                      arg_lit0("7", NULL, "7 byte uid"),
                      arg_lit0(NULL, "10", "10 byte uid"), arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, false);
...
static int CmdHF14AChaining(const char *Cmd) {
  ...
  CLIParserInit(
      &ctx, "hf 14a chaining",
      "Enable/Disable ISO14443a input chaining. Maximum input length goes from "
      "ATS.",
...
int infoHF14A(bool verbose, bool do_nack_test, bool do_aid_search) {
  ...
  */
  ...
  if (card.uidlen <= 4) {
    ...
    PrintAndLogEx(SUCCESS, "MANUFACTURER:    " _YELLOW_("%s"),
                  getTagInfo(card.uid[0]));
    ...
    case 0x46:
      if (memcmp(card.uid, "FSTN10m", 7) == 0) {
        isMifareClassic = false;
        printTag("Waveshare NFC-Powered e-Paper 1.54\" (please disregard "
                 "MANUFACTURER mapping above)");
      }
      break;
    case 0x57:
      if (memcmp(card.uid, "WSDZ10m", 7) == 0) {
        isMifareClassic = false;
        printTag("Waveshare NFC-Powered e-Paper (please disregard MANUFACTURER "
                 "mapping above)");
      }
      break;
    default:
      ...
      case 0x00: {
        ...
        select_status =
            resp.oldarg[0]; // 0: couldn't read, 1: OK, with ATS, 2: OK, no ATS
  ...
  if (select_status == 2) {
    ...
    clearCommandBuffer();
    SendCommandMIX(CMD_HF_ISO14443A_READER,
                   ISO14A_RAW | ISO14A_APPEND_CRC | ISO14A_NO_DISCONNECT, 2, 0,
                   rats, sizeof(rats));
    WaitForResponse(CMD_ACK, &resp);
  ...
  if (card.ats_len >= 3) { // a valid ATS consists of at least the length byte
    ...
    if (card.ats[0] != card.ats_len - 2) {
      PrintAndLogEx(WARNING,
                    "ATS may be corrupted. Length of ATS (%d bytes incl. 2 "
                    "Bytes CRC) doesn't match TL",
                    card.ats_len);
    }

    PrintAndLogEx(SUCCESS, "ATS: " _YELLOW_("%s") "[ %02x %02x ]",
                  sprint_hex(card.ats, card.ats_len - 2),
                  card.ats[card.ats_len - 1], card.ats[card.ats_len]);
    PrintAndLogEx(
        INFO,
        "     " _YELLOW_("%02x") "...............  TL    length is " _GREEN_(
            "%d") " bytes",
        card.ats[0], card.ats[0]);
    ...
    if (card.ats[0] > 1) { // there is a format byte (T0)
      ...
      PrintAndLogEx(
          INFO,
          "        " _YELLOW_(
              "%02X") "............  T0    TA1 is%s present, TB1 is%s present, "
                      "TC1 is%s present, FSCI is %d (FSC = %d)",
          card.ats[1], (ta1 ? "" : _RED_(" NOT")), (tb1 ? "" : _RED_(" NOT")),
          (tc1 ? "" : _RED_(" NOT")), fsci,
    ...
    if (ta1) {
      char dr[16], ds[16];
      dr[0] = ds[0] = '\0';
      if (card.ats[pos] & 0x10)
        strcat(ds, "2, ");
      if (card.ats[pos] & 0x20)
        strcat(ds, "4, ");
      if (card.ats[pos] & 0x40)
        strcat(ds, "8, ");
      if (card.ats[pos] & 0x01)
        strcat(dr, "2, ");
      if (card.ats[pos] & 0x02)
        strcat(dr, "4, ");
      if (card.ats[pos] & 0x04)
        strcat(dr, "8, ");
      if (strlen(ds) != 0)
        ds[strlen(ds) - 2] = '\0';
      if (strlen(dr) != 0)
      ...
      PrintAndLogEx(
          INFO,
          "           " _YELLOW_(
              "%02X") ".........  TA1   different divisors are%s supported, "
                      "DR: [%s], DS: [%s]",
          card.ats[pos], ((card.ats[pos] & 0x80) ? _RED_(" NOT") : ""), dr, ds);
    ...
    if (tb1) {
      ...
      PrintAndLogEx(INFO,
                    "              " _YELLOW_(
                        "%02X") "......  TB1   SFGI = %d (SFGT = %s%d/fc), FWI "
                                "= " _YELLOW_("%d") " (FWT = %d/fc)",
                    card.ats[pos], (sfgi), sfgi ? "" : "(not needed) ",
                    sfgi ? (1 << 12) << sfgi : 0, fwi, (1 << 12) << fwi);
      pos++;
    }
    ...
    if (tc1) {
      PrintAndLogEx(
          INFO,
          "                 " _YELLOW_(
              "%02X") "...  TC1   NAD is%s supported, CID is%s supported",
          card.ats[pos], (card.ats[pos] & 0x01) ? "" : _RED_(" NOT"),
    ...
    if (card.ats[0] > pos && card.ats[0] <= card.ats_len - 2) {
      ...
      if (card.ats[0] - pos >= 7) {
        ...
        if ((card.sak & 0x70) == 0x40) { // and no GetVersion()..
          ...
          } else if (memcmp(card.ats + pos, "\xC1\x05\x2F\x2F\x00\x35\xC7",
                            7) == 0) {
          ...
          } else if (memcmp(card.ats + pos, "\xC1\x05\x21\x30\x00\xF6\xD1",
                            7) == 0) {
            snprintf(tip + strlen(tip), sizeof(tip) - strlen(tip),
                     _GREEN_("%s"), "MIFARE Plus SE 1K (17pF)");

          } else if (memcmp(card.ats + pos, "\xC1\x05\x21\x30\x10\xF6\xD1",
                            7) == 0) {
            snprintf(tip + strlen(tip), sizeof(tip) - strlen(tip),
          ...
          if ((card.sak & 0x20) == 0x20) { // and no GetVersion()..
            ...
            if (memcmp(card.ats + pos, "\xC1\x05\x2F\x2F\x01\xBC\xD6", 7) ==
                0) {
              snprintf(tip + strlen(tip), sizeof(tip) - strlen(tip),
      ...
      if (card.ats[pos] == 0xC1) {
        ...
        PrintAndLogEx(SUCCESS, "    C1.....................   Mifare or "
                               "(multiple) virtual cards of various type");
        PrintAndLogEx(SUCCESS,
                      "       %02x..................   length is " _YELLOW_(
                          "%d") " bytes",
    ...
    if (do_aid_search) {
      ...
      if (root != NULL) {
        ...
        for (elmindx = 0; elmindx < json_array_size(root); elmindx++) {
          ...
          size_t resultlen = 0;
          int res =
              EMVSelect(ECC_CONTACTLESS, ActivateField, true, vaid, vaidlen,
                        result, sizeof(result), &resultlen, &sw, NULL);
          ActivateField = false;
          if (res)
            continue;
          ...
          if (resultlen > 3) {
            ...
            if (tlv) {
              ...
              const struct tlv *dfnametlv =
                  tlvdb_get_tlv(tlvdb_find_full(tlv, 0x84));
              if (dfnametlv) {
            ...
            if (dfnamelen) {
              if (dfnamelen == vaidlen) {
                if (memcmp(dfname, vaid, vaidlen) == 0) {
                  ...
                  PrintAndLogEx(INFO, "(DF) Name not equal to AID: %s :",
                                sprint_hex(dfname, dfnamelen));
...
static int CmdHf14AFindapdu(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf 14a apdufind",
                "Enumerate APDU's of ISO7816 protocol to find valid "
                "CLS/INS/P1/P2 commands.\n"
                ...
                "hf 14a apdufind --cla 80 --error-limit 20 --skip-ins a4 "
                "--skip-ins b0 --with-le\n");
  ...
  void *argtable[] = {
      ...
      arg_u64_0("r", "reset", "<number>",
                "Minimum secondes before resetting the tag (to prevent timeout "
                "issues). Default is 5 minutes"),
      arg_u64_0("e", "error-limit", "<number>",
                "Maximum times an status word other than 0x9000 or 0x6D00 is "
                "shown. Default is 512."),
      arg_strx0("s", "skip-ins", "<hex>",
                "Do not test an instructions (can be specifed multiple times)"),
  ...
  PrintAndLogEx(INFO, "Sending a test APDU (select file command) to check if "
                      "the tag is responding to APDU");
  param_gethex_to_eol("00a404000aa000000440000101000100", 0, aSELECT_AID,
                      sizeof(aSELECT_AID), &aSELECT_AID_n);
  int res = ExchangeAPDU14a(aSELECT_AID, aSELECT_AID_n, true, false, response,
                            sizeof(response), &response_n);
  if (res) {
    PrintAndLogEx(
        FAILED,
        "Tag did not responde to a test APDU (select file command). Aborting");
    return res;
  }
  PrintAndLogEx(SUCCESS,
                "Got response. Starting the APDU finder [ CLA " _GREEN_("%02X") " INS " _GREEN_(
                    "%02X") " P1 " _GREEN_("%02X") " P2 " _GREEN_("%02X") " ]",
                cla, ins, p1, p2);
  PrintAndLogEx(INFO, "Press " _GREEN_("<Enter>") " to exit");
  ...
  do {
    do {
      ...
      retry_ins:
        ...
        if (verbose) {
          PrintAndLogEx(
              INFO,
              "Status: [ CLA " _GREEN_("%02X") " INS " _GREEN_(
                  "%02X") " P1 " _GREEN_("%02X") " P2 " _GREEN_("%02X") " ]",
              cla, ins, p1, p2);
        }

        // Send APDU without Le (case 1) and with Le = 0 (case 2S), if "with-le"
        // was set.
        uint8_t command[5] = {cla, ins, p1, p2, 0x00};
        int command_n = 4;
        for (int i = 0; i < 1 + with_le; i++) {
          // Send APDU.
          res = ExchangeAPDU14a(command, command_n + i, activate_field,
                                keep_field_on, response, sizeof(response),
                                &response_n);
          if (res) {
            DropField();
            activate_field = true;
          ...
          if (sw_occurences < error_limit) {
            ...
            PrintAndLogEx(log_level, "Got response for APDU \"%s\": %04X (%s)",
                          sprint_hex_inrow(command, command_n + i), sw,
      ...
      if (t_since_last_reset > reset_time) {
        ...
        PrintAndLogEx(
            ...
            " seconds ago. Reseting the tag to prevent timeout issues",
            t_since_last_reset);
      }
      PrintAndLogEx(
          INFO,
    ...
    PrintAndLogEx(
        INFO,
        "Status: [ CLA " _GREEN_("%02X") " INS " _GREEN_("%02X") " P1 " _GREEN_(
            "%02X") " P2 " _GREEN_("%02X") " ]",
        cla, ins, p1, p2);
  } while (cla != cla_arg[0]);
...
static command_t CommandTable[] = {
    ...
    {"cuids", CmdHF14ACUIDs, IfPm3Iso14443a,
