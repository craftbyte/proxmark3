...
jooki_test_t jooks[] = {
    {{0x04, 0xDA, 0xB7, 0x6A, 0xE7, 0x4C, 0x80},
     "ruxow8lnn88uyeX+",
     0x01,
     0x00},
    {{0x04, 0xf0, 0x22, 0xc2, 0x33, 0x5e, 0x80}, "\0", 0x01, 0x00},
    {{0x04, 0x8C, 0xEC, 0xDA, 0xF0, 0x4A, 0x80},
     "ONrsVf7jX6IaSNV6",
     0x01,
     0x01},
    {{0x04, 0x92, 0xA7, 0x6A, 0xE7, 0x4C, 0x81},
     "Hjjpcx/mZwuveTF+",
     0x01,
     0x02},
    {{0x04, 0xD0, 0xB0, 0x3A, 0xD3, 0x63, 0x80}, "\0", 0x01, 0x02},
    {{0x04, 0x96, 0x42, 0xDA, 0xF0, 0x4A, 0x80},
     "vEWy0WO9wZNEzEok",
     0x01,
     0x03},
    {{0x04, 0x33, 0xb5, 0x62, 0x39, 0x4d, 0x80}, "\0", 0x01, 0x03},
    {{0x04, 0x17, 0xB7, 0x3A, 0xD3, 0x63, 0x81},
     "f0axEma+g2WnLGAm",
     0x01,
     0x05},
    {{0x04, 0x84, 0x27, 0x6A, 0xE7, 0x4C, 0x80},
     "VZB/OLBwOiM5Mpnp",
     0x01,
     0x05},
    {{0x04, 0x28, 0xF4, 0xDA, 0xF0, 0x4A, 0x81},
     "7WzlgEzqLgwTnWNy",
     0x01,
     0x05},
};
...
const uint8_t jooki_secret[] = {0x20, 0x20, 0x20, 0x6D, 0x24, 0x0B,
                                0xEB, 0x94, 0x2C, 0x80, 0x45, 0x16};
const uint8_t NFC_SECRET[] = {0x03, 0x9c, 0x25, 0x6f, 0xb9, 0x2e, 0xe8,
...
static int jooki_encode(uint8_t *iv, uint8_t tid, uint8_t fid, uint8_t *uid,
                        uint8_t *out) {
  if (out == NULL) {
  ...
  uint8_t d[JOOKI_PLAIN_LEN] = {iv[0],  iv[1],  iv[2],  tid,    fid,    uid[0],
  ...
  uint8_t b64[20];
  memset(b64, 0, 20);
  mbedtls_base64_encode(b64, sizeof(b64), &b64len, (const unsigned char *)enc,
                        sizeof(enc));
...
static int jooki_decode(uint8_t *b64, uint8_t *result) {
  ...
  size_t outputlen = 0;
  mbedtls_base64_decode(ndef, sizeof(ndef), &outputlen,
                        (const unsigned char *)b64, 16);

  PrintAndLogEx(DEBUG, "(decode_jooki) raw encoded... " _GREEN_("%s"),
                sprint_hex(ndef, sizeof(ndef)));
  ...
  PrintAndLogEx(DEBUG, "(decode_jooki) plain......... %s",
                sprint_hex(result, sizeof(ndef)));
  return PM3_SUCCESS;
}
...
static void jooki_printEx(uint8_t *b64, uint8_t *iv, uint8_t tid, uint8_t fid,
                          uint8_t *uid, bool verbose) {
  int idx = jooki_lookup(tid, fid);

  PrintAndLogEx(INFO, "Encoded URL.. %s ( %s )", sprint_hex(b64, 12), b64);
  PrintAndLogEx(INFO, "Figurine..... %02x %02x - " _GREEN_("%s, %s"), tid, fid,
  ...
  PrintAndLogEx(INFO, "NDEF raw..... %s",
                sprint_hex_inrow(ndefmsg, sizeof(ndefmsg)));
...
static int jooki_selftest(void) {
  ...
  for (int i = 0; i < ARRAYLEN(jooks); i++) {
    ...
    PrintAndLogEx(INFO, "Encoded URL.. %s ( %s )",
                  sprint_hex((const uint8_t *)jooks[i].b64, 12), jooks[i].b64);
    PrintAndLogEx(INFO, "Type......... %02x - " _GREEN_("%s") " ( %s )", tid,
                  (idx != -1) ? jooks_figures[idx].typedesc : "n/a",
                  tid_ok ? _GREEN_("ok") : _RED_("fail"));
    PrintAndLogEx(INFO, "Figurine..... %02x - " _GREEN_("%s") " ( %s )", fid,
                  (idx != -1) ? jooks_figures[idx].figdesc : "n/a",
                  fid_ok ? _GREEN_("ok") : _RED_("fail"));
    PrintAndLogEx(INFO, "iv........... %s", sprint_hex(iv, sizeof(iv)));
    PrintAndLogEx(INFO, "uid.......... %s ( %s )", sprint_hex(uid, sizeof(uid)),
                  uid_ok ? _GREEN_("ok") : _RED_("fail"));
    ...
    jooki_create_ndef(b64, ndefmsg);
    PrintAndLogEx(INFO, "NDEF raw .... %s",
                  sprint_hex(ndefmsg, sizeof(ndefmsg)));
...
static int CmdHF14AJookiEncode(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf jooki encode",
                ...
                "hf jooki encode -r --dragon   --> read uid from tag and use "
                "for encoding\n"
  ...
  uint8_t figure_abbr_val =
      t0 + t1 + t2 + t3 + t5 + t6 + t7 + t8 + t9 + ta + tb + tc + td;

  if (figure_abbr_val > 1) {
    PrintAndLogEx(
        ERR, "Select one tag type or use figurine type id and figurine id");
    return PM3_EINVARG;
  }
  ...
  if (use_tag) {
    ...
    if (ulen != JOOKI_UID_LEN) {
      PrintAndLogEx(ERR, "Wrong length of UID, expect %u, got %d",
                    JOOKI_UID_LEN, ulen);
      return PM3_EINVARG;
    }
  }
...
static int CmdHF14AJookiClone(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "hf jooki clone", "Write a Jooki token to a Ultralight or NTAG tag",
      "hf jooki clone -d <hex bytes>         --> where hex is raw NDEF\n"
      "hf jooki clone --b64 7WzlgEzqLgwTnWNy --> using base64 url parameter");

  void *argtable[] = {
      arg_param_begin, arg_str0("b", "b64", "<base64>", "base64 url parameter"),
      arg_str0("d", "data", "<hex>", "raw NDEF bytes"),
      arg_str0("p", "pwd", "<hex>",
               "password for authentication (EV1/NTAG 4 bytes)"),
      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, false);
  int blen = 16;
  uint8_t b64[JOOKI_B64_LEN] = {0x00};
  ...
  while ((i * 4) < dlen) {
    ...
    if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
      ...
      PrintAndLogEx(SUCCESS, "Write block %d ( %s )", blockno,
