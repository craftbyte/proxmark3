 ...
 * static int usage_hf14_sniff(void) {
    PrintAndLogEx(NORMAL, "It continuously gets data from the field and saves it
to: log, emulator, emulator file."); PrintAndLogEx(NORMAL, "Usage:  hf mf sniff
[h] [l] [d] [f]"); PrintAndLogEx(NORMAL, "Options:"); PrintAndLogEx(NORMAL, " h
this help"); PrintAndLogEx(NORMAL, "      l    save encrypted sequence to
logfile `uid.log`"); PrintAndLogEx(NORMAL, "      d    decrypt sequence and put
it to log file `uid.log`");
//  PrintAndLogEx(NORMAL, " n/a  e     decrypt sequence, collect read and write
commands and save the result of the sequence to emulator memory");
    PrintAndLogEx(NORMAL, "      f    decrypt sequence, collect read and write
commands and save the result of the sequence to emulator dump file `uid.eml`");
...
static int usage_hf14_hardnested(void) {
  ...
  PrintAndLogEx(NORMAL, "                       <target block number> <target "
                        "key A|B> [known target key (12 hex symbols)] [w] [s]");
  PrintAndLogEx(NORMAL, "  or  hf mf hardnested r [known target key]");
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(NORMAL, "Options:");
  PrintAndLogEx(NORMAL, "      h         this help");
  PrintAndLogEx(NORMAL,
                "      w         acquire nonces and UID, and write them to "
                "binary file with default name hf-mf-<UID>-nonces.bin");
  PrintAndLogEx(NORMAL, "      s         slower acquisition (required by some "
                        "non standard cards)");
  PrintAndLogEx(NORMAL,
                "      r         read hf-mf-<UID>-nonces.bin if tag present, "
                "otherwise read nonces.bin, then start attack");
  PrintAndLogEx(NORMAL, "      u <UID>   read/write hf-mf-<UID>-nonces.bin "
                        "instead of default name");
  PrintAndLogEx(NORMAL,
                "      f <name>  read/write <name> instead of default name");
  PrintAndLogEx(NORMAL, "      t         tests?");
  PrintAndLogEx(NORMAL, "      i <X>     set type of SIMD instructions. "
                        "Without this flag programs autodetect it.");
  ...
  PrintAndLogEx(NORMAL,
                _YELLOW_("      hf mf hardnested 0 A FFFFFFFFFFFF 4 A"));
  PrintAndLogEx(NORMAL,
                _YELLOW_("      hf mf hardnested 0 A FFFFFFFFFFFF 4 A w"));
  PrintAndLogEx(
      NORMAL,
      _YELLOW_("      hf mf hardnested 0 A FFFFFFFFFFFF 4 A f nonces.bin w s"));
  PrintAndLogEx(NORMAL, _YELLOW_("      hf mf hardnested r"));
  PrintAndLogEx(NORMAL, _YELLOW_("      hf mf hardnested r a0a1a2a3a4a5"));
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(NORMAL, "Add the known target key to check if it is present in "
                        "the remaining key space:");
  PrintAndLogEx(
      NORMAL,
      _YELLOW_("      hf mf hardnested 0 A A0A1A2A3A4A5 4 A FFFFFFFFFFFF"));
  return PM3_SUCCESS;
}

static int usage_hf14_autopwn(void) {
  PrintAndLogEx(NORMAL, "Usage:");
  PrintAndLogEx(NORMAL, "      hf mf autopwn [k] <sector number> <key A|B> "
                        "<key (12 hex symbols)>");
  PrintAndLogEx(NORMAL, "                    [* <card memory>] [f "
                        "<dictionary>[.dic]] [s] [i <simd type>] [l] [v]");
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(NORMAL, "Description:");
  PrintAndLogEx(NORMAL, "      This command automates the key recovery process "
                        "on MIFARE Classic cards.");
  PrintAndLogEx(NORMAL, "      It uses the darkside, nested, hardnested and "
                        "staticnested to recover keys.");
  PrintAndLogEx(NORMAL,
                "      If all keys are found, try dumping card content.");
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(NORMAL, "Options:");
  PrintAndLogEx(NORMAL, "      h                          this help");
  PrintAndLogEx(NORMAL,
  ...
  PrintAndLogEx(NORMAL, "      s                          slower acquisition "
                        "for hardnested (required by some non standard cards)");
  PrintAndLogEx(NORMAL,
                "      v                          verbose output (statistics)");
  PrintAndLogEx(NORMAL, "      l                          legacy mode (use the "
                        "slow 'mf chk' for the key enumeration)");
  PrintAndLogEx(
      NORMAL,
  ...
  PrintAndLogEx(NORMAL, "      i <simd type>              set type of SIMD "
                        "instructions for hardnested. Default: autodetection.");
#if defined(COMPILER_HAS_SIMD_AVX512)
  PrintAndLogEx(NORMAL, "        i 5   = AVX512");
#endif
#if defined(COMPILER_HAS_SIMD)
  ...
  PrintAndLogEx(NORMAL, "Examples:");
  PrintAndLogEx(
      NORMAL, _YELLOW_("      hf mf autopwn") "                                "
                                              "             -- target MIFARE "
                                              "Classic card with default keys");
  PrintAndLogEx(
      NORMAL,
      _YELLOW_(
  ...
  PrintAndLogEx(
      NORMAL,
      _YELLOW_(
          "      hf mf autopwn k 0 A FFFFFFFFFFFF") "                          "
                                                    "-- target MIFARE Classic "
                                                    "card with Sector0 typeA "
                                                    "with known key "
                                                    "'FFFFFFFFFFFF'");
  PrintAndLogEx(NORMAL,
                _YELLOW_("      hf mf autopwn k 0 A FFFFFFFFFFFF * 1 f "
                         "mfc_default_keys") "   -- this command combines the "
                                             "two above (reduce the need for "
                                             "nested / hardnested attacks, by "
                                             "using a dictionary)");
  return PM3_SUCCESS;
}
...
static int usage_hf14_keybrute(void) {
    PrintAndLogEx(NORMAL, "J_Run's 2nd phase of multiple sector nested
authentication key recovery"); PrintAndLogEx(NORMAL, "You have a known 4 last
bytes of a key recovered with mf_nonce_brute tool."); PrintAndLogEx(NORMAL,
"First 2 bytes of key will be bruteforced"); PrintAndLogEx(NORMAL, "");
    PrintAndLogEx(NORMAL, " ---[ This attack is obsolete,  try hardnested
instead ]---"); PrintAndLogEx(NORMAL, ""); PrintAndLogEx(NORMAL, "Usage:  hf mf
keybrute [h] <block number> <A|B> <key>"); PrintAndLogEx(NORMAL, "Options:");
    PrintAndLogEx(NORMAL, "      h               this help");
    PrintAndLogEx(NORMAL, "      <block number>  target block number");
    PrintAndLogEx(NORMAL, "      <A|B>           target key type");
    PrintAndLogEx(NORMAL, "      <key>           candidate key from
mf_nonce_brute tool"); PrintAndLogEx(NORMAL, "Examples:"); PrintAndLogEx(NORMAL,
_YELLOW_("           hf mf keybrute 1 A 000011223344")); return 0;
}
*/
static int usage_hf14_restore(void) {
  PrintAndLogEx(
      NORMAL, "Usage:   hf mf restore [card memory] u <UID> k <name> f <name>");
  PrintAndLogEx(NORMAL, "Options:");
  PrintAndLogEx(NORMAL, "  [card memory]: 0 = 320 bytes (MIFARE Mini), 1 = 1K "
                        "(default), 2 = 2K, 4 = 4K");
  PrintAndLogEx(NORMAL, "  u <UID>      : uid, try to restore from "
                        "hf-mf-<UID>-key.bin and hf-mf-<UID>-dump.bin");
  PrintAndLogEx(
      NORMAL,
      "  k <name>     : key filename, specific the full filename of key file");
  PrintAndLogEx(NORMAL, "  f <name>     : data filename, specific the full "
                        "filename of data file");
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(NORMAL, "Examples:");
  PrintAndLogEx(
      NORMAL,
      _YELLOW_("         hf mf restore") "                            -- read "
                                         "the UID from tag first, then restore "
                                         "from hf-mf-<UID>-key.bin and and "
                                         "hf-mf-<UID>-dump.bin");
  PrintAndLogEx(
      NORMAL,
      _YELLOW_(
          "         hf mf restore 1 u 12345678") "               -- restore "
                                                 "from hf-mf-12345678-key.bin "
                                                 "and hf-mf-12345678-dump.bin");
  PrintAndLogEx(NORMAL, _YELLOW_("         hf mf restore 1 u 12345678 k "
                                 "dumpkey.bin") " -- restore from dumpkey.bin "
                                                "and hf-mf-12345678-dump.bin");
  PrintAndLogEx(
      NORMAL,
      _YELLOW_(
          "         hf mf restore 4") "                          -- read the "
                                      "UID from tag with 4K memory first, then "
                                      "restore from hf-mf-<UID>-key.bin and "
                                      "and hf-mf-<UID>-dump.bin");
  return PM3_SUCCESS;
}
static int usage_hf14_decryptbytes(void) {
  PrintAndLogEx(NORMAL,
                "Decrypt Crypto-1 encrypted bytes given some known state of "
                "crypto. See tracelog to gather needed values\n");
  PrintAndLogEx(NORMAL,
                "Usage:   hf mf decrypt [h] <nt> <ar_enc> <at_enc> <data>");
  PrintAndLogEx(NORMAL, "Options:");
  PrintAndLogEx(NORMAL, "      h            this help");
  ...
  PrintAndLogEx(NORMAL, "      <data>       encrypted data, taken directly "
                        "after at_enc and forward");
  PrintAndLogEx(NORMAL, "Examples:");
  PrintAndLogEx(
      NORMAL,
      _YELLOW_("         hf mf decrypt b830049b 9248314a 9280e203 41e586f9"));
  PrintAndLogEx(NORMAL, "\n  this sample decrypts 41e586f9 -> 3003999a  "
                        "Annotated: 30 03 [99 9a]  read block 3 [crc]");
  return PM3_SUCCESS;
}
...
static int usage_hf14_eclr(void) {
  PrintAndLogEx(NORMAL, "It set card emulator memory to empty data blocks and "
                        "key A/B FFFFFFFFFFFF \n");
...
static int usage_hf14_eset(void) {
  ...
  PrintAndLogEx(
      NORMAL,
      _YELLOW_("        hf mf eset 1 000102030405060708090a0b0c0d0e0f"));
  return PM3_SUCCESS;
}
static int usage_hf14_eload(void) {
  ...
  PrintAndLogEx(
      NORMAL,
      "Usage:  hf mf eload [card memory] <file name w/o `.eml`> [numblocks]");
  PrintAndLogEx(NORMAL, "  [card memory]: 0 = 320 bytes (MIFARE Mini), 1 = 1K "
                        "(default), 2 = 2K, 4 = 4K, u = UL");
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(NORMAL, "Examples:");
...
static int usage_hf14_esave(void) {
  PrintAndLogEx(
      NORMAL,
      "It saves emul dump into the file `filename.eml` or `cardID.eml`");
  PrintAndLogEx(NORMAL,
                " Usage:  hf mf esave [card memory] [file name w/o `.eml`]");
  PrintAndLogEx(NORMAL, "  [card memory]: 0 = 320 bytes (MIFARE Mini), 1 = 1K "
                        "(default), 2 = 2K, 4 = 4K");
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(NORMAL, "Examples:");
...
static int usage_hf14_ekeyprn(void) {
  ...
  PrintAndLogEx(NORMAL, "Options:");
  PrintAndLogEx(NORMAL, "      h    this help");
  PrintAndLogEx(NORMAL, "      card memory - 0 - MINI(320 bytes), 1 - 1K, 2 - "
                        "2K, 4 - 4K, <other> - 1K");
  PrintAndLogEx(NORMAL,
...
static int usage_hf14_csetuid(void) {
  PrintAndLogEx(NORMAL, "Set UID, ATQA, and SAK for magic Chinese card. Only "
                        "works with magic cards");
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(NORMAL, "Usage:  hf mf csetuid [h] <UID 8 hex symbols> [ATQA 4 "
                        "hex symbols] [SAK 2 hex symbols] [w]");
  PrintAndLogEx(NORMAL, "Options:");
  PrintAndLogEx(NORMAL, "       h        this help");
...
static int usage_hf14_csetblk(void) {
  ...
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(NORMAL, "Usage:  hf mf csetblk [h] <block number> <block data "
                        "(32 hex symbols)> [w]");
  PrintAndLogEx(NORMAL, "Options:");
  PrintAndLogEx(NORMAL, "       h         this help");
  PrintAndLogEx(NORMAL, "       w         wipe card before writing");
  ...
  PrintAndLogEx(
      NORMAL,
      _YELLOW_("       hf mf csetblk 1 01020304050607080910111213141516"));
  PrintAndLogEx(
      NORMAL,
      _YELLOW_("       hf mf csetblk 1 01020304050607080910111213141516 w"));
  return PM3_SUCCESS;
}
static int usage_hf14_cload(void) {
  ...
  PrintAndLogEx(NORMAL, "       h            this help");
  PrintAndLogEx(NORMAL,
                "       e            load card with data from emulator memory");
  PrintAndLogEx(NORMAL,
                "       j <filename> load card with data from json file");
  PrintAndLogEx(NORMAL,
                "       b <filename> load card with data from binary file");
  PrintAndLogEx(NORMAL,
                "       <filename>   load card with data from eml file");
  PrintAndLogEx(NORMAL, "Examples:");
...
static int usage_hf14_csave(void) {
  PrintAndLogEx(NORMAL, "It saves `magic Chinese` card dump into the file "
                        "`filename.eml` or `cardID.eml`");
  PrintAndLogEx(NORMAL, "or into emulator memory");
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(
      NORMAL,
      "Usage:  hf mf csave [h] [e] [u] [card memory] o <file name w/o `.eml`>");
  PrintAndLogEx(NORMAL, "Options:");
  PrintAndLogEx(NORMAL, "       h             this help");
  PrintAndLogEx(NORMAL, "       e             save data to emulator memory");
  PrintAndLogEx(
      NORMAL,
      "       u             save data to file, use carduid as filename");
  PrintAndLogEx(NORMAL, "       card memory   0 = 320 bytes (MIFARE Mini), 1 = "
                        "1K (default), 2 = 2K, 4 = 4K");
...
static int usage_hf14_gen3uid(void) {
  PrintAndLogEx(
      NORMAL,
      "Set UID for magic GEN 3 card without manufacturer block changing");
  PrintAndLogEx(NORMAL, "");
...
static int usage_hf14_gen3block(void) {
  PrintAndLogEx(NORMAL,
                "Overwrite full manufacturer block for magic GEN 3 card");
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(
      NORMAL, "Usage:  hf mf gen3blk [h] [block data (up to 32 hex symbols)]");
  PrintAndLogEx(NORMAL, "Options:");
  PrintAndLogEx(NORMAL, "       h        this help");
  PrintAndLogEx(
      NORMAL,
      "       [block]  manufacturer block data up to 32 hex symbols to write");
  PrintAndLogEx(
      NORMAL,
      "                - If block data not specified, it prints current");
  PrintAndLogEx(NORMAL, "                  data without changes");
  PrintAndLogEx(
      NORMAL,
      "                - You can specify part of manufacturer block as");
  PrintAndLogEx(NORMAL,
                "                  4/7-bytes for UID change only for example");
  PrintAndLogEx(
      NORMAL,
      "                NOTE: BCC, SAK, ATQA will be calculated automatically");
  PrintAndLogEx(NORMAL, "Examples:");
  PrintAndLogEx(
      NORMAL, _YELLOW_("      hf mf gen3blk 01020304FFFFFFFF0102030405060708"));
...
static int usage_hf14_gen3freeze(void) {
  PrintAndLogEx(NORMAL, "Perma lock further UID changes. No more UID changes "
                        "available after operation completed");
  PrintAndLogEx(NORMAL, "");
...
static char *GenerateFilename(const char *prefix, const char *suffix) {
  ...
  int uidlen = 0;
  char *fptr = calloc(sizeof(char) * (strlen(prefix) + strlen(suffix)) +
                          sizeof(uid) * 2 + 1,
                      sizeof(uint8_t));
...
static void decode_print_st(uint16_t blockno, uint8_t *data) {
  if (mfIsSectorTrailer(blockno)) {
    ...
    PrintAndLogEx(NORMAL, "----------------------------------------------");
    PrintAndLogEx(NORMAL, "Key A      " _GREEN_("%s"),
                  sprint_hex_inrow(data, 6));
    PrintAndLogEx(NORMAL, "Key B      " _GREEN_("%s"),
                  sprint_hex_inrow(data + 10, 6));
    ...
    for (int i = 0; i < 4; i++) {
      PrintAndLogEx(NORMAL, "  block %d%s  " _YELLOW_("%s"), bln,
                    ((blinc > 1) && (i < 3) ? "+" : ""),
...
static int CmdHF14AMfDarkside(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mf darkside", "Darkside attack",
                "hf mf darkside\n"
  ...
  case -1:
    ...
    return PM3_ESOFT;
  case -2:
    PrintAndLogEx(FAILED, "card is not vulnerable to Darkside attack (doesn't "
                          "send NACK on authentication requests).");
    return PM3_ESOFT;
  case -3:
    PrintAndLogEx(FAILED, "card is not vulnerable to Darkside attack (its "
                          "random number generator is not predictable).");
    return PM3_ESOFT;
  case -4:
    PrintAndLogEx(FAILED,
                  "card is not vulnerable to Darkside attack (its random "
                  "number generator seems to be based on the wellknown");
    PrintAndLogEx(FAILED, "generating polynomial with 16 effective bits only, "
                          "but shows unexpected behaviour.");
    return PM3_ESOFT;
  case -5:
...
static int CmdHF14AMfWrBl(const char *Cmd) {
  ...
  if (strlen(Cmd) < 3) {
    PrintAndLogEx(NORMAL, "Usage:  hf mf wrbl    <block number> <key A/B> <key "
                          "(12 hex symbols)> <block data (32 hex symbols)>");
    PrintAndLogEx(NORMAL, "Examples:");
    PrintAndLogEx(
        NORMAL,
        "        hf mf wrbl 1 A FFFFFFFFFFFF 000102030405060708090A0B0C0D0E0F");
    return 0;
  }
  ...
  PrintAndLogEx(NORMAL, "--block no %d, key %c - %s", blockNo,
                keyType ? 'B' : 'A', sprint_hex(key, 6));
  ...
  clearCommandBuffer();
  SendCommandMIX(CMD_HF_MIFARE_WRITEBL, blockNo, keyType, 0, data,
                 sizeof(data));
...
static int CmdHF14AMfRdBl(const char *Cmd) {
  ...
  if (strlen(Cmd) < 3) {
    PrintAndLogEx(NORMAL, "Usage:  hf mf rdbl    <block number> <key A/B> <key "
                          "(12 hex symbols)>");
    PrintAndLogEx(NORMAL, "Examples:");
  ...
  clearCommandBuffer();
  SendCommandNG(CMD_HF_MIFARE_READBL, (uint8_t *)&payload,
...
static int CmdHF14AMfRdSc(const char *Cmd) {
  ...
  if (strlen(Cmd) < 3) {
    PrintAndLogEx(NORMAL, "Usage:  hf mf rdsc    <sector number> <key A/B> "
                          "<key (12 hex symbols)>");
    PrintAndLogEx(NORMAL, "Examples:");
  ...
  PrintAndLogEx(NORMAL, "--sector no %d, key %c - %s ", sectorNo,
                keyType ? 'B' : 'A', sprint_hex(key, 6));
  ...
  if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
    ...
    if (isOK) {
      ...
      for (int i = 0; i < blocks; i++) {
        PrintAndLogEx(NORMAL, "%3d | %s", start + i,
                      sprint_hex(data + (i * 16), 16));
      }
...
static int FastDumpWithEcFill(uint8_t numsectors) {
  ...
  if (resp.status != PM3_SUCCESS) {
    PrintAndLogEx(
        INFO, "fast dump reported back failure w KEY A,  swapping to KEY B");
...
static int CmdHF14AMfDump(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mf dump",
                ...
                "hf mf dump -f hf-mf-066C8B78-key-5.bin --> MIFARE 1k with "
                "keys from specified file\n");
  ...
  CLIParamStrToBuf(arg_get_str(ctx, 1), (uint8_t *)dataFilename, FILE_PATH_SIZE,
                   &datafnlen);

  int keyfnlen = 0;
  char keyFilename[FILE_PATH_SIZE] = {0};
  CLIParamStrToBuf(arg_get_str(ctx, 2), (uint8_t *)keyFilename, FILE_PATH_SIZE,
                   &keyfnlen);
  ...
  for (sectorNo = 0; sectorNo < numSectors; sectorNo++) {
    for (tries = 0; tries < MIFARE_SECTOR_RETRY; tries++) {
      ...
      payload.blockno =
          FirstBlockOfSector(sectorNo) + NumBlocksPerSector(sectorNo) - 1;
      payload.keytype = 0;
      ...
      if (WaitForResponseTimeout(CMD_HF_MIFARE_READBL, &resp, 1500)) {
        ...
        if (resp.status == PM3_SUCCESS) {
          rights[sectorNo][0] =
              ((data[7] & 0x10) >> 2) | ((data[8] & 0x1) << 1) |
              ((data[8] & 0x10) >> 4); // C1C2C3 for data area 0
          rights[sectorNo][1] =
              ((data[7] & 0x20) >> 3) | ((data[8] & 0x2) << 0) |
              ((data[8] & 0x20) >> 5); // C1C2C3 for data area 1
          rights[sectorNo][2] =
              ((data[7] & 0x40) >> 4) | ((data[8] & 0x4) >> 1) |
              ((data[8] & 0x40) >> 6); // C1C2C3 for data area 2
          rights[sectorNo][3] =
              ((data[7] & 0x80) >> 5) | ((data[8] & 0x8) >> 2) |
              ((data[8] & 0x80) >> 7); // C1C2C3 for sector trailer
          break;
        } else if (tries == 2) { // on last try set defaults
          PrintAndLogEx(FAILED,
                        "\ncould not get access rights for sector %2d. Trying "
                        "with defaults...",
                        sectorNo);
          rights[sectorNo][0] = rights[sectorNo][1] = rights[sectorNo][2] =
              0x00;
          rights[sectorNo][3] = 0x01;
        }
      } else {
        PrintAndLogEx(FAILED,
                      "\ncommand execute timeout when trying to read access "
                      "rights for sector %2d. Trying with defaults...",
                      sectorNo);
  ...
  for (sectorNo = 0; sectorNo < numSectors; sectorNo++) {
    for (blockNo = 0; blockNo < NumBlocksPerSector(sectorNo); blockNo++) {
      ...
      for (tries = 0; tries < MIFARE_SECTOR_RETRY; tries++) {
        if (blockNo == NumBlocksPerSector(sectorNo) -
                           1) { // sector trailer. At least the Access
                                // Conditions can always be read with key A.
          ...
          if ((rights[sectorNo][data_area] == 0x03) ||
              (rights[sectorNo][data_area] == 0x05)) { // only key B would work
            ...
            SendCommandNG(CMD_HF_MIFARE_READBL, (uint8_t *)&payload,
                          sizeof(mf_readblock_t));
            received =
                WaitForResponseTimeout(CMD_HF_MIFARE_READBL, &resp, 1500);
          } else if (rights[sectorNo][data_area] == 0x07) { // no key would work
            PrintAndLogEx(
                WARNING,
                "access rights do not allow reading of sector %2d block %3d",
                sectorNo, blockNo);
      ...
      if (received) {
        ...
        if (resp.status == PM3_SUCCESS) {
          ...
          PrintAndLogEx(SUCCESS, "successfully read block %2d of sector %2d.",
                        blockNo, sectorNo);
        } else {
          PrintAndLogEx(FAILED, "could not read block %2d of sector %2d",
                        blockNo, sectorNo);
          break;
        }
      } else {
        PrintAndLogEx(WARNING,
                      "command execute timeout when trying to read block %2d "
                      "of sector %2d.",
                      blockNo, sectorNo);
        break;
      }
  ...
  uint16_t bytes = 16 * (FirstBlockOfSector(numSectors - 1) +
                         NumBlocksPerSector(numSectors - 1));
...
static int CmdHF14AMfRestore(const char *Cmd) {
  ...
  for (sectorNo = 0; sectorNo < numSectors; sectorNo++) {
    for (blockNo = 0; blockNo < NumBlocksPerSector(sectorNo); blockNo++) {
      ...
      PrintAndLogEx(NORMAL, "Writing to block %3d: %s",
                    FirstBlockOfSector(sectorNo) + blockNo,
                    sprint_hex(bldata, 16));

      memcpy(data + 10, bldata, 16);
      clearCommandBuffer();
      SendCommandMIX(CMD_HF_MIFARE_WRITEBL,
                     FirstBlockOfSector(sectorNo) + blockNo, keyType, 0, data,
                     sizeof(data));
...
static int CmdHF14AMfNested(const char *Cmd) {
  ...
  CLIParserInit(
      ...
      "hf mf nested --single --blk 0 -a FFFFFFFFFFFF --tblk 4 --tkeya     --> "
      "Single sector key recovery. Use block 0 Key A to find block 4 Key A\n"
      "hf mf nested --mini --blk 0 -a -k FFFFFFFFFFFF                     --> "
      "Key recovery against MIFARE Mini\n"
      "hf mf nested --1k --blk 0 -a -k FFFFFFFFFFFF                       --> "
      "Key recovery against MIFARE Classic 1k\n"
      "hf mf nested --2k --blk 0 -a -k FFFFFFFFFFFF                       --> "
      "Key recovery against MIFARE 2k\n"
      "hf mf nested --4k --blk 0 -a -k FFFFFFFFFFFF                       --> "
      "Key recovery against MIFARE 4k");
  ...
  if (mfCheckKeys(blockNo, keyType, true, 1, key, &key64) != PM3_SUCCESS) {
    PrintAndLogEx(WARNING,
                  "Wrong key. Can't authenticate to block:%3d key type:%c",
                  blockNo, keyType ? 'B' : 'A');
    return PM3_EOPABORTED;
  }
  ...
  if (singleSector) {
    int16_t isOK =
        mfnested(blockNo, keyType, key, trgBlockNo, trgKeyType, keyBlock, true);
    switch (isOK) {
    case PM3_ETIMEOUT:
    ...
    for (int cnt = 0; cnt < ARRAYLEN(g_mifare_default_keys); cnt++) {
      num_to_bytes(g_mifare_default_keys[cnt], 6,
                   (uint8_t *)(keyBlock + cnt * 6));
    }

    PrintAndLogEx(SUCCESS, "Testing known keys. Sector count "_YELLOW_("%d"),
                  SectorsCnt);
    int res = mfCheckKeys_fast(SectorsCnt, true, true, 1,
                               ARRAYLEN(g_mifare_default_keys) + 1, keyBlock,
                               e_sector, false);
    if (res == PM3_SUCCESS) {
    ...
    PrintAndLogEx(
        SUCCESS, "Time to check " _YELLOW_("%zu") " known keys: %.0f seconds\n",
    ...
    for (trgKeyType = 0; trgKeyType < 2; ++trgKeyType) {
      for (uint8_t sectorNo = 0; sectorNo < SectorsCnt; ++sectorNo) {
        for (int i = 0; i < MIFARE_SECTOR_RETRY; i++) {
          ...
          int16_t isOK =
              mfnested(blockNo, keyType, key, FirstBlockOfSector(sectorNo),
                       trgKeyType, keyBlock, calibrate);
          switch (isOK) {
          case PM3_ETIMEOUT:
            PrintAndLogEx(ERR, "Command execute timeout\n");
            break;
          case PM3_EOPABORTED:
            ...
            break;
          case PM3_EFAILED:
            PrintAndLogEx(FAILED, "Tag isn't vulnerable to Nested Attack (PRNG "
                                  "is not predictable).\n");
            break;
          case PM3_ESOFT:
            // key not found
          ...
          case PM3_SUCCESS:
            ...
            mfCheckKeys_fast(SectorsCnt, true, true, 2, 1, keyBlock, e_sector,
                             false);
            continue;
          default:
            PrintAndLogEx(ERR, "Unknown error.\n");
    ...
    // 20160116 If Sector A is found, but not Sector B,  try just reading it of
    // the tag?
  ...
  jumptoend:
    ...
    if (transferToEml) {
      ...
      for (int i = 0; i < SectorsCnt; i++) {
        mfEmlGetMem(keyBlock, FirstBlockOfSector(i) + NumBlocksPerSector(i) - 1,
                    1);
        ...
        mfEmlSetMem(keyBlock, FirstBlockOfSector(i) + NumBlocksPerSector(i) - 1,
                    1);
      }
...
static int CmdHF14AMfNestedStatic(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mf staticnested",
                "Execute Nested attack against MIFARE Classic card with static "
                "nonce for key recovery",
  ...
  for (trgKeyType = 0; trgKeyType < 2; ++trgKeyType) {
    for (uint8_t sectorNo = 0; sectorNo < SectorsCnt; ++sectorNo) {
      ...
      for (int i = 0; i < 1; i++) {
        ...
        int16_t isOK =
            mfStaticNested(blockNo, keyType, key, FirstBlockOfSector(sectorNo),
                           trgKeyType, keyBlock);
        switch (isOK) {
        case PM3_ETIMEOUT:
          PrintAndLogEx(ERR, "Command execute timeout");
...
static int CmdHF14AMfNestedHard(const char *Cmd) {
  ...
  if (!know_target_key && nonce_file_read == false) {
    ...
    if (mfCheckKeys(blockNo, keyType, true, 1, key, &key64) != PM3_SUCCESS) {
      PrintAndLogEx(
          WARNING,
          "Key is wrong. Can't authenticate to block: %3d  key type: %c",
          blockNo, keyType ? 'B' : 'A');
      return 3;
    }
  }
  ...
  PrintAndLogEx(INFO,
                "Target block no:%3d, target key type:%c, known target key: "
                "0x%02x%02x%02x%02x%02x%02x%s",
                trgBlockNo, trgKeyType ? 'B' : 'A', trgkey[0], trgkey[1],
                trgkey[2], trgkey[3], trgkey[4], trgkey[5],
  ...
  int16_t isOK =
      mfnestedhard(blockNo, keyType, key, trgBlockNo, trgKeyType,
                   know_target_key ? trgkey : NULL, nonce_file_read,
                   nonce_file_write, slow, tests, &foundkey, filename);
...
static int CmdHF14AMfAutoPWN(const char *Cmd) {
  ...
  while ((ctmp = param_getchar(Cmd, cmdp)) && !errors) {
    ...
    case 'f':
      if (param_getstr(Cmd, cmdp + 1, filename, FILE_PATH_SIZE) >=
          FILE_PATH_SIZE) {
  ...
  if (verbose) {
    ...
    PrintAndLogEx(INFO, " key supplied .. " _YELLOW_("%s"),
                  know_target_key ? "True" : "False");
    PrintAndLogEx(INFO, " known sector .. " _YELLOW_("%d"), blockNo);
    PrintAndLogEx(INFO, " keytype ....... " _YELLOW_("%c"),
                  keyType ? 'B' : 'A');
    PrintAndLogEx(INFO, " known key ..... " _YELLOW_("%s"),
    ...
    if (has_staticnonce == NONCE_STATIC)
      ...
      PrintAndLogEx(INFO, " card PRNG ..... " _YELLOW_("%s"),
                    prng_type ? "WEAK" : "HARD");
    else
      PrintAndLogEx(INFO,
                    " card PRNG ..... " _YELLOW_(
                        "Could not determine PRNG,") " " _RED_("read failed."));

    PrintAndLogEx(INFO, " dictionary .... " _YELLOW_("%s"),
                  strlen(filename) ? filename : "NONE");
    PrintAndLogEx(INFO, " legacy mode ... " _YELLOW_("%s"),
                  legacy_mfchk ? "True" : "False");
  ...
  if (know_target_key == false) {
    ...
    if (verbose) {
      PrintAndLogEx(INFO,
                    "======================= " _YELLOW_(
                        "START KNOWN KEY ATTACK") " =======================");
    }

    if (mfCheckKeys(FirstBlockOfSector(blockNo), keyType, true, 1, key,
                    &key64) == PM3_SUCCESS) {
      PrintAndLogEx(
          INFO,
          "target sector:%3u key type: %c -- using valid key [ " _GREEN_(
              "%s") "] (used for nested / hardnested attack)",
          blockNo, keyType ? 'B' : 'A', sprint_hex(key, sizeof(key)));

      // Store the key for the nested / hardnested attack (if supplied by the
      // user)
      ...
      PrintAndLogEx(FAILED,
                    "Key is wrong. Can't authenticate to sector:"_RED_(
                        "%3d") " key type: "_RED_("%c") " key: " _RED_("%s"),
                    blockNo, keyType ? 'B' : 'A', sprint_hex(key, sizeof(key)));
    ...
    for (int i = 0; i < sectors_cnt; i++) {
      for (int j = 0; j < 2; j++) {
        if (e_sector[i].foundKey[j] == 0) {
          if (mfCheckKeys(FirstBlockOfSector(i), j, true, 1, key, &key64) ==
              PM3_SUCCESS) {
            e_sector[i].Key[j] = bytes_to_num(key, 6);
            e_sector[i].foundKey[j] = 'U';

            // If the user supplied secctor / keytype was wrong --> just be nice
            // and correct it ;)
            if (know_target_key == false) {
              ...
              PrintAndLogEx(
                  SUCCESS,
                  "target sector:%3u key type: %c -- found valid key "
                  "[ " _GREEN_("%s") "] (used for nested / hardnested attack)",
                  i, j ? 'B' : 'A', sprint_hex(key, sizeof(key)));
            } else {
              PrintAndLogEx(SUCCESS,
                            "target sector:%3u key type: %c -- found valid key "
                            "[ " _GREEN_("%s") "]",
  ...
  if (has_filename) {
    ...
    if (res != PM3_SUCCESS || key_cnt == 0 || keyBlock == NULL) {
      PrintAndLogEx(FAILED, "An error occurred while loading the dictionary! "
                            "(we will use the default keys now)");
      if (keyBlock != NULL) {
        free(keyBlock);
      }
      load_success = false;
  ...
  if (has_filename == false || load_success == false) {
    ...
    PrintAndLogEx(SUCCESS,
                  "loaded " _GREEN_("%2d") " keys from hardcoded default array",
                  key_cnt);
  }
  ...
  if (legacy_mfchk) {
      ...
      for (int j = 0; j < 2; j++) {
        ...
        if (e_sector[i].foundKey[j] == 0) {
          for (uint32_t k = 0; k < key_cnt; k++) {
            ...
            if (mfCheckKeys(FirstBlockOfSector(i), j, true, 1,
                            (keyBlock + (6 * k)), &key64) == PM3_SUCCESS) {
    ...
    uint32_t chunksize =
        key_cnt > (PM3_CMD_DATA_SIZE / 6) ? (PM3_CMD_DATA_SIZE / 6) : key_cnt;
    ...
    for (uint8_t strategy = 1; strategy < 3; strategy++) {
      ...
      for (uint32_t i = 0; i < key_cnt; i += chunksize) {
        ...
        res = mfCheckKeys_fast(sectors_cnt, firstChunk, lastChunk, strategy,
                               size, keyBlock + (i * 6), e_sector, false);
        if (firstChunk)
          firstChunk = false;
        // all keys,  aborted
        if (res == PM3_SUCCESS) {
  ...
  for (int i = 0; i < sectors_cnt; i++) {
    for (int j = 0; j < 2; j++) {
      if (e_sector[i].foundKey[j] == 1) {
        ...
        // Store valid credentials for the nested / hardnested attack if none
        // exist
        if (know_target_key == false) {
          num_to_bytes(e_sector[i].Key[j], 6, key);
          know_target_key = true;
          ...
          PrintAndLogEx(
              SUCCESS,
              "target sector:%3u key type: %c -- found valid key [ " _GREEN_(
                  "%s") "] (used for nested / hardnested attack)",
              i, j ? 'B' : 'A', sprint_hex(tmp_key, sizeof(tmp_key)));
        } else {
          PrintAndLogEx(
              SUCCESS,
              "target sector:%3u key type: %c -- found valid key [ " _GREEN_(
  ...
  if (know_target_key == false) {
    ...
    if (prng_type && has_staticnonce != NONCE_STATIC) {
      ...
      case -2:
        PrintAndLogEx(FAILED,
                      "\nCard is not vulnerable to Darkside attack (doesn't "
                      "send NACK on authentication requests).");
        goto noValidKeyFound;
      case -3:
        PrintAndLogEx(FAILED,
                      "\nCard is not vulnerable to Darkside attack (its random "
                      "number generator is not predictable).");
        goto noValidKeyFound;
      case -4:
        PrintAndLogEx(FAILED,
                      "\nCard is not vulnerable to Darkside attack (its random "
                      "number generator seems to be based on the wellknown");
        PrintAndLogEx(FAILED, "generating polynomial with 16 effective bits "
                              "only, but shows unexpected behaviour.");
        goto noValidKeyFound;
      case -5:
        PrintAndLogEx(WARNING, "\nAborted via keyboard.");
        goto noValidKeyFound;
      ...
      PrintAndLogEx(
          SUCCESS,
          "target sector:%3u key type: %c -- found valid key [ " _GREEN_(
              "%012" PRIx64) " ] (used for nested / hardnested attack)",
          blockNo, keyType ? 'B' : 'A', key64);
    } else {
    noValidKeyFound:
  ...
  for (current_sector_i = 0; current_sector_i < sectors_cnt;
       current_sector_i++) {
    for (current_key_type_i = 0; current_key_type_i < 2; current_key_type_i++) {
      ...
      if (e_sector[current_sector_i].foundKey[current_key_type_i] == 0) {
        ...
        if (bytes_to_num(tmp_key, 6) != 0) {
          // <!> The fast check --> mfCheckKeys_fast(sectors_cnt, true, true, 2,
          // 1, tmp_key, e_sector, false);
          ...
          for (int i = 0; i < sectors_cnt; i++) {
            for (int j = 0; j < 2; j++) {
              ...
              if (mfCheckKeys(FirstBlockOfSector(i), j, true, 1, tmp_key,
                              &key64) == PM3_SUCCESS) {
                e_sector[i].Key[j] = bytes_to_num(tmp_key, 6);
                e_sector[i].foundKey[j] = 'R';
                PrintAndLogEx(SUCCESS,
                              "target sector:%3u key type: %c -- found valid "
                              "key [ " _GREEN_("%s") "]",
                              i, j ? 'B' : 'A',
        ...
        if (current_key_type_i == 1) {
          if (e_sector[current_sector_i].foundKey[0] &&
              !e_sector[current_sector_i].foundKey[1]) {
            if (verbose) {
              PrintAndLogEx(
                  INFO,
                  "======================= " _YELLOW_(
                      "START READ B KEY ATTACK") " =======================");
              PrintAndLogEx(INFO, "reading  B  key: sector: %3d key type: %c",
                            current_sector_i, current_key_type_i ? 'B' : 'A');
            }
            uint8_t sectrail = (FirstBlockOfSector(current_sector_i) +
                                NumBlocksPerSector(current_sector_i) - 1);

            mf_readblock_t payload;
            payload.blockno = sectrail;
            payload.keytype = 0;

            num_to_bytes(e_sector[current_sector_i].Key[0], 6,
                         payload.key); // KEY A
            ...
            if (key64) {
              ...
              if (verbose) {
                PrintAndLogEx(WARNING,
                              ...
                              current_sector_i, current_key_type_i ? 'B' : 'A');
                PrintAndLogEx(INFO, " -- reading the B key was not possible, "
                                    "maybe due to access rights?");
              }
            }
          }
        }
      ...
      skipReadBKey:
        if (e_sector[current_sector_i].foundKey[current_key_type_i] == 0) {
          ...
          tryNested:
            isOK = mfnested(FirstBlockOfSector(blockNo), keyType, key,
                            FirstBlockOfSector(current_sector_i),
                            current_key_type_i, tmp_key, calibrate);
            ...
            case PM3_EFAILED: {
              PrintAndLogEx(FAILED, "Tag isn't vulnerable to Nested Attack "
                                    "(PRNG is probably not predictable).");
            ...
            case PM3_ESOFT: {
              // key not found
              calibrate = false;
              // this can happen on some old cards, it's worth trying some more
              // before switching to slower hardnested
              if (retries++ < MIFARE_SECTOR_RETRY) {
                PrintAndLogEx(FAILED,
                              "Nested attack failed, trying again (%i/%i)",
                              retries, MIFARE_SECTOR_RETRY);
                goto tryNested;
              } else {
                PrintAndLogEx(FAILED,
            ...
            case PM3_SUCCESS: {
              calibrate = false;
              e_sector[current_sector_i].Key[current_key_type_i] =
                  bytes_to_num(tmp_key, 6);
          ...
          tryHardnested
            ...
            isOK = mfnestedhard(FirstBlockOfSector(blockNo), keyType, key,
                                FirstBlockOfSector(current_sector_i),
                                current_key_type_i, NULL, false, false, slow, 0,
                                &foundkey, NULL);
            DropField();
            if (isOK) {
              switch (isOK) {
            ...
            // Copy the found key to the tmp_key variale (for the following
            // print statement, and the mfCheckKeys above)
          ...
          tryStaticnested:
            if (verbose) {
              PrintAndLogEx(
                  INFO,
                  "======================= " _YELLOW_(
                      "START STATIC NESTED ATTACK") " =======================");
              PrintAndLogEx(INFO, "sector no: %3d, target key type: %c",
                            current_sector_i, current_key_type_i ? 'B' : 'A');
            }

            isOK = mfStaticNested(blockNo, keyType, key,
                                  FirstBlockOfSector(current_sector_i),
                                  current_key_type_i, tmp_key);
            DropField();
            switch (isOK) {
            case PM3_ETIMEOUT: {
              PrintAndLogEx(ERR, "\nError: No response from Proxmark3.");
...
all_found:
  ...
  t1 = msclock() - t1;
  PrintAndLogEx(INFO, "autopwn execution time: " _YELLOW_("%.0f") " seconds",
                (float)t1 / 1000.0);
...
static int CmdHF14AMfChk_fast(const char *Cmd) {
  ...
  CLIParserInit(
      &ctx, "hf mf fchk",
      "This is a improved checkkeys method speedwise. It checks MIFARE Classic "
      "tags sector keys against a dictionary file with keys",
      "hf mf fchk --mini -k FFFFFFFFFFFF              --> Key recovery against "
      "MIFARE Mini\n"
      "hf mf fchk --1k -k FFFFFFFFFFFF                --> Key recovery against "
      "MIFARE Classic 1k\n"
      "hf mf fchk --2k -k FFFFFFFFFFFF                --> Key recovery against "
      "MIFARE 2k\n"
      "hf mf fchk --4k -k FFFFFFFFFFFF                --> Key recovery against "
      "MIFARE 4k\n"
      "hf mf fchk --1k -f mfc_default_keys.dic        --> Target 1K using "
      "default dictionary file\n"
      "hf mf fchk --1k --emu                          --> Target 1K, write to "
      "emulator memory\n"
      "hf mf fchk --1k --dump                         --> Target 1K, write to "
      "file\n"
      "hf mf fchk --1k --mem                          --> Target 1K, use "
      "dictionary from flashmemory");
  ...
  int fnlen = 0;
  char filename[FILE_PATH_SIZE] = {0};
  CLIParamStrToBuf(arg_get_str(ctx, 9), (uint8_t *)filename, FILE_PATH_SIZE,
                   &fnlen);
  ...
  if (keylen >= 6) {
    ...
    for (int i = 0; i < numKeys; i++) {
      PrintAndLogEx(NORMAL, "[%2d] key %s", keycnt,
                    sprint_hex((keyBlock + 6 * keycnt), 6));
      keycnt++;
    }
  }
  ...
  if (fnlen > 0) {
    char *dict_path;
    int res =
        searchFile(&dict_path, DICTIONARIES_SUBDIR, filename, ".dic", false);
    if (res != PM3_SUCCESS) {
      free(keyBlock);
      return PM3_EFILE;
    }
    f = fopen(dict_path, "r");
    if (!f) {
      PrintAndLogEx(FAILED, "File: " _YELLOW_("%s") ": not found or locked.",
                    dict_path);
      free(dict_path);
      free(keyBlock);
      return PM3_EFILE;
    ...
    while (fgets(buf, sizeof(buf), f)) {
      ...
      if (buf[0] == '#')
        continue; // The line start with # is comment, skip

      // codesmell, only checks first char?
      if (!isxdigit(buf[0])) {
        PrintAndLogEx(FAILED,
                      "File content error. '" _YELLOW_(
                          "%s") "' must include 12 HEX symbols",
                      buf);
        continue;
      }
      ...
      // PrintAndLogEx(NORMAL, "check key[%2d] %012" PRIx64, keycnt,
      // bytes_to_num(keyBlock + 6*keycnt, 6));
      keycnt++;
      memset(buf, 0, sizeof(buf));
    }
    fclose(f);
    PrintAndLogEx(SUCCESS, "Loaded %2d keys from " _YELLOW_("%s"), keycnt,
                  filename);
  }
  ...
  if (keycnt == 0 && !use_flashmemory) {
    ...
    for (; keycnt < ARRAYLEN(g_mifare_default_keys); keycnt++)
      PrintAndLogEx(NORMAL, "[%2d] %02x%02x%02x%02x%02x%02x", keycnt,
                    (keyBlock + 6 * keycnt)[0], (keyBlock + 6 * keycnt)[1],
                    (keyBlock + 6 * keycnt)[2], (keyBlock + 6 * keycnt)[3],
                    (keyBlock + 6 * keycnt)[4], (keyBlock + 6 * keycnt)[5]);
  }
  ...
  uint32_t chunksize =
      keycnt > (PM3_CMD_DATA_SIZE / 6) ? (PM3_CMD_DATA_SIZE / 6) : keycnt;
  bool firstChunk = true, lastChunk = false;
  ...
  if (use_flashmemory) {
    ...
    mfCheckKeys_fast(sectorsCnt, true, true, 1, 0, keyBlock, e_sector,
                     use_flashmemory);
  } else {
    ...
    for (uint8_t strategy = 1; strategy < 3; strategy++) {
      ...
      for (i = 0; i < keycnt; i += chunksize) {
        ...
        res = mfCheckKeys_fast(sectorsCnt, firstChunk, lastChunk, strategy,
                               size, keyBlock + (i * 6), e_sector, false);
  ...
  if (found_keys == 0) {
    ...
    if (use_flashmemory && found_keys == (sectorsCnt << 1)) {
      PrintAndLogEx(SUCCESS, "Card dumped as well. run " _YELLOW_("`%s %c`"),
                    "hf mf esave", GetFormatFromSector(sectorsCnt));
    }
...
static int CmdHF14AMfChk(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mf chk", "Check keys on MIFARE Classic card",
                "hf mf chk --mini -k FFFFFFFFFFFF              --> Check all "
                "sectors, all keys against MIFARE Mini\n"
                "hf mf chk --1k -k FFFFFFFFFFFF                --> Check all "
                "sectors, all keys against MIFARE Classic 1k\n"
                "hf mf chk --2k -k FFFFFFFFFFFF                --> Check all "
                "sectors, all keys against MIFARE 2k\n"
                "hf mf chk --4k -k FFFFFFFFFFFF                --> Check all "
                "sectors, all keys against MIFARE 4k\n"
                "hf mf chk --1k --emu                          --> Check all "
                "sectors, all keys, 1K, and write to emulator memory\n"
                "hf mf chk --1k --dump                         --> Check all "
                "sectors, all keys, 1K, and write to file\n"
                "hf mf chk -a --blk 0 -f mfc_default_keys.dic  --> Check "
                "dictionary against block 0, key A");
  ...
  int fnlen = 0;
  char filename[FILE_PATH_SIZE] = {0};
  CLIParamStrToBuf(arg_get_str(ctx, 12), (uint8_t *)filename, FILE_PATH_SIZE,
                   &fnlen);
  ...
  // check keys.
  for (trgKeyType = (keyType == 2) ? 0 : keyType; trgKeyType < 2;
       (keyType == 2) ? (++trgKeyType) : (trgKeyType = 2)) {

    // loop sectors but block is used as to keep track of from which blocks to
    // test
    int b = blockNo;
    for (int i = 0; i < SectorsCnt; ++i) {

      // skip already found keys.
      if (e_sector[i].foundKey[trgKeyType])
        continue;
      ...
      for (uint16_t c = 0; c < keycnt; c += max_keys) {
        ...
        if (mfCheckKeys(b, trgKeyType, clearLog, size, &keyBlock[6 * c],
                        &key64) == PM3_SUCCESS) {
  ...
  if (keyType != 1) {
    ...
    for (int i = 0; i < SectorsCnt; i++) {
      ...
      if (e_sector[i].foundKey[0] && !e_sector[i].foundKey[1]) {
        ...
        PrintAndLogEx(INFO,
                      "Sector %u, First block of sector %u, Num of block %u", s,
                      FirstBlockOfSector(s), NumBlocksPerSector(s));
...
void readerAttack(sector_t *k_sector, uint8_t k_sectorsCount, nonces_t data,
                  bool setEmulatorMem, bool verbose) {
  ...
  if (success) {
    ...
    if (setEmulatorMem) {
      uint8_t memBlock[16] = {0,    0,    0, 0, 0, 0, 0xff, 0x0F,
                              0x80, 0x69, 0, 0, 0, 0, 0,    0};
      ...
      PrintAndLogEx(INFO, "Setting Emulator Memory Block %02d: [%s]",
                    (sector * 4) + 3, sprint_hex(memBlock, sizeof(memBlock)));
...
static int CmdHF14AMfSim(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mf sim", "Simulate MIFARE card",
                "hf mf sim --mini                         --> MIFARE Mini\n"
                "hf mf sim --1k                           --> MIFARE Classic "
                "1k (default)\n"
                "hf mf sim --1k -u 0a0a0a0a               --> MIFARE Classic "
                "1k with 4b UID\n"
                "hf mf sim --1k -u 11223344556677         --> MIFARE Classic "
                "1k with 7b UID\n"
                "hf mf sim --1k -u 11223344 -i --crack    --> Perform reader "
                "attack in interactive mode\n"
  ...
  void *argtable[] = {
      arg_param_begin,
      arg_str0("u", "uid", "<hex>",
               "UID 4,7 or 10bytes. If not specified, the UID 4b/7b from "
               "emulator memory will be used"),
      arg_lit0(NULL, "mini", "MIFARE Classic Mini / S20"),
      arg_lit0(NULL, "1k", "MIFARE Classic 1k / S50"),
      arg_lit0(NULL, "2k", "MIFARE Classic/Plus 2k"),
      arg_lit0(NULL, "4k", "MIFARE Classic 4k / S70"),
      arg_str0(NULL, "atqa", "<hex>",
               "Provide explicit ATQA (2 bytes, overrides option t)"),
      arg_str0(NULL, "sak", "<hex>",
               "Provide explicit SAK (1 bytes, overrides option t)"),
      arg_int0("n", "num", "<dec> ",
               "Automatically exit simulation after <numreads> blocks have "
               "been read by reader. 0 = infinite"),
      arg_lit0("i", "interactive",
               "Console will not be returned until simulation finishes or is "
               "aborted"),
      arg_lit0(NULL, "crack",
  ...
  if ((flags & (FLAG_4B_UID_IN_DATA | FLAG_7B_UID_IN_DATA |
                FLAG_10B_UID_IN_DATA)) == 0) {
...
void printKeyTableEx(uint8_t sectorscnt, sector_t *e_sector,
  ...
  for (uint8_t i = 0; i < sectorscnt; i++) {
    ...
    if (e_sector[i].foundKey[0] > 1) {
      PrintAndLogEx(SUCCESS,
                    "| "_YELLOW_("%03d") " | " _GREEN_("%s") "   | " _YELLOW_(
                        "%c") " | " _GREEN_("%s") "   | " _YELLOW_("%c") " |",
                    i, strA, e_sector[i].foundKey[0], strB,
      ...
      PrintAndLogEx(SUCCESS,
                    "| "_YELLOW_("%03d") " | " _GREEN_("%s") "   | " _YELLOW_(
                        "%d") " | " _GREEN_("%s") "   | " _YELLOW_("%d") " |",
                    s, strA, e_sector[i].foundKey[0], strB,
                    e_sector[i].foundKey[1]);
    }
  }
  PrintAndLogEx(SUCCESS, "|-----|----------------|---|----------------|---|");
  ...
  if (e_sector[0].foundKey[0] > 1) {
    PrintAndLogEx(
        INFO, "( " _YELLOW_("D") ":Dictionary / " _YELLOW_("S") ":darkSide "
                                                                "/ " _YELLOW_("U") ":User / " _YELLOW_("R") ":Reused / " _YELLOW_("N") ":Nested / " _YELLOW_(
                                                                    "H") ":Hard"
                                                                         "neste"
                                                                         "d "
                                                                         "/"
                                                                         " " _YELLOW_("C") ":statiCnested / " _YELLOW_("A") ":keyA "
                                                                                                                            ")");
  } else {
    PrintAndLogEx(SUCCESS,
...
static int CmdHF14AMfEGetBlk(const char *Cmd) {
  ...
  if (mfEmlGetMem(data, blockNo, 1) == PM3_SUCCESS) {
    PrintAndLogEx(NORMAL, "data[%3d]:%s", blockNo,
...
static int CmdHF14AMfEGetSc(const char *Cmd) {
  ...
  for (int i = 0; i < blocks; i++) {
    ...
    if (res == PM3_SUCCESS) {
      if (start + i == 0) {
        PrintAndLogEx(INFO, "%03d | " _RED_("%s"), start + i,
                      sprint_hex_ascii(data, sizeof(data)));
      } else if (mfIsSectorTrailer(i)) {
        PrintAndLogEx(INFO, "%03d | " _YELLOW_("%s"), start + i,
                      sprint_hex_ascii(data, sizeof(data)));
      } else {
        PrintAndLogEx(INFO, "%03d | %s ", start + i,
                      sprint_hex_ascii(data, sizeof(data)));
      }
    }
  }
...
int CmdHF14AMfELoad(const char *Cmd) {
  ...
  if (blockWidth == 4) {
    ...
    if ((blockNum != numBlocks)) {
      PrintAndLogEx(WARNING,
                    "Warning, Ultralight/Ntag file content, Loaded %d blocks "
                    "of expected %d blocks into emulator memory",
                    blockNum, numBlocks);
      return PM3_SUCCESS;
    }
  } else {
    ...
    if ((blockNum != numBlocks)) {
      PrintAndLogEx(WARNING,
                    "Error, file content, Only loaded %d blocks, must be %d "
                    "blocks into emulator memory",
                    blockNum, numBlocks);
...
static int CmdHF14AMfEView(const char *Cmd) {
  ...
  for (uint16_t i = 0; i < blocks; i++) {
    if (i == 0) {
      PrintAndLogEx(INFO, "%03d | " _RED_("%s"), i,
                    sprint_hex_ascii(dump + (i * 16), 16));
    } else if (mfIsSectorTrailer(i)) {
      PrintAndLogEx(INFO, "%03d | " _YELLOW_("%s"), i,
                    sprint_hex_ascii(dump + (i * 16), 16));
    } else {
      PrintAndLogEx(INFO, "%03d | %s ", i,
                    sprint_hex_ascii(dump + (i * 16), 16));
    }
  }
  PrintAndLogEx(INFO, "----+-------------------------------------------------+-"
...
static int CmdHF14AMfEKeyPrn(const char *Cmd) {
  ...
  for (int i = 0; i < sectors_cnt; i++) {
    ...
    if (mfEmlGetMem(data, FirstBlockOfSector(i) + NumBlocksPerSector(i) - 1,
                    1) != PM3_SUCCESS) {
      PrintAndLogEx(WARNING, "error get block %d",
                    FirstBlockOfSector(i) + NumBlocksPerSector(i) - 1);
...
static int CmdHF14AMfCSetUID(const char *Cmd) {
  ...
  PrintAndLogEx(NORMAL, "--wipe card:%s  uid:%s", (wipeCard) ? "YES" : "NO",
                sprint_hex(uid, 4));

  res = mfCSetUID(uid, (atqaPresent) ? atqa : NULL, (atqaPresent) ? sak : NULL,
                  oldUid, wipeCard);
  if (res) {
...
static int CmdHF14AMfCWipe(const char *cmd) {
  ...
  CLIParserInit(&ctx, "hf mf cwipe",
                "Wipe gen1 magic chinese card. Set "
                "UID/ATQA/SAK/Data/Keys/Access to default values.",
                "hf mf cwipe                           --> wipe card\n"
                "hf mf cwipe -u 09080706 -a 0004 -s 18 --> set UID, ATQA and "
                "SAK and wipe card");

  void *argtable[] = {
      arg_param_begin, arg_str0("u", "uid", "<UID (hex 4b)>", "UID for card"),
      arg_str0("a", "atqa", "<ATQA (hex 2b)>", "ATQA for card"),
      arg_str0("s", "sak", "<SAK (hex 1b)>", "SAK for card"), arg_param_end};
  CLIExecWithReturn(ctx, cmd, argtable, true);
  ...
  int res = mfCWipe((uidLen) ? uid : NULL, (atqaLen) ? atqa : NULL,
                    (sakLen) ? sak : NULL);
  if (res) {
...
static int CmdHF14AMfCSetBlk(const char *Cmd) {
  ...
  PrintAndLogEx(NORMAL, "--block number:%2d data:%s", blockNo,
                sprint_hex(block, 16));
...
static int CmdHF14AMfCLoad(const char *Cmd) {
  ...
  if (fillFromEmulator) {
    for (int blockNum = 0; blockNum < 16 * 4; blockNum += 1) {
      ...
      if (blockNum == 0)
        flags =
            MAGIC_INIT + MAGIC_WUPC; // switch on field and send magic sequence
      if (blockNum == 1)
        flags = 0; // just write
      if (blockNum == 16 * 4 - 1)
        flags =
            MAGIC_HALT + MAGIC_OFF; // Done. Magic Halt and switch off field.
  ...
  while (datalen) {
    ...
    if (blockNum >= MIFARE_1K_MAXBLOCK)
      break;
  }
  PrintAndLogEx(NORMAL, "\n");
...
static int CmdHf14AMfDecryptBytes(const char *Cmd) {
  ...
  char ctmp = tolower(param_getchar(Cmd, 0));
  if (strlen(Cmd) < 1 || ctmp == 'h')
    return usage_hf14_decryptbytes();
...
static int CmdHF14AMfice(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mf ice", "Collect MIFARE Classic nonces to file",
                "hf mf ice\n"
  ...
  do {
    ...
    flags |= slow ? 0x0002 : 0;
    clearCommandBuffer();
    SendCommandMIX(CMD_HF_MIFARE_ACQ_NONCES, blockNo + keyType * 0x100,
                   trgBlockNo + trgKeyType * 0x100, flags, NULL, 0);
...
out:
  ...
  clearCommandBuffer();
  SendCommandMIX(CMD_HF_MIFARE_ACQ_NONCES, blockNo + keyType * 0x100,
                 trgBlockNo + trgKeyType * 0x100, 4, NULL, 0);
  return PM3_SUCCESS;
}
...
static int CmdHF14AMfAuth4(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mf auth4",
                ...
                "hf mf auth4 4000 000102030405060708090a0b0c0d0e0f -> executes "
                "authentication\n"
                "hf mf auth4 9003 FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF -> executes "
                "authentication\n");

  void *argtable[] = {
      arg_param_begin, arg_str1(NULL, NULL, "<Key Num (HEX 2 bytes)>", NULL),
      arg_str1(NULL, NULL, "<Key Value (HEX 16 bytes)>", NULL), arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
  ...
  if (keynlen != 2) {
    PrintAndLogEx(ERR, "<Key Num> must be 2 bytes long instead of: %d",
                  keynlen);
    return PM3_ESOFT;
  }

  if (keylen != 16) {
    PrintAndLogEx(ERR, "<Key Value> must be 16 bytes long instead of: %d",
                  keylen);
    return PM3_ESOFT;
  }
...
static int CmdHF14AMfMAD(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "hf mf mad", "Checks and prints MIFARE Application Directory (MAD)",
      "hf mf mad -> shows MAD if exists\n"
      "hf mf mad --aid e103 -k ffffffffffff -b -> shows NDEF data if exists. "
      "read card with custom key and key B\n"
  ...
  void *argtable[] = {
      ...
      arg_lit0("b", "keyb",
               "use key B for access printing sectors (by default: key A)"),
  ...
  bool got_first = true;
  if (mfReadSector(MF_MAD1_SECTOR, MF_KEY_A, (uint8_t *)g_mifare_mad_key,
                   sector0) != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, "error, read sector 0. card don't have MAD or don't "
                           "have MAD on default keys");
    got_first = false;
  } else {
  ...
  if (got_first == false && keylen == 6) {
    ...
    if (mfReadSector(MF_MAD1_SECTOR, MF_KEY_A, userkey, sector0) !=
        PM3_SUCCESS) {
      PrintAndLogEx(ERR, "error, read sector 0. card don't have MAD or don't "
                         "the custom key is wrong");
    } else {
      PrintAndLogEx(INFO, "Authentication ( " _GREEN_("OK") " )");
      got_first = true;
    }
  ...
  if (haveMAD2) {
    if (mfReadSector(MF_MAD2_SECTOR, MF_KEY_A, (uint8_t *)g_mifare_mad_key,
                     sector10)) {
      PrintAndLogEx(ERR, "error, read sector 0x10. card don't have MAD or "
                         "don't have MAD on default keys");
      return PM3_ESOFT;
    }
  ...
  if (aidlen == 2 || decodeholder) {
      ...
      for (int i = 0; i < madlen; i++) {
        if (aaid == mad[i]) {
          ...
          for (int j = 0; j < (verbose ? 4 : 3); j++)
            PrintAndLogEx(NORMAL, " [%03d] %s", (i + 1) * 4 + j,
                          sprint_hex(&vsector[j * 16], 16));
        }
      }
    }
    ...
    if (decodeholder) {
      ...
      PrintAndLogEx(NORMAL, "");
      PrintAndLogEx(INFO,
                    "-------- " _CYAN_("Card Holder Info 0x%04x") " --------",
                    aaid);
...
static int CmdHFMFNDEF(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mf ndef", "Prints NFC Data Exchange Format (NDEF)",
                "hf mf ndef -> shows NDEF parsed data\n"
                "hf mf ndef -vv -> shows NDEF parsed and raw data\n"
                "hf mf ndef --aid e103 -k ffffffffffff -b -> shows NDEF data "
                "with custom AID, key and with key B\n");
  ...
  if (mfReadSector(MF_MAD1_SECTOR, MF_KEY_A, (uint8_t *)g_mifare_mad_key,
    ...
    PrintAndLogEx(ERR, "error, read sector 0. card don't have MAD or don't "
                       "have MAD on default keys");
    PrintAndLogEx(HINT,
                  "Try " _YELLOW_("`hf mf ndef -k `") " with your custom key");
    return PM3_ESOFT;
  }
  ...
  if (haveMAD2) {
    ...
    if (mfReadSector(MF_MAD2_SECTOR, MF_KEY_A, (uint8_t *)g_mifare_mad_key,
      ...
      PrintAndLogEx(ERR, "error, read sector 0x10. card don't have MAD or "
                         "don't have MAD on default keys");
      PrintAndLogEx(
          HINT, "Try " _YELLOW_("`hf mf ndef -k `") " with your custom key");
      return PM3_ESOFT;
    }
  }
...
static int CmdHFMFPersonalize(const char *cmd) {
  ...
  CLIParserInit(
      ...
      "hf mf personalize UIDF1 -> double size UID according to ISO/IEC14443-3, "
      "optional usage of selection process shortcut\n"
      "hf mf personalize UIDF2 -> single size random ID according to "
      "ISO/IEC14443-3\n"
      "hf mf personalize UIDF3 -> single size NUID according to "
      "ISO/IEC14443-3\n"
      "hf mf personalize -t B -k B0B1B2B3B4B5 UIDF3 -> use key B = "
      "0xB0B1B2B3B4B5 instead of default key A");

  void *argtable[] = {
      arg_param_begin,
      arg_str0("t", "keytype", "<A|B>",
               "key type (A or B) to authenticate sector 0 (default: A)"),
      arg_str0("k", "key", "<key (hex 6 Bytes)>",
               "key to authenticate sector 0 (default: FFFFFFFFFFFF)"),
      arg_str1(NULL, NULL, "<UIDF0|UIDF1|UIDF2|UIDF3>",
  ...
  int keytypestr_len;
  int res = CLIParamStrToBuf(arg_get_str(ctx, 1), (uint8_t *)keytypestr, 1,
                             &keytypestr_len);
  ...
  int opt_len;
  uint8_t pers_option;
  res = CLIParamStrToBuf(arg_get_str(ctx, 3), (uint8_t *)pers_optionstr, 5,
                         &opt_len);
  str_lower(pers_optionstr);

  if (res || (!res && opt_len > 0 && opt_len != 5) ||
      (strncmp(pers_optionstr, "uidf0", 5) &&
       strncmp(pers_optionstr, "uidf1", 5) &&
       strncmp(pers_optionstr, "uidf2", 5) &&
       strncmp(pers_optionstr, "uidf3", 5))) {
    PrintAndLogEx(ERR, "ERROR: invalid personalization option. Must be one of "
                       "UIDF0, UIDF1, UIDF2, or UIDF3");
    CLIParserFree(ctx);
    return PM3_EINVARG;
  }
  ...
  SendCommandNG(CMD_HF_MIFARE_PERSONALIZE_UID, (uint8_t *)&payload,
                sizeof(payload));

  PacketResponseNG resp;
  if (!WaitForResponseTimeout(CMD_HF_MIFARE_PERSONALIZE_UID, &resp, 2500))
    return PM3_ETIMEOUT;

  PrintAndLogEx(SUCCESS, "Personalization %s",
                resp.status == PM3_SUCCESS ? "SUCCEEDED" : "FAILED");
...
static int CmdHf14AGen3Block(const char *Cmd) {
  ...
  if (ctmp != '\0' &&
      param_gethex_to_eol(Cmd, 0, block, sizeof(block), &blocklen))
    return usage_hf14_gen3block();
...
static int CmdHf14AMfSuperCard(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mf supercard", "Extract info from a `super card`",
                "hf mf supercard");

  void *argtable[] = {arg_param_begin, arg_lit0("r", "reset", "reset card"),
                      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
  ...
  if (reset_card) {
    ...
    res = ExchangeAPDU14a(aRESET, sizeof(aRESET), activate_field, keep_field_on,
  ...
  res = ExchangeAPDU14a(aFIRST, sizeof(aFIRST), activate_field, keep_field_on,
                        responseA, sizeof(responseA), &respAlen);
  if (res) {
    DropField();
    return res;
  }
  ...
  res = ExchangeAPDU14a(aSECOND, sizeof(aSECOND), activate_field, keep_field_on,
                        responseB, sizeof(responseB), &respBlen);
  if (res) {
    DropField();
    return res;
  }

  // uint8_t inA[] = { 0x72, 0xD7, 0xF4, 0x3E, 0xFD, 0xAB, 0xF2, 0x35, 0xFD,
  // 0x49, 0xEE, 0xDC, 0x44, 0x95, 0x43, 0xC4}; uint8_t inB[] = { 0xF0, 0xA2,
  // 0x67, 0x6A, 0x04, 0x6A, 0x72, 0x12, 0x76, 0xA4, 0x1D, 0x02, 0x1F, 0xEA,
  // 0x20, 0x85};
  ...
  if (res) {
    PrintAndLogEx(SUCCESS,
                  ...
                  sprint_hex_inrow(outA, 4), data.sector,
                  (data.keytype == 0x60) ? 'A' : 'B', key64);
  } else {
...
static command_t CommandTable[] = {
    ...
    {"csave", CmdHF14AMfCSave, IfPm3Iso14443a,
    ...
    {"cwipe", CmdHF14AMfCWipe, IfPm3Iso14443a,
     "Wipe card to default UID/Sectors/Keys"},
    {"-----------", CmdHelp, IfPm3Iso14443a,
     "----------------------- " _CYAN_(
         "magic gen3") " -----------------------"},
    {"gen3uid", CmdHf14AGen3UID, IfPm3Iso14443a,
     "Set UID without manufacturer block"},
    {"gen3blk", CmdHf14AGen3Block, IfPm3Iso14443a,
     "Overwrite full manufacturer block"},
    {"gen3freeze", CmdHf14AGen3Freeze, IfPm3Iso14443a,
     "Perma lock further UID changes"},

    {"-----------", CmdHelp, IfPm3Iso14443a,
     "----------------------- " _CYAN_("i") " -----------------------"},
    {"ice", CmdHF14AMfice, IfPm3Iso14443a,
