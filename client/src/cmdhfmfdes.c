...
uint8_t desdefaultkeys[3][8] = {
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // Official
    {0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47},
    {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07}};

uint8_t aesdefaultkeys[5][16] = {
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00}, // Official, TRF7970A
    {0x79, 0x70, 0x25, 0x53, 0x79, 0x70, 0x25, 0x53, 0x79, 0x70, 0x25, 0x53,
     0x79, 0x70, 0x25, 0x53}, // TRF7970A
    {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB,
     0xCC, 0xDD, 0xEE, 0xFF}, // TRF7970A
    {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
     0x0c, 0x0d, 0x0e, 0x0f},
    {0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b,
     0x4c, 0x4d, 0x4e, 0x4f}};

uint8_t k3kdefaultkeys[1][24] = {
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}};

struct desfire_tag mf_state = {.session_key = NULL,
                               .authentication_scheme = AS_LEGACY,
                               .authenticated_key_no = NOT_YET_AUTHENTICATED,
                               .crypto_buffer = NULL,
                               .crypto_buffer_size = 0,
...
static mfdes_authinput_t currentauth[0xF] = {
    {.keyno = -1}, {.keyno = -1}, {.keyno = -1}, {.keyno = -1}, {.keyno = -1},
    {.keyno = -1}, {.keyno = -1}, {.keyno = -1}, {.keyno = -1}, {.keyno = -1},
    {.keyno = -1}, {.keyno = -1}, {.keyno = -1}, {.keyno = -1}, {.keyno = -1}};
...
/*
  ...
  the LSBit is set to '0' if the size is exactly 2^n
    ...
    For this version of DESFire the 7 MSBits are set to 0x0C (2^12 = 4096) and
  the LSBit is '0'.
*/
static char *getCardSizeStr(uint8_t fsize) {
  ...
  // is  LSB set?
  if (fsize & 1)
    snprintf(retStr, sizeof(buf), "0x%02X ( " _GREEN_("%d - %d bytes") " )",
             fsize, usize, lsize);
  else
    snprintf(retStr, sizeof(buf), "0x%02X ( " _GREEN_("%d bytes") " )", fsize,
             lsize);
  return buf;
}
...
static char *getVersionStr(uint8_t major, uint8_t minor) {
  ...
  if (major == 0x00)
    snprintf(retStr, sizeof(buf), "%x.%x ( " _GREEN_("DESFire MF3ICD40") " )",
             major, minor);
  else if (major == 0x01 && minor == 0x00)
    snprintf(retStr, sizeof(buf), "%x.%x ( " _GREEN_("DESFire EV1") " )", major,
             minor);
  else if (major == 0x12 && minor == 0x00)
    snprintf(retStr, sizeof(buf), "%x.%x ( " _GREEN_("DESFire EV2") " )", major,
             minor);
  else if (major == 0x42 && minor == 0x00)
    snprintf(retStr, sizeof(buf), "%x.%x ( " _GREEN_("DESFire EV2") " )", major,
             minor);
  else if (major == 0x33 && minor == 0x00)
    snprintf(retStr, sizeof(buf), "%x.%x ( " _GREEN_("DESFire EV3") " )", major,
             minor);
  else if (major == 0x30 && minor == 0x00)
    snprintf(retStr, sizeof(buf), "%x.%x ( " _GREEN_("DESFire Light") " )",
             major, minor);
  else if (major == 0x10 && minor == 0x00)
    snprintf(retStr, sizeof(buf), "%x.%x ( " _GREEN_("NTAG413DNA") " )", major,
             minor);
  else
    snprintf(retStr, sizeof(buf), "%x.%x ( " _YELLOW_("Unknown") " )", major,
             minor);
  return buf;
...
static int DESFIRESendApdu(bool activate_field, bool leavefield_on, sAPDU apdu,
                           uint8_t *result, uint32_t max_result_len,
                           uint32_t *result_len, uint16_t *sw) {
  ...
  *result_len = 0;
  ...
  res = ExchangeAPDU14a(data, datalen, activate_field, leavefield_on, result,
                        max_result_len, (int *)result_len);
  if (res) {
    return res;
  }
  ...
  *result_len -= 2;
  ...
  if (isw != 0x9000 && isw != status(MFDES_S_OPERATION_OK) &&
      isw != status(MFDES_S_SIGNATURE) &&
      isw != status(MFDES_S_ADDITIONAL_FRAME) &&
      isw != status(MFDES_S_NO_CHANGES)) {
    if (GetAPDULogging()) {
      if (isw >> 8 == 0x61) {
        PrintAndLogEx(ERR, "APDU chaining len: 0x%02x -->", isw & 0xff);
      } else {
        PrintAndLogEx(ERR, "APDU(%02x%02x) ERROR: [0x%4X] %s", apdu.CLA,
                      apdu.INS, isw,
                      GetAPDUCodeDescription(isw >> 8, isw & 0xff));
...
static const char *getstatus(uint16_t *sw) {
  ...
  if (((*sw >> 8) & 0xFF) == 0x91) {
    ...
    case MFDES_E_PERMISSION_DENIED:
      return "Current configuration/status does not allow the requested "
             "command";
    ...
    case MFDES_E_AUTHENTIFICATION_ERROR:
      return "Current authentication status does not allow the requested "
             "command";

    case MFDES_E_BOUNDARY:
      return "Attempted to read/write data from/to beyond the file's/record's "
             "limit";
    ...
    case MFDES_E_COMMAND_ABORTED:
      return "Previous command was not fully completed / Not all Frames were "
    ...
    case MFDES_E_COUNT:
      return "Application count is limited to 28, not addition "
    ...
    case MFDES_E_EEPROM:
      return "Eeprom error due to loss of power, internal backup/rollback "
             "mechanism activated";
...
static int send_desfire_cmd(sAPDU *apdu, bool select, uint8_t *dest,
                            uint32_t *recv_len, uint16_t *sw,
                            uint32_t splitbysize, bool readalldata) {
  if (apdu == NULL) {
  ...
  *sw = 0;
  ...
  uint32_t i = 1;
  int res =
      DESFIRESendApdu(select, true, *apdu, data, sizeof(data), &resplen, sw);
  if (res != PM3_SUCCESS) {
...
static int handler_desfire_auth(mfdes_authinput_t *payload,
                                mfdes_auth_res_t *rpayload) {
  // 3 different way to authenticate   AUTH (CRC16) , AUTH_ISO (CRC32) ,
  // AUTH_AES (CRC32) 4 different crypto arg1   DES, 3DES, 3K3DES, AES 3
  // different communication modes,  PLAIN,MAC,CRYPTO
  ...
  if (payload->kdfAlgo == MFDES_KDF_ALGO_AN10922) {
    ...
    PrintAndLogEx(DEBUG, " Derrived key: " _GREEN_("%s"),
                  sprint_hex(key->data, key_block_size(key)));
  } else if (payload->kdfAlgo == MFDES_KDF_ALGO_GALLAGHER) {
    // We will overrite any provided KDF input since a gallagher specific KDF
    // was requested.
    payload->kdfInputLen = 11;

    if (mfdes_kdf_input_gallagher(tag->info.uid, tag->info.uidlen,
                                  payload->keyno, tag->selected_application,
                                  payload->kdfInput,
                                  &payload->kdfInputLen) != PM3_SUCCESS) {
      PrintAndLogEx(FAILED, "Could not generate Gallagher KDF input");
    }

    mifare_kdf_an10922(key, payload->kdfInput, payload->kdfInputLen);
    PrintAndLogEx(DEBUG, "    KDF Input: " _YELLOW_("%s"),
                  sprint_hex(payload->kdfInput, payload->kdfInputLen));
    PrintAndLogEx(DEBUG, " Derrived key: " _GREEN_("%s"),
                  sprint_hex(key->data, key_block_size(key)));
  }
  ...
  if (payload->mode != MFDES_AUTH_PICC) {
    ...
    int res =
        send_desfire_cmd(&apdu, false, recv_data, &recv_len, &sw, 0, false);
    if (res != PM3_SUCCESS) {
      return 1;
    }
  ...
  if (payload->mode != MFDES_AUTH_PICC) {
    ...
    if (res != PM3_SUCCESS) {
        PrintAndLogEx(SUCCESS, "Sending auth command %02X "
    _RED_("failed"),subcommand); return PM3_ESOFT;
    }*/
  }
  ...
  if (payload->mode == MFDES_AUTH_DES || payload->mode == MFDES_AUTH_ISO ||
    ...
    mbedtls_aes_crypt_cbc(&ctx, MBEDTLS_AES_DECRYPT, rndlen, IV, encRndA,
                          encRndA);
  }
  ...
  for (uint32_t x = 0; x < rndlen; x++) {
    if (RndA[x] != encRndA[x]) {
      if (g_debugMode > 1) {
        ...
        PrintAndLogEx(DEBUG, "Generated_RndA : %s",
                      sprint_hex(encRndA, rndlen));
      }
      return 11;
    }
  }
...
static int test_desfire_authenticate(void) {
  uint8_t data[] = {0x00};
  sAPDU apdu = {0x90, MFDES_AUTHENTICATE, 0x00, 0x00, 0x01,
                data}; // 0x0A, KEY 0
  uint32_t recv_len = 0;
  uint16_t sw = 0;
...
static int test_desfire_authenticate_iso(void) {
  uint8_t data[] = {0x00};
  sAPDU apdu = {0x90, MFDES_AUTHENTICATE_ISO, 0x00, 0x00, 0x01,
                data}; // 0x1A, KEY 0
  uint32_t recv_len = 0;
  uint16_t sw = 0;
  int res = send_desfire_cmd(&apdu, true, NULL, &recv_len, &sw, 0, false);
...
static int test_desfire_authenticate_aes(void) {
  uint8_t data[] = {0x00};
  sAPDU apdu = {0x90, MFDES_AUTHENTICATE_AES, 0x00, 0x00, 0x01,
                data}; // 0xAA, KEY 0
  uint32_t recv_len = 0;
  uint16_t sw = 0;
  int res = send_desfire_cmd(&apdu, true, NULL, &recv_len, &sw, 0, false);
...
static int desfire_print_freemem(uint32_t free_mem) {
  PrintAndLogEx(
      SUCCESS,
      "   Available free memory on card         : " _GREEN_("%d bytes"),
      free_mem);
  return PM3_SUCCESS;
}
...
static int handler_desfire_freemem(uint32_t *free_mem) {
  ...
  *free_mem = 0;
  ...
  size_t plen = apdu.Lc;
  uint8_t *p = mifare_cryto_preprocess_data(tag, (uint8_t *)apdu.data, &plen, 0,
  ...
  p = mifare_cryto_postprocess_data(tag, apdu.data, &dlen,
...
static int mifare_desfire_change_key(uint8_t key_no, uint8_t *new_key,
                                     uint8_t new_algo, uint8_t *old_key,
                                     uint8_t old_algo, uint8_t aes_version) {
  ...
  */
  ...
  uint8_t *p =
      mifare_cryto_preprocess_data(tag, data + 1, (size_t *)&cmdcnt, 0,
  ...
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't change key -> %s"),
  ...
  size_t sn = recv_len;
  p = mifare_cryto_postprocess_data(tag, data, &sn,
                                    MDCM_PLAIN | CMAC_COMMAND | CMAC_VERIFY);
...
// --- GET SIGNATURE
static int desfire_print_signature(uint8_t *uid, uint8_t uidlen,
...
#define PUBLIC_DESFIRE_ECDA_KEYLEN 57
  const ecdsa_publickey_t nxp_desfire_public_keys[] = {
      {"NTAG424DNA, DESFire EV2",
       "048A9B380AF2EE1B98DC417FECC263F8449C7625CECE82D9B916C992DA209D68422B81E"
       "C20B65A66B5102A61596AF3379200599316A00A1410"},
      {"NTAG413DNA, DESFire EV1",
       "04BB5D514F7050025C7D0F397310360EEC91EAF792E96FC7E0F496CB4E669D414F877B7"
       "B27901FE67C2E3B33CD39D1C797715189AC951C2ADD"},
      {"DESFire EV2",
       "04B304DC4C615F5326FE9383DDEC9AA892DF3A57FA7FFB3276192BC0EAA252ED45A865E"
       "3B093A3D0DCE5BE29E92F1392CE7DE321E3E5C52B3A"},
      {"DESFire EV3",
       "041DB46C145D0A36539C6544BD6D9B0AA62FF91EC48CBC6ABAE36E0089A46F0D08C8A71"
       "5EA40A63313B92E90DDC1730230E0458A33276FB743"},
      {"NTAG424DNA, NTAG424DNATT, DESFire Light EV2",
       "04B304DC4C615F5326FE9383DDEC9AA892DF3A57FA7FFB3276192BC0EAA252ED45A865E"
       "3B093A3D0DCE5BE29E92F1392CE7DE321E3E5C52B3B"},
      {"DESFire Light",
       "040E98E117AAA36457F43173DC920A8757267F44CE4EC5ADD3C54075571AEBBF7B942A9"
       "774A1D94AD02572427E5AE0A2DD36591B1FB34FCF3D"},
      {"MIFARE Plus EV1",
       "044409ADC42F91A8394066BA83D872FB1D16803734E911170412DDF8BAD1A4DADFD0416"
       "291AFE1C748253925DA39A5F39A1C557FFACD34C62E"}};
  ...
  for (i = 0; i < ARRAYLEN(nxp_desfire_public_keys); i++) {
    ...
    param_gethex_to_eol(nxp_desfire_public_keys[i].value, 0, key,
                        PUBLIC_DESFIRE_ECDA_KEYLEN, &dl);

    int res =
        ecdsa_signature_r_s_verify(MBEDTLS_ECP_DP_SECP224R1, key, uid, uidlen,
                                   signature, signature_len, false);
    is_valid = (res == 0);
    if (is_valid)
      break;
  }
  ...
  if (is_valid == false || i == ARRAYLEN(nxp_desfire_public_keys)) {
    ...
    PrintAndLogEx(INFO, "             TAG IC Signature: %s",
                  sprint_hex_inrow(signature, 16));
    PrintAndLogEx(INFO, "                             : %s",
                  sprint_hex_inrow(signature + 16, 16));
    PrintAndLogEx(INFO, "                             : %s",
                  sprint_hex_inrow(signature + 32, 16));
    PrintAndLogEx(INFO, "                             : %s",
                  sprint_hex_inrow(signature + 48, signature_len - 48));
    PrintAndLogEx(SUCCESS, "       Signature verification: " _RED_("failed"));
    return PM3_ESOFT;
  }

  PrintAndLogEx(INFO, " IC signature public key name: " _GREEN_("%s"),
                nxp_desfire_public_keys[i].desc);
  PrintAndLogEx(INFO, "IC signature public key value: %.32s",
                nxp_desfire_public_keys[i].value);
  PrintAndLogEx(INFO, "                             : %.32s",
                nxp_desfire_public_keys[i].value + 32);
  PrintAndLogEx(INFO, "                             : %.32s",
                nxp_desfire_public_keys[i].value + 64);
  PrintAndLogEx(INFO, "                             : %.32s",
                nxp_desfire_public_keys[i].value + 96);
  PrintAndLogEx(INFO, "    Elliptic curve parameters: NID_secp224r1");
  PrintAndLogEx(INFO, "             TAG IC Signature: %s",
                sprint_hex_inrow(signature, 16));
  PrintAndLogEx(INFO, "                             : %s",
...
static int handler_desfire_signature(uint8_t *signature,
...
static int desfire_print_keyversion(uint8_t key_idx, uint8_t key_version) {
  PrintAndLogEx(SUCCESS, "   Key [%u]  Version : %d (0x%02x)", key_idx,
...
static int handler_desfire_keyversion(uint8_t curr_key, uint8_t *num_versions) {
  ...
  sAPDU apdu = {0x90,     MFDES_GET_KEY_VERSION, 0x00, 0x00, 0x01,
                &curr_key}; // 0x64
  uint32_t recv_len = 0;
  uint16_t sw = 0;
  int res =
      send_desfire_cmd(&apdu, false, num_versions, &recv_len, &sw, 0, true);
...
static int desfire_print_amk_keysetting(uint8_t key_settings, uint8_t num_keys,
                                        int algo) {
  ...
  case 0x0:
    ...
    break;
  case 0xE:
    PrintAndLogEx(SUCCESS, "  -- Authentication with the key to be changed "
                           "(same KeyNo) is necessary to change a key");
    break;
  case 0xF:
    PrintAndLogEx(SUCCESS, "  -- All keys (except AMK,see Bit0) within this "
                           "application are frozen");
    break;
  default:
    PrintAndLogEx(SUCCESS,
                  "  -- Authentication with the specified key is necessary to "
                  "change any key.\n"
                  "A change key and a PICC master key (CMK) can only be "
                  "changed after authentication with the master key.\n"
                  "For keys other then the master or change key, an "
                  "authentication with the same key is needed.");
    break;
  }

  PrintAndLogEx(SUCCESS, "   [1000] AMK Configuration changeable   : %s",
                (key_settings & (1 << 3)) ? _GREEN_("YES") : "NO (frozen)");
  PrintAndLogEx(SUCCESS, "   [0100] AMK required for create/delete : %s",
                (key_settings & (1 << 2)) ? "NO" : "YES");
  PrintAndLogEx(SUCCESS, "   [0010] Directory list access with AMK : %s",
                (key_settings & (1 << 1)) ? "NO" : "YES");
  PrintAndLogEx(SUCCESS, "   [0001] AMK is changeable              : %s",
                (key_settings & (1 << 0)) ? _GREEN_("YES") : "NO (frozen)");
  return PM3_SUCCESS;
}

// --- KEY SETTING  PICC Master Key (CMK)
static int desfire_print_piccmk_keysetting(uint8_t key_settings,
                                           uint8_t num_keys, int algo) {
  // PrintAndLogEx(INFO, "--- " _CYAN_("PICC Master Key (CMK) settings"));
  // number of Master keys (0x01)
  PrintAndLogEx(SUCCESS,
                "   Number of Masterkeys                  : " _YELLOW_("%u"),
                (num_keys & 0x3F));
  const char *str =
  ...
  if (handler_desfire_keyversion(0, &cmk_num_versions) == PM3_SUCCESS) {
    PrintAndLogEx(
        SUCCESS,
        "   PICC Master key Version               : " _YELLOW_("%d (0x%02x)"),
        cmk_num_versions, cmk_num_versions);
  }
  ...
  if (res == PM3_SUCCESS)
    PrintAndLogEx(SUCCESS, "   [0x0A] Authenticate      : %s",
                  (res == PM3_SUCCESS) ? _YELLOW_("YES") : "NO");

  res = test_desfire_authenticate_iso();
  if (res == PM3_SUCCESS)
    PrintAndLogEx(SUCCESS, "   [0x1A] Authenticate ISO  : %s",
                  (res == PM3_SUCCESS) ? _YELLOW_("YES") : "NO");

  res = test_desfire_authenticate_aes();
  if (res == PM3_SUCCESS)
    PrintAndLogEx(SUCCESS, "   [0xAA] Authenticate AES  : %s",
                  (res == PM3_SUCCESS) ? _YELLOW_("YES") : "NO");

  PrintAndLogEx(
      INFO, "-------------------------------------------------------------");
  PrintAndLogEx(INFO, " Key setting: 0x%02X [%c%c%c%c]", key_settings,
                (key_settings & (1 << 3)) ? '1' : '0',
                (key_settings & (1 << 2)) ? '1' : '0',
                (key_settings & (1 << 1)) ? '1' : '0',
                (key_settings & (1 << 0)) ? '1' : '0');

  PrintAndLogEx(SUCCESS, "   [1...] CMK Configuration changeable   : %s",
                (key_settings & (1 << 3)) ? _GREEN_("YES") : "NO (frozen)");
  PrintAndLogEx(SUCCESS, "   [.1..] CMK required for create/delete : %s",
                (key_settings & (1 << 2)) ? _GREEN_("NO") : "YES");
  PrintAndLogEx(SUCCESS, "   [..1.] Directory list access with CMK : %s",
                (key_settings & (1 << 1)) ? _GREEN_("NO") : "YES");
  PrintAndLogEx(SUCCESS, "   [...1] CMK is changeable              : %s",
                (key_settings & (1 << 0)) ? _GREEN_("YES") : "NO (frozen)");

  return PM3_SUCCESS;
}

static int handler_desfire_getkeysettings(uint8_t *key_settings,
                                          uint8_t *num_keys) {
...
static int handler_desfire_dfnames(dfname_t *dest, uint8_t *dfname_count) {
  ...
  if (g_debugMode > 1) {
    if (dest == NULL)
      PrintAndLogEx(ERR, "DEST = NULL");
    if (dfname_count == NULL)
      PrintAndLogEx(ERR, "DFNAME_COUNT = NULL");
  }
  ...
  *dfname_count = 0;
  ...
  uint32_t recv_len = 0;
  uint16_t sw = 0;
  int res = send_desfire_cmd(&apdu, true, (uint8_t *)dest, &recv_len, &sw,
                             sizeof(dfname_t), true);
  if (res != PM3_SUCCESS) {
    return res;
  }
...
static int handler_desfire_select_application(uint8_t *aid) {
  ...
  int res = send_desfire_cmd(&apdu, !tag->rf_field_on, NULL, &recv_len, &sw,
                             sizeof(dfname_t), true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't select AID 0x%X -> %s"),
...
static int key_setting_to_algo(uint8_t aid[3], uint8_t *key_setting,
                               mifare_des_authalgo_t *algo, uint8_t *num_keys) {
...
static int handler_desfire_fileids(uint8_t *dest, uint32_t *file_ids_len) {
  if (g_debugMode > 1) {
    if (dest == NULL)
      PrintAndLogEx(ERR, "DEST=NULL");
    if (file_ids_len == NULL)
      PrintAndLogEx(ERR, "FILE_IDS_LEN=NULL");
  }
  if (dest == NULL || file_ids_len == NULL)
    return PM3_EINVARG;
  ...
  *file_ids_len = 0;
  ...
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't get file ids -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
...
// none, verified
static int handler_desfire_filesettings(uint8_t file_id, uint8_t *dest,
                                        uint32_t *destlen) {
  if (g_debugMode > 1) {
    if (dest == NULL)
      PrintAndLogEx(ERR, "DEST=NULL");
    if (destlen == NULL)
      PrintAndLogEx(ERR, "DESTLEN=NULL");
  }
  if (dest == NULL || destlen == NULL)
    return PM3_EINVARG;
  sAPDU apdu = {0x90,    MFDES_GET_FILE_SETTINGS, 0x00, 0x00, 0x01,
                &file_id}; // 0xF5
  uint16_t sw = 0;
  int res = send_desfire_cmd(&apdu, false, dest, destlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't get file settings -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
...
static int handler_desfire_createapp(aidhdr_t *aidhdr, bool usename,
  ...
  sAPDU apdu = {0x90, MFDES_CREATE_APPLICATION, 0x00,
                0x00, sizeof(aidhdr_t),         (uint8_t *)aidhdr}; // 0xCA
  ...
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't create aid -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
  }
  return res;
...
static int handler_desfire_deleteapp(const uint8_t *aid) {
  ...
  sAPDU apdu = {0x90,          MFDES_DELETE_APPLICATION, 0x00, 0x00, 3,
                (uint8_t *)aid}; // 0xDA
  uint16_t sw = 0;
  uint32_t recvlen = 0;
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't delete aid -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
  }
  return res;
...
static int handler_desfire_credit(mfdes_value_t *value, uint8_t cs) {
  sAPDU apdu = {0x90, MFDES_CREDIT, 0x00,
                0x00, 1 + 4,        (uint8_t *)value}; // 0x0C
  uint16_t sw = 0;
  uint32_t recvlen = 0;

  size_t plen = apdu.Lc;
  uint8_t *p = mifare_cryto_preprocess_data(tag, (uint8_t *)apdu.data, &plen, 0,
                                            cs | MAC_COMMAND | CMAC_COMMAND |
                                                ENC_COMMAND);
  apdu.Lc = (uint8_t)plen;
  apdu.data = p;
  ...
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't credit value -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
...
static int handler_desfire_limitedcredit(mfdes_value_t *value, uint8_t cs) {
  sAPDU apdu = {0x90,  MFDES_LIMITED_CREDIT, 0x00, 0x00,
                1 + 4, (uint8_t *)value}; // 0x1C
  uint16_t sw = 0;
  uint32_t recvlen = 0;
  ...
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't credit limited value -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
...
static int handler_desfire_debit(mfdes_value_t *value, uint8_t cs) {
  ...
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't debit value -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
...
static int handler_desfire_readdata(mfdes_data_t *data, MFDES_FILE_TYPE_T type,
  ...
  if (data->fileno > 0x1F)
    return PM3_EINVARG;
  sAPDU apdu = {0x90, MFDES_READ_DATA, 0x00,
                0x00, 1 + 3 + 3,       (uint8_t *)data}; // 0xBD
  ...
  size_t plen = apdu.Lc;
  uint8_t *p = mifare_cryto_preprocess_data(tag, (uint8_t *)data, &plen, 0,
                                            MDCM_PLAIN | CMAC_COMMAND);
  apdu.Lc = (uint8_t)plen;
  apdu.data = p;
  ...
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't read data -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
  ...
  size_t dlen = resplen;
  p = mifare_cryto_postprocess_data(
      tag, data->data, &dlen, cs | CMAC_COMMAND | CMAC_VERIFY | MAC_VERIFY);
  if (dlen != -1)
    resplen = dlen;
...
static int handler_desfire_getvalue(mfdes_value_t *value, uint32_t *resplen,
                                    uint8_t cs) {

  if (value->fileno > 0x1F)
    return PM3_EINVARG;

  sAPDU apdu = {0x90, MFDES_GET_VALUE, 0x00, 0x00,
                0x01, &value->fileno}; // 0xBD
  uint16_t sw = 0;
  *resplen = 0;
  ...
  p = mifare_cryto_postprocess_data(
      tag, value->value, &dlen, cs | CMAC_COMMAND | CMAC_VERIFY | MAC_VERIFY);
  return res;
}
...
static int handler_desfire_writedata(mfdes_data_t *data, MFDES_FILE_TYPE_T type,
  ...
  /*                  LC  FN  OF  OF  OF  LN  LN  LN  DD  DD  DD
      90  3d  00  00  16  01  00  00  00  0f  00  00  00  0f  20  00  3b  00  34
     04  06  e1  04  0f  fe  00  00  00 90  3d  00  00  09  02  00  00  00  02
     00  00  00  00  00
  */
  ...
  uint8_t tmp[59] = {0};
  mfdes_data_t sdata;
  sAPDU apdu = {0x90, MFDES_WRITE_DATA, 0x00, 0x00,
                0,    (uint8_t *)&sdata}; // 0x3D
  tmp[0] = data->fileno;
  apdu.data = tmp;
  if (type == MFDES_RECORD_FILE)
  ...
  while (datatowrite) {
    ...
    uint8_t *p = mifare_cryto_preprocess_data(
        tag, (uint8_t *)&data->data[pos], &plen, 0,
        cs | MAC_COMMAND | CMAC_COMMAND | ENC_COMMAND);
    if (plen != -1)
      datasize = (uint8_t)plen;
    ...
    if (res != PM3_SUCCESS) {
      PrintAndLogEx(WARNING, _RED_("   Can't write data -> %s"),
                    GetErrorString(res, &sw));
      DropFieldDesfire();
      return res;
    }
  ...
  if (type == MFDES_RECORD_FILE) {
    if (handler_desfire_commit_transaction() != PM3_SUCCESS) {
      PrintAndLogEx(WARNING, _RED_("   Can't commit transaction -> %s"),
                    GetErrorString(res, &sw));
      DropFieldDesfire();
      return res;
    }
...
static int handler_desfire_deletefile(uint8_t fileno) {
  ...
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't delete file -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
...
static int handler_desfire_clearrecordfile(uint8_t fileno) {
  ...
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't clear record file -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  } else {
...
static int handler_desfire_create_value_file(mfdes_value_file_t *value) {
  ...
  sAPDU apdu = {0x90, MFDES_CREATE_VALUE_FILE,    0x00,
                0x00, sizeof(mfdes_value_file_t), (uint8_t *)value}; // 0xCc

  uint16_t sw = 0;
  uint32_t recvlen = 0;
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't create value -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
...
static int handler_desfire_create_std_file(mfdes_file_t *file) {
  if (file->fileno > 0x1F)
    return PM3_EINVARG;
  sAPDU apdu = {0x90, MFDES_CREATE_STD_DATA_FILE, 0x00,
                0x00, sizeof(mfdes_file_t),       (uint8_t *)file}; // 0xCD

  uint16_t sw = 0;
  uint32_t recvlen = 0;
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't create file -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
...
static int handler_desfire_create_linearrecordfile(mfdes_linear_t *file) {
  ...
  if (memcmp(file->recordsize, "\x00\x00\x00", 3) == 0)
    return PM3_EINVARG;
  sAPDU apdu = {0x90, MFDES_CREATE_LINEAR_RECORD_FILE, 0x00,
                0x00, sizeof(mfdes_linear_t),          (uint8_t *)file}; // 0xC1

  uint16_t sw = 0;
  uint32_t recvlen = 0;
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't create linear record file -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
...
static int handler_desfire_create_cyclicrecordfile(mfdes_linear_t *file) {
  ...
  if (file->fileno > 0x1F)
    return PM3_EINVARG;
  sAPDU apdu = {0x90, MFDES_CREATE_CYCLIC_RECORD_FILE, 0x00,
                0x00, sizeof(mfdes_linear_t),          (uint8_t *)file}; // 0xC0

  uint16_t sw = 0;
  uint32_t recvlen = 0;
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't create cyclic record file -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
...
static int handler_desfire_create_backup_file(mfdes_file_t *file) {
  ...
  sAPDU apdu = {0x90, MFDES_CREATE_BACKUP_DATA_FILE, 0x00,
                0x00, sizeof(mfdes_file_t),          (uint8_t *)file}; // 0xCB

  uint16_t sw = 0;
  uint32_t recvlen = 0;
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't create backup file -> %s"),
                  GetErrorString(res, &sw));
    DropFieldDesfire();
    return res;
  }
...
static int getKeySettings(uint8_t *aid) {
  ...
  if (memcmp(aid, "\x00\x00\x00", 3) == 0) {
    ...
    // CARD MASTER KEY
    // PrintAndLogEx(INFO, "--- " _CYAN_("CMK - PICC, Card Master Key
    // settings"));
    ...
    // PrintAndLogEx(SUCCESS, "--- " _CYAN_("AMK - Application Master Key
    // settings"));
    ...
    if (num_keys > 1) {
      for (uint8_t i = 0x01; i < num_keys; ++i) {
        if (handler_desfire_keyversion(i, &num_version) == PM3_SUCCESS) {
          ...
          PrintAndLogEx(WARNING, "   Can't read key %d  (0x%02x) version", i,
                        i);
        }
      }
    }
...
static int desfire_authenticate(int cmdAuthMode, int cmdAuthAlgo, uint8_t *aid,
                                uint8_t *key, int cmdKeyNo, uint8_t cmdKdfAlgo,
                                uint8_t kdfInputLen, uint8_t *kdfInput,
                                mfdes_auth_res_t *rpayload) {
  switch (cmdAuthMode) {
  case MFDES_AUTH_DES:
  ...
  case MFDES_AUTH_PICC:
    ...
    break;
  default:
    PrintAndLogEx(WARNING, "Wrong Auth mode (%d) -> (1=normal, 2=iso, 3=aes)",
                  cmdAuthMode);
    return PM3_EINVARG;
  }
...
static int CmdHF14ADesGetUID(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mfdes getuid", "Get UID from a MIFARE DESfire tag",
                "hf mfdes getuid");
  ...
  PrintAndLogEx(SUCCESS, "    UID: " _GREEN_("%s"),
                sprint_hex(uid, sizeof(uid)));
  return res;
}

static int CmdHF14ADesSelectApp(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mfdes selectaid", "Select Application ID",
                "hf mfdes selectaid -a 123456");

  void *argtable[] = {
      arg_param_begin,
      arg_strx0("a", "aid", "<aid>",
                "App ID to select as hex bytes (3 bytes,big endian)"),
      arg_param_end};
...
static int CmdHF14ADesCreateApp(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mfdes createaid", "Create Application ID",
                "hf mfdes createaid -a 123456 -f 1111 -k 0E -l 2E --name Test");

  void *argtable[] = {
      arg_param_begin,
      arg_strx0("a", "aid", "<aid>",
                "App ID to create as hex bytes (3 hex bytes)"),
      arg_strx0("f", "fid", "<fid>", "File ID to create (optional)"),
      arg_strx0("k", "ks1", "<keysetting1>",
                "Key Setting 1 (Application Master Key Settings)"),
  ...
  /* KeySetting 1 (AMK Setting):
     ...
     1:   Free Directory list access without master key
          ...
          1: No AMK auth needed for GetFileIDs, GetISOFileIDs, GetFileSettings,
     GetKeySettings 2:   Free create/delete without master key 0:
     CreateFile/DeleteFile only with AMK auth 1:  CreateFile/DeleteFile always
     ...
     4-7: ChangeKey Access Rights
          ...
          0xE: Auth with the key to be changed (same KeyNo) is necessary to
     change a key 0xF: All Keys within this application are frozen
  ...
  */
  ...
  PrintAndLogEx(INFO, "AID      %s",
                sprint_hex_inrow(aidhdr.aid, sizeof(aidhdr.aid)));
  PrintAndLogEx(INFO, "Key set1 0x%02X", aidhdr.keysetting1);
  PrintAndLogEx(INFO, "Key Set2 0x%02X", aidhdr.keysetting2);
  if (usefid)
    PrintAndLogEx(INFO, "FID      %s",
                  sprint_hex_inrow(aidhdr.fid, sizeof(aidhdr.fid)));
  if (usename)
...
static int CmdHF14ADesDeleteApp(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mfdes deleteaid", "Delete Application ID",
                "hf mfdes deleteaid -a 123456");

  void *argtable[] = {arg_param_begin,
                      arg_strx0("a", "aid", "<aid>",
                                "App ID (3 hex bytes, big endian) to delete"),
                      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, false);
  int aidlength = 3;
  uint8_t aid[3] = {0};
...
static int CmdHF14ADesClearRecordFile(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mfdes clearfile",
                ...
                "aid before running this command.",
                "hf mfdes clearfile -n 01");

  void *argtable[] = {
      arg_param_begin,
      arg_strx0("n", "fileno", "<fileno>",
                "File Number (1 hex byte, 0x00 - 0x1F)"),
      arg_strx0(
          "a", "aid", "<aid>",
          "App ID to select as hex bytes (3 bytes, big endian, optional)"),
      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, false);
...
static int CmdHF14ADesDeleteFile(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mfdes deletefile", "Delete File",
                "hf mfdes deletefile -n 01 -> Make sure to select aid or "
                "authenticate aid before running this command.");
...
static int CmdHF14ADesCreateFile(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "hf mfdes createfile", "Create Standard/Backup File",
  ...
  void *argtable[] = {
      ...
      arg_int0("c", "com.set", "<comset>",
               "Communication setting (0=Plain,1=Plain+MAC,3=Enciphered)"),
      //        arg_strx0("r", "accessrights", "<accessrights>", "Access rights
      //        (2 hex bytes -> RW/Chg/R/W, 0-D Key, E Free, F Denied)"),
      arg_strx0("r", "rights", "<accessrights>",
                "Access rights (2 hex bytes -> RW/Chg/R/W, 0-D Key, E Free, F "
                "Denied)"),
      arg_strx0("s", "filesize", "<filesize>",
                "File size (3 hex bytes, big endian)"),
  ...
  if (comset != 0 && comset != 1 && comset != 3) {
    PrintAndLogEx(ERR, "Communication setting must be either 0=Plain, "
                       "1=Plain+MAC or 3=Encrypt.");
    return PM3_EINVARG;
  }
...
static int CmdHF14ADesGetValueData(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mfdes getvalue",
                ...
                "Make sure to select aid or authenticate aid before running "
                "this command.",
  ...
  if (res == PM3_SUCCESS) {
    ...
    for (uint32_t i = 0; i < len; i += 16) {
      PrintAndLogEx(NORMAL, "%02d/0x%02X | %s| %s", i, i,
                    sprint_hex(&value.value[i], len > 16 ? 16 : len),
                    sprint_ascii(&value.value[i], len > 16 ? 16 : len));
    }
  } else {
...
static int CmdHF14ADesReadData(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mfdes readdata",
                ...
                "hf mfdes readdata -n 01 -t 0 -> Read all data from standard "
                "file, fileno 01");
  ...
  void *argtable[] = {
      ...
      arg_strx0("n", "fileno", "<fileno>",
                "File Number (1 hex byte, 0x00 - 0x1F)"),
      arg_strx0("o", "offset", "<offset>",
                "File Offset (3 hex bytes, big endian), optional"),
      arg_strx0("l", "length", "<length>",
                "Length to read (3 hex bytes, big endian -> 000000 = Read all "
                "data),optional"),
      arg_int0("t", "type", "<type>",
               "File Type (0=Standard/Backup, 1=Record)"),
      arg_strx0("a", "aid", "<aid>",
                "App ID to select as hex bytes (3 bytes,big endian,optional)"),
      arg_param_end};
  ...
  if (data != NULL) {
    ...
    if (res == PM3_SUCCESS) {
      ...
      for (uint32_t i = 0; i < len; i += 16) {
        uint32_t l = len - i;
        PrintAndLogEx(NORMAL, "%02d/0x%02X | %s| %s", i, i,
                      sprint_hex(&ft.data[i], l > 16 ? 16 : l),
                      sprint_ascii(&ft.data[i], l > 16 ? 16 : l));
      }
    } else {
...
static int CmdHF14ADesChangeValue(const char *Cmd) {
  ...
  void *argtable[] = {
      ...
      arg_strx0("n", "fileno", "<fileno>",
                "File Number (1 hex byte, 0x00 - 0x1F)"),
      arg_strx0("d", "value", "<value>",
                "Value to increase (4 hex bytes, big endian)"),
      arg_int0("m", "mode", "<mode>",
               "Mode (0=Credit, 1=LimitedCredit, 2=Debit)"),
      arg_strx0("a", "aid", "<aid>",
                "App ID to select as hex bytes (3 bytes,big endian)"),
      arg_param_end};
...
static int CmdHF14ADesWriteData(const char *Cmd) {
  ...
  void *argtable[] = {
      ...
      arg_strx0("o", "offset", "<offset>",
                "File Offset (3 hex bytes, big endian), optional"),
      arg_strx0("d", "data", "<data>",
                "Data to write (hex bytes, 256 bytes max)"),
      arg_int0("t", "type", "<type>",
               "File Type (0=Standard/Backup, 1=Record)"),
      arg_strx0(
          "a", "aid", "<aid>",
  ...
  // iceman:  we only have a 1024 byte commandline input array. So this is
  // pointlessly large. with 2char hex, 512bytes could be input. Instead large
  // binary inputs should be BINARY files and written to card.
  int dlength = 512;
  uint8_t data[512] = {0};
...
static int CmdHF14ADesCreateRecordFile(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mfdes createrecordfile",
                ...
                "Make sure to select aid or authenticate aid before running "
                "this command.",
                "hf mfdes createrecordfile -f 1122 -n 02 -c 0 -r EEEE -s "
                "000010 -m 000005 -a 123456");
  ...
  void *argtable[] = {
      ...
      //        arg_strx0("r", "accessrights", "<accessrights>", "Access rights
      //        (2 hex bytes -> RW/Chg/R/W, 0-D Key, E Free, F Denied)"),
      //        arg_strx0("s", "recordsize", "<recordsize>", "Record size (3 hex
      //        bytes, big endian, 000001 to FFFFFF)"),
      arg_strx0("r", "rights", "<accessrights>",
                "Access rights (2 hex bytes -> RW/Chg/R/W, 0-D Key, E Free, F "
                "Denied)"),
      arg_strx0("s", "size", "<recordsize>",
                "Record size (3 hex bytes, big endian, 000001 to FFFFFF)"),
      arg_strx0("m", "maxrecord", "<maxrecord>",
                "Max. Number of Records (3 hex bytes, big endian)"),
      arg_lit0("b", "cyclic",
               "Create cyclic record file instead of linear record file"),
      arg_strx0("a", "aid", "<aid>",
                "App ID to select as hex bytes (3 bytes,big endian,optional)"),
      arg_param_end};
  ...
  if (res == PM3_SUCCESS) {
    ...
    PrintAndLogEx(ERR, "Couldn't create linear/cyclic record file. Error %d",
                  res);
  }
  DropFieldDesfire();
  return res;
...
static int CmdHF14ADesCreateValueFile(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mfdes createvaluefile",
                ...
                "Make sure to select aid or authenticate aid before running "
                "this command.",
                "hf mfdes createvaluefile -n 03 -c 0 -r EEEE -l 00000000 -u "
                "00002000 -v 00000001 -m 02 -a 123456\n");
  ...
  void *argtable[] = {
      ...
      arg_strx0("r", "rights", "<accessrights>",
                "Access rights (2 hex bytes -> RW/Chg/R/W, 0-D Key, E Free, F "
                "Denied)"),
      arg_strx0("l", "lowerlimit", "<lowerlimit>",
                "Lower limit (4 hex bytes, big endian)"),
      arg_strx0("u", "upperlimit", "<upperlimit>",
                "Upper limit (4 hex bytes, big endian)"),
      arg_strx0("v", "value", "<value>", "Value (4 hex bytes, big endian)"),
      arg_strx0("m", "limitcredit", "<limitcredit>",
                "Limited Credit enabled (1 hex byte [Bit 0=LimitedCredit, "
                "1=FreeValue])"),
      arg_strx0("a", "aid", "<aid>",
                "App ID to select as hex bytes (3 bytes,big endian,optional)"),
      arg_param_end};
...
static int CmdHF14ADesFormatPICC(const char *Cmd) {
  ...
  int res = send_desfire_cmd(&apdu, false, NULL, &recvlen, &sw, 0, true);
  if (res != PM3_SUCCESS) {
    PrintAndLogEx(WARNING, _RED_("   Can't format picc -> %s"),
                  GetErrorString(res, &sw));
  } else {
...
static int CmdHF14ADesInfo(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mfdes info", "Get info from MIFARE DESfire tags",
                "hf mfdes info");
  ...
  if (cardtype == PLUS_EV1) {
    PrintAndLogEx(INFO, "Card seems to be MIFARE Plus EV1.  Try " _YELLOW_(
                            "`hf mfp info`"));
    return PM3_SUCCESS;
  }
  ...
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(
      INFO, "--- " _CYAN_("Tag Information") " ---------------------------");
  PrintAndLogEx(
      INFO, "-------------------------------------------------------------");
  PrintAndLogEx(SUCCESS, "              UID: " _GREEN_("%s"),
                sprint_hex(info.uid, info.uidlen));
  PrintAndLogEx(SUCCESS, "     Batch number: " _GREEN_("%s"),
                sprint_hex(info.details + 7, 5));
  PrintAndLogEx(
      SUCCESS,
      "  Production date: week " _GREEN_("%02x") " / " _GREEN_("20%02x"),
      info.details[12], info.details[13]);
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(INFO, "--- " _CYAN_("Hardware Information"));
  PrintAndLogEx(INFO, "   raw: %s",
                sprint_hex_inrow(info.versionHW, sizeof(info.versionHW)));

  PrintAndLogEx(INFO, "     Vendor Id: " _YELLOW_("%s"),
                getTagInfo(info.versionHW[0]));
  PrintAndLogEx(INFO, "          Type: " _YELLOW_("0x%02X"), info.versionHW[1]);
  PrintAndLogEx(INFO, "       Subtype: " _YELLOW_("0x%02X"), info.versionHW[2]);
  PrintAndLogEx(INFO, "       Version: %s",
                getVersionStr(info.versionHW[3], info.versionHW[4]));
  PrintAndLogEx(INFO, "  Storage size: %s", getCardSizeStr(info.versionHW[5]));
  PrintAndLogEx(INFO, "      Protocol: %s",
                getProtocolStr(info.versionHW[6], true));
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(INFO, "--- " _CYAN_("Software Information"));
  PrintAndLogEx(INFO, "   raw: %s",
                sprint_hex_inrow(info.versionSW, sizeof(info.versionSW)));
  PrintAndLogEx(INFO, "     Vendor Id: " _YELLOW_("%s"),
                getTagInfo(info.versionSW[0]));
  PrintAndLogEx(INFO, "          Type: " _YELLOW_("0x%02X"), info.versionSW[1]);
  PrintAndLogEx(INFO, "       Subtype: " _YELLOW_("0x%02X"), info.versionSW[2]);
  PrintAndLogEx(INFO, "       Version: " _YELLOW_("%d.%d"), info.versionSW[3],
                info.versionSW[4]);
  PrintAndLogEx(INFO, "  Storage size: %s", getCardSizeStr(info.versionSW[5]));
  PrintAndLogEx(INFO, "      Protocol: %s",
                getProtocolStr(info.versionSW[6], false));
  ...
  if (major == 0 && minor == 5)
    PrintAndLogEx(INFO, "\t0.5 - DESFire MF3ICD40, Support for wrapping "
                        "commands inside ISO 7816 style APDUs");
  if (major == 0 && minor == 6)
    PrintAndLogEx(
        INFO,
        "\t0.6 - DESFire MF3ICD40, Add ISO/IEC 7816 command set compatibility");
  if (major == 1 && minor == 3)
    PrintAndLogEx(INFO, "\t1.3 - DESFire Ev1 MF3ICD21/41/81, Support extended "
                        "APDU commands, EAL4+");
  if (major == 1 && minor == 4)
    PrintAndLogEx(INFO, "\t1.4 - DESFire Ev1 MF3ICD21/41/81, EAL4+");
  if (major == 2 && minor == 0)
    PrintAndLogEx(
        INFO, "\t2.0 - DESFire Ev2, Originality check, proximity check, EAL5");
  if (major == 3 && minor == 0)
    PrintAndLogEx(INFO, "\t3.0 - DESFire Ev3, Originality check, proximity "
                        "check, badass EAL6 ?");

  if (major == 0 && minor == 2)
    PrintAndLogEx(INFO, "\t0.2 - DESFire Light, Originality check, ");

  if (cardtype == DESFIRE_EV2 || cardtype == DESFIRE_LIGHT ||
      cardtype == DESFIRE_EV3 || cardtype == NTAG413DNA) {
...
static void DecodeFileType(uint8_t filetype) {
  switch (filetype) {
  case 0x00:
    PrintAndLogEx(INFO, "     File Type: 0x%02X -> Standard Data File",
                  filetype);
    break;
  case 0x01:
    ...
    break;
  case 0x02:
    PrintAndLogEx(INFO, "     File Type: 0x%02X -> Value Files with Backup",
                  filetype);
    break;
  case 0x03:
    PrintAndLogEx(INFO,
                  "     File Type: 0x%02X -> Linear Record Files with Backup",
                  filetype);
    break;
  case 0x04:
    PrintAndLogEx(INFO,
                  "     File Type: 0x%02X -> Cyclic Record Files with Backup",
                  filetype);
    break;
  default:
...
static void DecodeAccessRights(uint16_t accrights) {
  ...
  PrintAndLogEx(INFO,
                "     Access Rights: 0x%04X - Change %s - RW %s - W %s - R %s",
                accrights, car, rwa, wa, ra);
  free(car);
  free(rwa);
  free(wa);
  free(ra);
...
static int DecodeFileSettings(uint8_t *src, int src_len, int maclen) {
  ...
  if (src_len == 1 + 1 + 2 + 3 + maclen) {
    ...
    int limitcredvalue =
        (src[15] << 24) + (src[14] << 16) + (src[13] << 8) + src[12];
    ...
    PrintAndLogEx(
        INFO,
        "     Lower limit: %d (0x%X) - Upper limit: %d (0x%X) - limited credit "
        "value: %d (0x%X) - limited credit enabled: %d",
        lowerlimit, lowerlimit, upperlimit, upperlimit, limitcredvalue,
        limitcredvalue, limited_credit_enabled);
    return PM3_SUCCESS;
    ...
    DecodeAccessRights(accrights);
    PrintAndLogEx(INFO,
                  "     Record size: %d (0x%X) - MaxNumberRecords: %d (0x%X) - "
                  "Current Number Records: %d (0x%X)",
                  recordsize, recordsize, maxrecords, maxrecords, currentrecord,
                  currentrecord);
    return PM3_SUCCESS;
  }
  return PM3_ESOFT;
...
static int CmdHF14ADesDump(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mfdes dump",
                "Tries to dump all files on a DESFire tag", "hf mfdes dump");

  void *argtable[] = {arg_param_begin,
                      //        arg_strx0("a",  "aid",      "<aid>", "Use
                      //        specific AID (3 hex bytes, big endian)"),
                      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
  CLIParserFree(ctx);
  ...
  for (uint32_t i = 0; i < app_ids_len; i += 3) {
    ...
    PrintAndLogEx(SUCCESS, "  AID : " _GREEN_("%02X%02X%02X"), aid[2], aid[1],
                  aid[0]);
    if ((aid[2] >> 4) == 0xF) {
      uint16_t short_aid =
          ((aid[2] & 0xF) << 12) | (aid[1] << 4) | (aid[0] >> 4);
      PrintAndLogEx(
          SUCCESS,
          "  AID mapped to MIFARE Classic AID (MAD): " _YELLOW_("%02X"),
          short_aid);
      PrintAndLogEx(SUCCESS, "  MAD AID Cluster  0x%02X      : " _YELLOW_("%s"),
                    short_aid >> 8, cluster_to_text(short_aid >> 8));
      MADDFDecodeAndPrint(short_aid);
    } else {
      AIDDFDecodeAndPrint(aid);
    }
    for (uint8_t m = 0; m < dfname_count; m++) {
      if (dfnames[m].aid[0] == aid[0] && dfnames[m].aid[1] == aid[1] &&
          dfnames[m].aid[2] == aid[2]) {
        PrintAndLogEx(SUCCESS,
                      "  -  DF " _YELLOW_("%02X%02X") " Name : " _YELLOW_("%s"),
                      dfnames[m].fid[1], dfnames[m].fid[0], dfnames[m].name);
      }
    }
    ...
    for (int j = (int)file_ids_len - 1; j >= 0; j--) {
      ...
      res =
          handler_desfire_filesettings(file_ids[j], filesettings, &fileset_len);
      if (res != PM3_SUCCESS)
        continue;
      ...
      if (fileset_len == 1 + 1 + 2 + 3 + maclen) {
        int filesize =
            (filesettings[6] << 16) + (filesettings[5] << 8) + filesettings[4];
        mfdes_data_t fdata;
        ...
        fdata.data = data;
        res =
            handler_desfire_readdata(&fdata, MFDES_DATA_FILE, filesettings[1]);
        if (res == PM3_SUCCESS) {
          PrintAndLogEx(NORMAL, "\nOffset  | Data                              "
                                "              | Ascii");
          PrintAndLogEx(NORMAL, "----------------------------------------------"
          ...
          for (uint32_t n = 0; n < len; n += 16) {
            PrintAndLogEx(NORMAL, "%02d/0x%02X | %s| %s", n, n,
                          sprint_hex(&fdata.data[n], len > 16 ? 16 : len),
                          sprint_ascii(&fdata.data[n], len > 16 ? 16 : len));
          }
        } else {
          PrintAndLogEx(ERR, "Couldn't read value. Error %d", res);
          res = handler_desfire_select_application(aid);
        ...
        if (res == PM3_SUCCESS) {
          ...
          for (uint32_t n = 0; n < len; n += 16) {
            PrintAndLogEx(NORMAL, "%02d/0x%02X | %s| %s", n, n,
                          sprint_hex(&value.value[n], len > 16 ? 16 : len),
                          sprint_ascii(&value.value[n], len > 16 ? 16 : len));
          }
        } else {
          PrintAndLogEx(ERR, "Couldn't read value. Error %d", res);
          res = handler_desfire_select_application(aid);
        ...
        uint32_t maxrecords =
            (filesettings[9] << 16) + (filesettings[8] << 8) + filesettings[7];
        uint32_t filesize =
            (filesettings[6] << 16) + (filesettings[5] << 8) + filesettings[4];
        mfdes_data_t fdata;
        fdata.fileno = file_ids[j];
...
static int CmdHF14ADesEnumApplications(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mfdes enum",
                "Enumerate all AID's on MIFARE DESfire tag", "hf mfdes enum");
  ...
  PrintAndLogEx(NORMAL, "");
  PrintAndLogEx(
      INFO, "-- MIFARE DESFire Enumerate applications --------------------");
  PrintAndLogEx(
      INFO, "-------------------------------------------------------------");
  PrintAndLogEx(SUCCESS, " Tag report " _GREEN_("%d") " application%c",
                app_ids_len / 3, (app_ids_len == 3) ? ' ' : 's');
  ...
  for (uint32_t i = 0; i < app_ids_len; i += 3) {
    ...
    PrintAndLogEx(SUCCESS, " Tag report " _GREEN_("%d") " file%c", file_ids_len,
                  (file_ids_len == 1) ? ' ' : 's');
    for (int j = (int)file_ids_len - 1; j >= 0; j--) {
      ...
      if (DecodeFileSettings(filesettings, fileset_len, maclen) !=
          PM3_SUCCESS) {
        PrintAndLogEx(INFO, "  Settings [%u] %s", fileset_len,
                      sprint_hex(filesettings, fileset_len));
      }
    }
  }
  PrintAndLogEx(
...
static int CmdHF14ADesBruteApps(const char *Cmd) {
  ...
  void *argtable[] = {
      arg_param_begin,
      arg_strx0("s", "start", "<hex>",
                "Starting App ID as hex bytes (3 bytes,big endian)"),
      arg_strx0("e", "end", "<hex>",
                "Last App ID as hex bytes (3 bytes,big endian)"),
      arg_int0("i", "step", "<dec>",
               "Increment step when bruteforcing (decimal integer)"),
  ...
  CLIParserFree(ctx);
  // TODO: We need to check the tag version, EV1 should stop after 26 apps are
  // found
  if (mad) {
    idIncrement = 0x10;
    startAid[0] = 0xF0;
  ...
  PrintAndLogEx(
      INFO, "Enumerating through all AIDs manually, this will take a while!");
  for (uint32_t id = idStart; id <= idEnd && id >= idStart; id += idIncrement) {
    ...
    uint8_t appId[3] = {0};
    htole24(id, appId);
    sAPDU apdu = {0x90, MFDES_SELECT_APPLICATION, 0x00, 0x00, 0x03,
                  appId}; // 0x5a
    uint16_t sw = 0;
    uint8_t data[255 * 5] = {0x00};
    uint32_t resplen = 0;
    DESFIRESendApdu(!tag->rf_field_on, true, apdu, data, sizeof(data), &resplen,
                    &sw);
    if (sw == status(MFDES_S_OPERATION_OK)) {
...
static int CmdHF14ADesChangeKey(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mfdes changekey",
                ...
                "Make sure to select aid or authenticate aid before running "
                "this command.",
                "hf mfdes changekey -n 0 -t 1 -k 0000000000000000 -u 1 -j "
                "0102030405060708 -> DES,keynumber 0");

  void *argtable[] = {
      arg_param_begin,
      arg_int0("n", "keyno", "<keyno>", "Key number used for authentification"),
      arg_int0("t", "algo", "<algo>",
               "Current key algo (1=DES, 2=3DES(2K2DES), 3=3K3DES, 4=AES)"),
      arg_str0("k", "key", "<Key>", "Current Key (HEX 8-24 bytes)"),
      arg_int0("u", "newalgo", "<newalgo>",
               "New key algo (1=DES, 2=3DES(2K2DES), 3=3K3DES, 4=AES)"),
      arg_str0("j", "newkey", "<newkey>", "New Key (HEX 8-24 bytes)"),
      arg_int0("v", "aesversion", "<aesversion>",
               "Aes version (if aes is used)"),
      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, false);
  ...
  PrintAndLogEx(INFO, "old key: %s (%s)", sprint_hex_inrow(key, keylen),
                getEncryptionAlgoStr(cmdAuthAlgo));
  PrintAndLogEx(INFO, "new key: %s (%s)", sprint_hex_inrow(newkey, newkeylen),
                getEncryptionAlgoStr(newcmdAuthAlgo));

  int error = mifare_desfire_change_key(cmdKeyNo, newkey, newcmdAuthAlgo, key,
                                        cmdAuthAlgo, aesversion);
  if (error == PM3_SUCCESS) {
...
static int CmdHF14ADesAuth(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "hf mfdes auth", "Authenticates MIFARE DESFire using Key",
      "hf mfdes auth -m 3 -t 4 -a 808301 -n 0 -k "
      "00000000000000000000000000000000 -> AES,keynumber 0, aid 0x803201\n"
      "hf mfdes auth -m 2 -t 2 -a 000000 -n 1 -k "
      "00000000000000000000000000000000 -> 3DES,keynumber 1, aid 0x000000\n"
      "hf mfdes auth -m 1 -t 1 -a 000000 -n 2 -k 0000000000000000 -> "
      "DES,keynumber 2, aid 0x000000\n"
      "hf mfdes auth -m 1 -t 1 -a 000000 -n 0 -> DES, defaultkey, aid "
      "0x000000\n"
      "hf mfdes auth -m 2 -t 2 -a 000000 -n 0 -> 3DES, defaultkey, aid "
      "0x000000\n"
      "hf mfdes auth -m 3 -t 4 -a 000000 -n 0 -> 3K3DES, defaultkey, aid "
      "0x000000\n"
      "hf mfdes auth -m 3 -t 4 -a 000000 -n 0 -> AES, defaultkey, aid "
      "0x000000");
  ...
  void *argtable[] = {
      ...
      arg_int0("t", "algo", "<algo>",
               "Crypt algo (1=DES, 2=3DES(2K2DES), 3=3K3DES, 4=AES)"),
      arg_strx0("a", "aid", "<aid>",
                "AID used for authentification (HEX 3 bytes)"),
      arg_int0("n", "keyno", "<keyno>", "Key number used for authentification"),
      arg_str0("k", "key", "<Key>", "Key for checking (HEX 8-24 bytes)"),
      arg_int0(
          "d", "kdf", "<kdf>",
          "Key Derivation Function (KDF) (0=None, 1=AN10922, 2=Gallagher)"),
  ...
  mfdes_auth_res_t rpayload;
  int error =
      desfire_authenticate(cmdAuthMode, cmdAuthAlgo, aid, key, cmdKeyNo,
                           cmdKDFAlgo, kdfInputLen, kdfInput, &rpayload);
  if (error == PM3_SUCCESS) {
    PrintAndLogEx(SUCCESS, "  Key        : " _GREEN_("%s"),
                  sprint_hex(key, keylength));
    PrintAndLogEx(SUCCESS, "  SESSION    : " _GREEN_("%s"),
                  sprint_hex(rpayload.sessionkey, keylength));
    PrintAndLogEx(
        INFO, "-------------------------------------------------------------");
  } else {
    AuthToError(error);
...
static void DesFill2bPattern(uint8_t deskeyList[MAX_KEYS_LIST_LEN][8],
                             uint32_t *deskeyListLen,
                             uint8_t aeskeyList[MAX_KEYS_LIST_LEN][16],
                             uint32_t *aeskeyListLen,
                             uint8_t k3kkeyList[MAX_KEYS_LIST_LEN][24],
                             uint32_t *k3kkeyListLen, uint32_t *startPattern) {
  ...
  for (uint32_t pt = *startPattern; pt < 0x10000; pt++) {
    ...
    if (*k3kkeyListLen != MAX_KEYS_LIST_LEN) {
      ...
      memcpy(&k3kkeyList[*k3kkeyListLen][16], &k3kkeyList[*k3kkeyListLen][0],
             4);
      (*k3kkeyListLen)++;
    }
...
static int AuthCheckDesfire(
    uint8_t *aid, uint8_t deskeyList[MAX_KEYS_LIST_LEN][8],
    uint32_t deskeyListLen, uint8_t aeskeyList[MAX_KEYS_LIST_LEN][16],
    uint32_t aeskeyListLen, uint8_t k3kkeyList[MAX_KEYS_LIST_LEN][24],
    uint32_t k3kkeyListLen, uint8_t cmdKdfAlgo, uint8_t kdfInputLen,
    uint8_t *kdfInput, uint8_t foundKeys[4][0xE][24 + 1], bool *result) {

  uint32_t curaid =
      (aid[0] & 0xFF) + ((aid[1] & 0xFF) << 8) + ((aid[2] & 0xFF) << 16);
  ...
  if (memcmp(aid, "\x00\x00\x00", 3) != 0) {
    ...
    if (handler_desfire_fileids(file_ids, &file_ids_len) == PM3_SUCCESS) {
      ...
      for (int j = (int)file_ids_len - 1; j >= 0; j--) {
        ...
        res = handler_desfire_filesettings(file_ids[j], filesettings,
                                           &fileset_len);
        if (res == PM3_SUCCESS) {
          ...
          if (change_access_rights == 0xE)
            change_access_rights = 0x0;
          if (read_write_access == 0xE)
            read_write_access = 0x0;
          if (write_access == 0xE)
            write_access = 0x0;
          if (read_access == 0xE)
            read_access = 0x0;
    ...
    for (uint8_t keyno = 0; keyno < 0xE; keyno++) {
      ...
      if (usedkeys[keyno] == 1 && foundKeys[0][keyno][0] == 0) {
        for (uint32_t curkey = 0; curkey < deskeyListLen; curkey++) {
          mfdes_auth_res_t rpayload;
          error = desfire_authenticate(MFDES_AUTH_DES, MFDES_ALGO_DES, aid,
                                       deskeyList[curkey], keyno, 0, 0, NULL,
                                       &rpayload);
          if (error == PM3_SUCCESS) {
            PrintAndLogEx(
                SUCCESS, "AID 0x%06X, Found DES Key %u        : " _GREEN_("%s"),
                curaid, keyno, sprint_hex(deskeyList[curkey], 8));
    ...
    for (uint8_t keyno = 0; keyno < 0xE; keyno++) {
      ...
      if (usedkeys[keyno] == 1 && foundKeys[1][keyno][0] == 0) {
        for (uint32_t curkey = 0; curkey < aeskeyListLen; curkey++) {
          mfdes_auth_res_t rpayload;
          error = desfire_authenticate(MFDES_AUTH_DES, MFDES_ALGO_3DES, aid,
                                       aeskeyList[curkey], keyno, 0, 0, NULL,
                                       &rpayload);
          if (error == PM3_SUCCESS) {
            PrintAndLogEx(
                SUCCESS,
                "AID 0x%06X, Found 3DES Key %u        : " _GREEN_("%s"), curaid,
                keyno, sprint_hex(aeskeyList[curkey], 16));
  ...
  if (aes) {
    ...
    for (uint8_t keyno = 0; keyno < 0xE; keyno++) {
      ...
      if (usedkeys[keyno] == 1 && foundKeys[2][keyno][0] == 0) {
        for (uint32_t curkey = 0; curkey < aeskeyListLen; curkey++) {
          mfdes_auth_res_t rpayload;
          error = desfire_authenticate(MFDES_AUTH_AES, MFDES_ALGO_AES, aid,
                                       aeskeyList[curkey], keyno, cmdKdfAlgo,
                                       kdfInputLen, kdfInput, &rpayload);
          if (error == PM3_SUCCESS) {
            PrintAndLogEx(
                SUCCESS, "AID 0x%06X, Found AES Key %u        : " _GREEN_("%s"),
                curaid, keyno, sprint_hex(aeskeyList[curkey], 16));
    ...
    for (uint8_t keyno = 0; keyno < 0xE; keyno++) {
      ...
      if (usedkeys[keyno] == 1 && foundKeys[3][keyno][0] == 0) {
        for (uint32_t curkey = 0; curkey < k3kkeyListLen; curkey++) {
          mfdes_auth_res_t rpayload;
          error = desfire_authenticate(MFDES_AUTH_ISO, MFDES_ALGO_3K3DES, aid,
                                       k3kkeyList[curkey], keyno, 0, 0, NULL,
                                       &rpayload);
          if (error == PM3_SUCCESS) {
            PrintAndLogEx(
                SUCCESS, "AID 0x%06X, Found 3K3 Key %u        : " _GREEN_("%s"),
                curaid, keyno, sprint_hex(k3kkeyList[curkey], 24));
...
static int CmdHF14aDesChk(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "hf mfdes chk", "Checks keys with MIFARE DESFire card.",
      "hf mfdes chk -a 123456 -k 000102030405060708090a0b0c0d0e0f -> check key "
      "on aid 0x123456\n"
      "hf mfdes chk -d mfdes_default_keys -> check keys from dictionary "
      "against all existing aid on card\n"
      "hf mfdes chk -d mfdes_default_keys -a 123456 -> check keys from "
      "dictionary against aid 0x123456\n"
      "hf mfdes chk -a 123456 --pattern1b -j keys -> check all 1-byte keys "
      "pattern on aid 0x123456 and save found keys to json\n"
      "hf mfdes chk -a 123456 --pattern2b --startp2b FA00 -> check all 2-byte "
      "keys pattern on aid 0x123456. Start from key FA00FA00...FA00");

  void *argtable[] = {
      arg_param_begin,
      arg_strx0("a", "aid", "<aid>",
                "Use specific AID (3 hex bytes, big endian)"),
      arg_str0("k", "key", "<Key>", "Key for checking (HEX 16 bytes)"),
      arg_str0("d", "dict", "<file>", "File with keys dictionary"),
      arg_lit0(NULL, "pattern1b",
               "Check all 1-byte combinations of key (0000...0000, "
               "0101...0101, 0202...0202, ...)"),
      arg_lit0(NULL, "pattern2b",
               "Check all 2-byte combinations of key (0000...0000, "
               "0001...0001, 0002...0002, ...)"),
      arg_str0(
          NULL, "startp2b", "<Pattern>",
          "Start key (2-byte HEX) for 2-byte search (use with `--pattern2b`)"),
  ...
  if (CLIParamStrToBuf(arg_get_str(ctx, 3), dict_filename, FILE_PATH_SIZE,
                       &dict_filenamelen)) {
  ...
  if (dict_filenamelen && (pattern1b || pattern2b)) {
    PrintAndLogEx(ERR, "Pattern search mode and dictionary mode can't be used "
                       "in one command.");
    CLIParserFree(ctx);
    return PM3_EINVARG;
  }
  ...
  int jsonnamelen = 0;
  if (CLIParamStrToBuf(arg_get_str(ctx, 7), jsonname, sizeof(jsonname),
                       &jsonnamelen)) {
  ...
  if (pattern2b) {
    DesFill2bPattern(deskeyList, &deskeyListLen, aeskeyList, &aeskeyListLen,
                     k3kkeyList, &k3kkeyListLen, &startPattern);
  }
  ...
  if (dict_filenamelen) {
    ...
    res = loadFileDICTIONARYEx((char *)dict_filename, deskeyList,
                               sizeof(deskeyList), NULL, 8, &deskeyListLen, 0,
                               &endFilePosition, true);
    if (res == PM3_SUCCESS && endFilePosition)
      PrintAndLogEx(SUCCESS,
                    "First part of des dictionary successfully loaded.");

    endFilePosition = 0;
    res = loadFileDICTIONARYEx((char *)dict_filename, aeskeyList,
                               sizeof(aeskeyList), NULL, 16, &aeskeyListLen, 0,
                               &endFilePosition, true);
    if (res == PM3_SUCCESS && endFilePosition)
      PrintAndLogEx(SUCCESS,
    ...
    endFilePosition = 0;
    res = loadFileDICTIONARYEx((char *)dict_filename, k3kkeyList,
                               sizeof(k3kkeyList), NULL, 24, &k3kkeyListLen, 0,
                               &endFilePosition, true);
    if (res == PM3_SUCCESS && endFilePosition)
      PrintAndLogEx(SUCCESS,
  ...
  if (aeskeyListLen != 0) {
    PrintAndLogEx(INFO, "Loaded " _YELLOW_("%" PRIu32) " aes keys",
                  aeskeyListLen);
  }

  if (deskeyListLen != 0) {
    PrintAndLogEx(INFO, "Loaded " _YELLOW_("%" PRIu32) " des keys",
                  deskeyListLen);
  }

  if (k3kkeyListLen != 0) {
    PrintAndLogEx(INFO, "Loaded " _YELLOW_("%" PRIu32) " k3kdes keys",
                  k3kkeyListLen);
  }
  ...
  for (uint32_t x = 0; x < app_ids_len / 3; x++) {
    ...
    uint32_t curaid = (app_ids[x * 3] & 0xFF) +
                      ((app_ids[(x * 3) + 1] & 0xFF) << 8) +
                      ((app_ids[(x * 3) + 2] & 0xFF) << 16);
    PrintAndLogEx(ERR, "Checking aid 0x%06X...", curaid);

    res =
        AuthCheckDesfire(&app_ids[x * 3], deskeyList, deskeyListLen, aeskeyList,
                         aeskeyListLen, k3kkeyList, k3kkeyListLen, cmdKDFAlgo,
                         kdfInputLen, kdfInput, foundKeys, &result);
    ...
    if (dict_filenamelen) {
      ...
      uint32_t keycnt = 0;
      res = loadFileDICTIONARYEx((char *)dict_filename, deskeyList,
                                 sizeof(deskeyList), NULL, 16, &keycnt,
                                 endFilePosition, &endFilePosition, false);
      if (res == PM3_SUCCESS && endFilePosition)
        deskeyListLen = keycnt;

      keycnt = 0;
      res = loadFileDICTIONARYEx((char *)dict_filename, aeskeyList,
                                 sizeof(aeskeyList), NULL, 16, &keycnt,
                                 endFilePosition, &endFilePosition, false);
      if (res == PM3_SUCCESS && endFilePosition)
      ...
      keycnt = 0;
      res = loadFileDICTIONARYEx((char *)dict_filename, k3kkeyList,
                                 sizeof(k3kkeyList), NULL, 16, &keycnt,
                                 endFilePosition, &endFilePosition, false);
      if (res == PM3_SUCCESS && endFilePosition)
  ...
  if ((jsonnamelen > 0) && result) {
    ...
    uint64_t select_status =
        resp.oldarg[0]; // 0: couldn't read, 1: OK, with ATS, 2: OK, no ATS, 3:
                        // proprietary Anticollision
    ...
    // length:
    // UID(10b)+SAK(1b)+ATQA(2b)+ATSlen(1b)+ATS(atslen)+foundKeys[2][64][AES_KEY_LEN
    // + 1]
...
static int CmdHF14aDesNDEF(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf mfdes ndef",
                  ...
                  "hf mfdes ndef -a e103 -k d3f7d3f7d3f7d3f7d3f7d3f7d3f7d3f7 ->
shows NDEF data with custom AID and key");
...
file_ids[j]);
        ...
        int res = handler_desfire_filesettings(file_ids[j], filesettings,
&fileset_len); if (res != PM3_SUCCESS) continue;

        int maclen = 0; // To be implemented

        if (fileset_len == 1 + 1 + 2 + 3 + maclen) {
            int filesize = (filesettings[6] << 16) + (filesettings[5] << 8) +
filesettings[4]; mfdes_data_t fdata; fdata.fileno = file_ids[j];
            memset(fdata.offset, 0, 3);
            memset(fdata.length, 0, 3);
...
filesettings[1]); if (res == PM3_SUCCESS) { uint32_t len =
le24toh(fdata.length); NDEFDecodeAndPrint(data, datalen, verbose);
...
----------------"); print_buffer(data, datalen, 1);
    ...
    PrintAndLogEx(HINT, "Try " _YELLOW_("`hf mfdes ndef -vv`") " for more
details"); return PM3_SUCCESS;
}
*/
/*
static int CmdHF14aDesMAD(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf mfdes mad",
                  ...
                  "hf mfdes mad -a e103 -k d3f7d3f7d3f7d3f7d3f7d3f7d3f7d3f7 ->
shows MAD data with custom AID and key");
    ...
    PrintAndLogEx(HINT, "Try " _YELLOW_("`hf mfdes mad -v`") " for more
details"); return PM3_SUCCESS;
}
*/
...
/*static int CmdTest(const char *Cmd) {
    ...
    uint8_t key[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    ...
    PrintAndLogEx(SUCCESS, "  Res        : " _GREEN_("%s"), sprint_hex(both,
16)); return PM3_SUCCESS;
}
*/
...
static command_t CommandTable[] = {
    ...
    {"-----------", CmdHelp, IfPm3Iso14443a,
     "----------------------- " _CYAN_("general") " -----------------------"},
    {"auth", CmdHF14ADesAuth, IfPm3Iso14443a,
    ...
    {"enum", CmdHF14ADesEnumApplications, IfPm3Iso14443a,
     "Tries enumerate all applications"},
    {"formatpicc", CmdHF14ADesFormatPICC, IfPm3Iso14443a, "Format PICC"},
    {"getuid", CmdHF14ADesGetUID, IfPm3Iso14443a, "Get random uid"},
    {"info", CmdHF14ADesInfo, IfPm3Iso14443a, "Tag information"},
    {"list", CmdHF14ADesList, AlwaysAvailable,
     "List DESFire (ISO 14443A) history"},
    //    {"ndef",             CmdHF14aDesNDEF,             IfPm3Iso14443a,
    //    "Prints NDEF records from card"},
    //    {"mad",             CmdHF14aDesMAD,             IfPm3Iso14443a,
    //    "Prints MAD records from card"},
    {"-----------", CmdHelp, IfPm3Iso14443a,
     "----------------------- " _CYAN_("AID") " -----------------------"},
    {"bruteaid", CmdHF14ADesBruteApps, IfPm3Iso14443a,
     "Recover AIDs by bruteforce"},
    {"createaid", CmdHF14ADesCreateApp, IfPm3Iso14443a,
     "Create Application ID"},
    {"deleteaid", CmdHF14ADesDeleteApp, IfPm3Iso14443a,
     "Delete Application ID"},
    {"selectaid", CmdHF14ADesSelectApp, IfPm3Iso14443a,
     "Select Application ID"},
    {"-----------", CmdHelp, IfPm3Iso14443a,
     "----------------------- " _CYAN_("Files") " -----------------------"},
    {"changevalue", CmdHF14ADesChangeValue, IfPm3Iso14443a,
     "Write value of a value file (credit/debit/clear)"},
    {"clearfile", CmdHF14ADesClearRecordFile, IfPm3Iso14443a,
     "Clear record File"},
    {"createfile", CmdHF14ADesCreateFile, IfPm3Iso14443a,
     "Create Standard/Backup File"},
    {"createvaluefile", CmdHF14ADesCreateValueFile, IfPm3Iso14443a,
     "Create Value File"},
    {"createrecordfile", CmdHF14ADesCreateRecordFile, IfPm3Iso14443a,
     "Create Linear/Cyclic Record File"},
    ...
    {"readdata", CmdHF14ADesReadData, IfPm3Iso14443a,
     "Read data from standard/backup/record file"},
    {"writedata", CmdHF14ADesWriteData, IfPm3Iso14443a,
