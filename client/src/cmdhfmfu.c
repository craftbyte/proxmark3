...
uint8_t default_3des_keys[][16] = {
    {0x42, 0x52, 0x45, 0x41, 0x4b, 0x4d, 0x45, 0x49, 0x46, 0x59, 0x4f, 0x55,
     0x43, 0x41, 0x4e, 0x21}, // 3des std key
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
     0x00, 0x00, 0x00, 0x00}, // all zeroes
    {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
     0x0c, 0x0d, 0x0e, 0x0f}, // 0x00-0x0F
    {0x49, 0x45, 0x4D, 0x4B, 0x41, 0x45, 0x52, 0x42, 0x21, 0x4E, 0x41, 0x43,
     0x55, 0x4F, 0x59, 0x46}, // NFC-key
    {0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
     0x01, 0x01, 0x01, 0x01}, // all ones
    {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
     0xFF, 0xFF, 0xFF, 0xFF}, // all FF
    {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB,
     0xCC, 0xDD, 0xEE, 0xFF} // 11 22 33
};
...
uint32_t UL_TYPES_ARRAY[] = {
    UNKNOWN,        UL,         UL_C,        UL_EV1_48,
    UL_EV1_128,     NTAG,       NTAG_203,    NTAG_210,
    NTAG_212,       NTAG_213,   NTAG_215,    NTAG_216,
    MY_D,           MY_D_NFC,   MY_D_MOVE,   MY_D_MOVE_NFC,
    MY_D_MOVE_LEAN, FUDAN_UL,   UL_EV1,      NTAG_213_F,
    NTAG_216_F,     UL_NANO_40, NTAG_I2C_1K, NTAG_213_TT,
    NTAG_213_C};

uint8_t UL_MEMORY_ARRAY[ARRAYLEN(UL_TYPES_ARRAY)] = {
    MAX_UL_BLOCKS,      MAX_UL_BLOCKS,  MAX_ULC_BLOCKS,    MAX_ULEV1a_BLOCKS,
    MAX_ULEV1b_BLOCKS,  MAX_NTAG_203,   MAX_NTAG_203,      MAX_NTAG_210,
    MAX_NTAG_212,       MAX_NTAG_213,   MAX_NTAG_215,      MAX_NTAG_216,
    MAX_UL_BLOCKS,      MAX_MY_D_NFC,   MAX_MY_D_MOVE,     MAX_MY_D_MOVE,
    MAX_MY_D_MOVE_LEAN, MAX_UL_BLOCKS,  MAX_ULEV1a_BLOCKS, MAX_NTAG_213,
    MAX_NTAG_216,       MAX_UL_NANO_40, MAX_NTAG_I2C_1K,   MAX_NTAG_213,
    MAX_NTAG_213};
...
int ul_read_uid(uint8_t *uid) {
  ...
  // read uid from tag
  clearCommandBuffer();
  SendCommandMIX(CMD_HF_ISO14443A_READER, ISO14A_CONNECT | ISO14A_NO_RATS, 0, 0,
                 NULL, 0);
  ...
  if (card.uidlen != 7) {
    PrintAndLogEx(WARNING, "Wrong sized UID, expected 7bytes got %d",
                  card.uidlen);
    return PM3_ESOFT;
  }
...
static void ul_switch_on_field(void) {
  ...
  SendCommandMIX(CMD_HF_ISO14443A_READER,
                 ISO14A_CONNECT | ISO14A_NO_DISCONNECT | ISO14A_NO_RATS, 0, 0,
                 NULL, 0);
}

static int ul_send_cmd_raw(uint8_t *cmd, uint8_t cmdlen, uint8_t *response,
                           uint16_t responseLength) {
  clearCommandBuffer();
  SendCommandOLD(CMD_HF_ISO14443A_READER,
                 ISO14A_RAW | ISO14A_NO_DISCONNECT | ISO14A_APPEND_CRC |
                     ISO14A_NO_RATS,
                 cmdlen, 0, cmd, cmdlen);
  PacketResponseNG resp;
  ...
  uint16_t resplen =
      (resp.oldarg[0] < responseLength) ? resp.oldarg[0] : responseLength;
...
static int ul_auth_select(iso14a_card_select_t *card, TagTypeUL_t tagtype,
                          bool hasAuthKey, uint8_t *authkey, uint8_t *pack,
                          uint8_t packSize) {
...
static int ulev1_readTearing(uint8_t counter, uint8_t *response,
                             uint16_t responseLength) {
...
static int ul_fudan_check(void) {
  ...
  uint8_t cmd[4] = {0x30, 0x00, 0x02,
                    0xa7}; // wrong crc on purpose  should be 0xa8
  clearCommandBuffer();
  SendCommandMIX(CMD_HF_ISO14443A_READER,
                 ISO14A_RAW | ISO14A_NO_DISCONNECT | ISO14A_NO_RATS, 4, 0, cmd,
                 sizeof(cmd));
  PacketResponseNG resp;
  if (!WaitForResponseTimeout(CMD_ACK, &resp, 1500))
    return UL_ERROR;
  ...
  return (!resp.data.asBytes[0])
             ? FUDAN_UL
             : UL; // if response == 0x00 then Fudan, else Genuine NXP
}
...
static int ul_print_default(uint8_t *data, uint8_t *real_uid) {
  ...
  PrintAndLogEx(SUCCESS, "    UID[0]: %02X, %s", real_uid[0],
                getTagInfo(real_uid[0]));
  if (real_uid[0] == 0x05 &&
      ((real_uid[1] & 0xf0) >> 4) == 2) { // is infineon and 66RxxP
  ...
  if (mful_uid_layout) {
    ...
    if (data[3] == crc0)
      ...
      PrintAndLogEx(NORMAL, "      BCC0: %02X, crc should be %02X", data[3],
                    crc0);
    ...
    if (data[8] == crc1)
      ...
      PrintAndLogEx(NORMAL, "      BCC1: %02X, crc should be %02X", data[8],
                    crc1);
    PrintAndLogEx(SUCCESS, "  Internal: %02X (%s)", data[9],
                  (data[9] == 0x48) ? _GREEN_("default")
                                    : _RED_("not default"));
  } else {
    PrintAndLogEx(SUCCESS, "Blocks 0-2: %s", sprint_hex(data + 0, 12));
  }

  PrintAndLogEx(SUCCESS, "      Lock: %s - %s", sprint_hex(data + 10, 2),
                sprint_bin(data + 10, 2));

  PrintAndLogEx(SUCCESS, "OneTimePad: %s - %s", sprint_hex(data + 12, 4),
...
static int ndef_print_CC(uint8_t *data) {
  ...
  //    PrintAndLogEx(SUCCESS, "  %02X : version %d.%d supported by tag",
  //    data[1], (data[1] & 0xF0) >> 4, data[1] & 0x0F);
  PrintAndLogEx(SUCCESS, "  %02X: version %d.%d supported by tag", data[1],
                cc_major, cc_minor);
  PrintAndLogEx(SUCCESS, "       : %s / %s", rStr, wStr);

  PrintAndLogEx(SUCCESS, "  %02X: Physical Memory Size: %d bytes", data[2],
                data[2] * 8);
  if (data[2] == 0x06)
  ...
  PrintAndLogEx(SUCCESS, "  xxx      - %02X: RFU (%s)", msb3,
                (msb3 == 0) ? _GREEN_("ok") : _RED_("fail"));
  PrintAndLogEx(SUCCESS, "     x     - %02X: %s special frame", sf,
                (sf) ? "support" : "don\'t support");
  PrintAndLogEx(SUCCESS, "      x    - %02X: %s lock block", lb,
                (lb) ? "support" : "don\'t support");
  PrintAndLogEx(SUCCESS, "       xx  - %02X: RFU (%s)", mlrule,
                (mlrule == 0) ? _GREEN_("ok") : _RED_("fail"));
  PrintAndLogEx(SUCCESS, "         x - %02X: IC %s multiple block reads",
                mbread, (mbread) ? "support" : "don\'t support");
  return PM3_SUCCESS;
}
...
int ul_print_type(uint32_t tagtype, uint8_t spaces) {
  ...
  if (tagtype & UL)
    ...
    snprintf(typestr, sizeof(typestr),
             "%*sTYPE: " _YELLOW_("MIFARE Ultralight Nano 40bytes (MF0UNH00)"),
             spaces, "");
  else if (tagtype & UL_EV1_48)
    snprintf(typestr, sizeof(typestr),
             "%*sTYPE: " _YELLOW_("MIFARE Ultralight EV1 48bytes (MF0UL1101)"),
             spaces, "");
  else if (tagtype & UL_EV1_128)
    snprintf(typestr, sizeof(typestr),
             "%*sTYPE: " _YELLOW_("MIFARE Ultralight EV1 128bytes (MF0UL2101)"),
             spaces, "");
  else if (tagtype & UL_EV1)
    snprintf(typestr, sizeof(typestr),
             "%*sTYPE: " _YELLOW_("MIFARE Ultralight EV1 UNKNOWN"), spaces, "");
  else if (tagtype & NTAG)
    snprintf(typestr, sizeof(typestr), "%*sTYPE: " _YELLOW_("NTAG UNKNOWN"),
             spaces, "");
    ...
    snprintf(typestr, sizeof(typestr),
             "%*sTYPE: " _YELLOW_("NTAG I2C plus 888bytes (NT3H2111FHK)"),
             spaces, "");
  else if (tagtype & NTAG_I2C_2K_PLUS)
    snprintf(typestr, sizeof(typestr),
             "%*sTYPE: " _YELLOW_("NTAG I2C plus 1912bytes (NT3H2211FHK)"),
             spaces, "");
    ...
    snprintf(typestr, sizeof(typestr),
             "%*sTYPE: " _YELLOW_("INFINEON my-d\x99 move (SLE 66R01P)"),
             spaces, "");
  else if (tagtype & MY_D_MOVE_NFC)
    snprintf(typestr, sizeof(typestr),
             "%*sTYPE: " _YELLOW_("INFINEON my-d\x99 move NFC (SLE 66R01P)"),
             spaces, "");
  else if (tagtype & MY_D_MOVE_LEAN)
    snprintf(typestr, sizeof(typestr),
             "%*sTYPE: " _YELLOW_("INFINEON my-d\x99 move lean (SLE 66R01L)"),
             spaces, "");
  else if (tagtype & FUDAN_UL)
    snprintf(typestr, sizeof(typestr),
             "%*sTYPE: " _YELLOW_(
                 "FUDAN Ultralight Compatible (or other compatible)"),
             spaces, "");
  else
    snprintf(typestr, sizeof(typestr), "%*sTYPE: " _YELLOW_("Unknown %06x"),
             spaces, "", tagtype);
  ...
  snprintf(typestr + strlen(typestr), sizeof(typestr) - strlen(typestr), " %s ",
           (tagtype & MAGIC) ? _GREEN_("magic") : "");
  tagtype &= ~(MAGIC);
  snprintf(typestr + strlen(typestr), sizeof(typestr) - strlen(typestr), "%s",
           (tagtype & MAGIC_1A) ? _GREEN_("Gen 1a") : "");
  snprintf(typestr + strlen(typestr), sizeof(typestr) - strlen(typestr), "%s",
           (tagtype & MAGIC_1B) ? _GREEN_("Gen 1b") : "");
...
static int ulc_print_3deskey(uint8_t *data) {
  PrintAndLogEx(INFO, "    deskey1 [44/0x2C]: %s [%s]", sprint_hex(data, 4),
                sprint_ascii(data, 4));
  PrintAndLogEx(INFO, "    deskey1 [45/0x2D]: %s [%s]", sprint_hex(data + 4, 4),
                sprint_ascii(data + 4, 4));
  PrintAndLogEx(INFO, "    deskey2 [46/0x2E]: %s [%s]", sprint_hex(data + 8, 4),
                sprint_ascii(data + 8, 4));
  PrintAndLogEx(INFO, "    deskey2 [47/0x2F]: %s [%s]",
                sprint_hex(data + 12, 4), sprint_ascii(data + 12, 4));
  PrintAndLogEx(INFO, "3des key: " _GREEN_("%s"),
                sprint_hex_inrow(SwapEndian64(data, 16, 8), 16));
  return PM3_SUCCESS;
}
...
static int ulc_print_configuration(uint8_t *data) {
  ...
  PrintAndLogEx(NORMAL, " Higher Lockbits [40/0x28]: %s - %s",
                sprint_hex(data, 4), sprint_bin(data, 2));
  PrintAndLogEx(NORMAL, "         Counter [41/0x29]: %s - %s",
                sprint_hex(data + 4, 4), sprint_bin(data + 4, 2));

  bool validAuth = (data[8] >= 0x03 && data[8] <= 0x30);
  if (validAuth)
    PrintAndLogEx(NORMAL,
                  "           Auth0 [42/0x2A]: %s page %d/0x%02X and above "
                  "need authentication",
                  sprint_hex(data + 8, 4), data[8], data[8]);
  else {
    if (data[8] == 0) {
      PrintAndLogEx(NORMAL, "           Auth0 [42/0x2A]: %s default",
                    sprint_hex(data + 8, 4));
    } else {
      PrintAndLogEx(NORMAL,
                    "           Auth0 [42/0x2A]: %s auth byte is out-of-range",
                    sprint_hex(data + 8, 4));
    }
  }
  PrintAndLogEx(NORMAL, "           Auth1 [43/0x2B]: %s %s",
                sprint_hex(data + 12, 4),
                (data[12] & 1) ? "write access restricted"
                               : "read and write access restricted");
  return PM3_SUCCESS;
}

static int ulev1_print_configuration(uint32_t tagtype, uint8_t *data,
                                     uint8_t startPage) {
  ...
  PrintAndLogEx(INFO, "  cfg0 [%u/0x%02X]: %s", startPage, startPage,
                sprint_hex(data, 4));
  ...
  if ((tagtype & (NTAG_213_F | NTAG_213_TT | NTAG_216_F))) {
    ...
    PrintAndLogEx(INFO, "                    - SLEEP mode %s",
                  (sleep_en) ? "enabled" : "disabled");
    ...
    case 0:
      ...
      break;
    case 1:
      PrintAndLogEx(INFO, "                    - enabled by first "
                          "State-of-Frame (start of communication)");
      break;
    case 2:
      PrintAndLogEx(INFO,
    ...
    if ((tagtype & NTAG_213_F) || (tagtype & NTAG_213_TT)) {
      ...
      case 1: {
        PrintAndLogEx(
            INFO, "         mirror start block %02X | byte pos %02X - %s",
            data[2], mirror_byte,
            (data[2] >= 0x4 && data[2] <= 0x24) ? "OK" : "Invalid value");
        break;
      }
      case 2: {
        PrintAndLogEx(
            INFO, "         mirror start block %02X | byte pos %02X - %s",
            data[2], mirror_byte,
            (data[2] >= 0x4 && data[2] <= 0x26) ? "OK" : "Invalid value");
        break;
      }
      case 3: {
        PrintAndLogEx(
            INFO, "         mirror start block %02X | byte pos %02X - %s",
            data[2], mirror_byte,
            (data[2] >= 0x4 && data[2] <= 0x22) ? "OK" : "Invalid value");
        break;
      }
      default:
        break;
      ...
      case 1: {
        PrintAndLogEx(
            INFO, "         mirror start block %02X | byte pos %02X - %s",
            data[2], mirror_byte,
            (data[2] >= 0x4 && data[2] <= 0xDE) ? "OK" : "Invalid value");
        break;
      }
      case 2: {
        PrintAndLogEx(
            INFO, "         mirror start block %02X | byte pos %02X - %s",
            data[2], mirror_byte,
            (data[2] >= 0x4 && data[2] <= 0xE0) ? "OK" : "Invalid value");
        break;
      }
      case 3: {
        PrintAndLogEx(
            INFO, "         mirror start block %02X | byte pos %02X - %s",
            data[2], mirror_byte,
            (data[2] >= 0x4 && data[2] <= 0xDC) ? "OK" : "Invalid value");
        break;
      }
      default:
        break;
  ...
  PrintAndLogEx(INFO, "                    - strong modulation mode %s",
                (strg_mod_en) ? "enabled" : "disabled");

  if (data[3] < 0xff)
    PrintAndLogEx(INFO,
                  "                    - page %d and above need authentication",
                  data[3]);
  else
    PrintAndLogEx(INFO,
                  "                    - pages don't need authentication");

  PrintAndLogEx(INFO, "  cfg1 [%u/0x%02X]: %s", startPage + 1, startPage + 1,
                sprint_hex(data + 4, 4));
  if (authlim == 0)
    PrintAndLogEx(
        INFO, "                    - " _GREEN_("Unlimited password attempts"));
  else
    PrintAndLogEx(
        INFO,
        "                    - Max number of password attempts is " _YELLOW_(
            "%d"),
        authlim);

  PrintAndLogEx(INFO, "                    - NFC counter %s",
                (nfc_cnf_en) ? "enabled" : "disabled");
  PrintAndLogEx(INFO, "                    - NFC counter %s",
                (nfc_cnf_prot_pwd) ? "password protection enabled"
                                   : "not protected");

  PrintAndLogEx(INFO, "                    - user configuration %s",
                cfglck ? "permanently locked" : "writeable");
  PrintAndLogEx(INFO,
                "                    - %s access is protected with password",
                prot ? "read and write" : "write");
  PrintAndLogEx(
      INFO,
      "                    - %02X, Virtual Card Type Identifier is %sdefault",
      vctid, (vctid == 0x05) ? "" : "not ");
  PrintAndLogEx(INFO, "  PWD  [%u/0x%02X]: %s- (cannot be read)", startPage + 2,
                startPage + 2, sprint_hex(data + 8, 4));
  PrintAndLogEx(INFO, "  PACK [%u/0x%02X]: %s      - (cannot be read)",
                startPage + 3, startPage + 3, sprint_hex(data + 12, 2));
  PrintAndLogEx(INFO, "  RFU  [%u/0x%02X]:       %s- (cannot be read)",
                startPage + 3, startPage + 3, sprint_hex(data + 14, 2));
  return PM3_SUCCESS;
}
...
static int ulev1_print_counters(void) {
  ...
  for (uint8_t i = 0; i < 3; ++i) {
    ...
    if (len == 3) {
      ...
      PrintAndLogEx(SUCCESS, "            - %02X tearing ( %s )", tear[0],
...
static int ulev1_print_signature(TagTypeUL_t tagtype, uint8_t *uid,
                                 uint8_t *signature, size_t signature_len) {

#define PUBLIC_ECDA_KEYLEN 33
  // known public keys for the originality check (source:
  // https://github.com/alexbatalov/node-nxp-originality-verifier) ref: AN11350
  // NTAG 21x Originality Signature Validation ref: AN11341 MIFARE Ultralight
  // EV1 Originality Signature Validation
  const ecdsa_publickey_t nxp_mfu_public_keys[] = {
      {"NXP Mifare Classic MFC1C14_x",
       "044F6D3F294DEA5737F0F46FFEE88A356EED95695DD7E0C27A591E6F6F65962BAF"},
      {"Manufacturer Mifare Classic MFC1C14_x",
       "046F70AC557F5461CE5052C8E4A7838C11C7A236797E8A0730A101837C004039C2"},
      {"NXP ICODE DNA, ICODE SLIX2",
  ...
  */
  ...
  for (i = 0; i < ARRAYLEN(nxp_mfu_public_keys); i++) {
    ...
    param_gethex_to_eol(nxp_mfu_public_keys[i].value, 0, key,
                        PUBLIC_ECDA_KEYLEN, &dl);

    int res = ecdsa_signature_r_s_verify(MBEDTLS_ECP_DP_SECP128R1, key, uid, 7,
  ...
  if (is_valid == false || i == ARRAYLEN(nxp_mfu_public_keys)) {
    ...
    PrintAndLogEx(INFO, "             TAG IC Signature: %s",
                  sprint_hex_inrow(signature, signature_len));
    PrintAndLogEx(SUCCESS,
                  "       Signature verification ( " _RED_("fail") " )");
    return PM3_ESOFT;
  }

  PrintAndLogEx(INFO, " IC signature public key name: %s",
                nxp_mfu_public_keys[i].desc);
  PrintAndLogEx(INFO, "IC signature public key value: %s",
                nxp_mfu_public_keys[i].value);
  PrintAndLogEx(INFO, "    Elliptic curve parameters: NID_secp128r1");
  PrintAndLogEx(INFO, "             TAG IC Signature: %s",
                sprint_hex_inrow(signature, signature_len));
  PrintAndLogEx(SUCCESS,
...
static int ulev1_print_version(uint8_t *data) {
  ...
  PrintAndLogEx(INFO, "       Vendor ID: %02X, %s", data[1],
                getTagInfo(data[1]));
  PrintAndLogEx(INFO, "    Product type: %s", getProductTypeStr(data[2]));
  PrintAndLogEx(INFO, " Product subtype: %02X, %s", data[3],
                (data[3] == 1) ? "17 pF" : "50pF");
  ...
  PrintAndLogEx(INFO, "   Protocol type: %02X%s", data[7],
                (data[7] == 0x3) ? ", ISO14443-3 Compliant" : "");
  return PM3_SUCCESS;
}
...
static int ul_magic_test(void) {
  ...
  // 2) make a wrong length write to page0, and see if tag answers with
  // ACK/NACK:
...
uint32_t GetHF14AMfU_Type(void) {
  ...
  if (card.atqa[1] != 0x00 || card.atqa[0] != 0x44 || card.sak != 0x00) {
    // PrintAndLogEx(NORMAL, "Tag is not Ultralight | NTAG | MY-D  [ATQA: %02X
  ...
  if (card.uid[0] != 0x05) {
    ...
    switch (nib) {
    // case 0: tagtype =  SLE66R35E7; break; //or SLE 66R35E7 - mifare compat...
    // should have different sak/atqa for mf 1k
    case 1:
      tagtype = MY_D;
      break; // or SLE 66RxxS ... up to 512 pages of 8 user bytes...
    case 2:
      tagtype = (MY_D_NFC);
      break; // or SLE 66RxxP ... up to 512 pages of 8 user bytes... (or in nfc
             // mode FF pages of 4 bytes)
    case 3:
      tagtype = (MY_D_MOVE | MY_D_MOVE_NFC);
      break; // or SLE 66R01P // 38 pages of 4 bytes //notice: we can not
...
static int CmdHF14AMfUInfo(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hf mfu info",
                ...
                "Sometimes the tags are locked down, and you may need a key to "
  ...
  void *argtable[] = {
      arg_param_begin,
      arg_str0("k", "key", "<hex>",
               "key for authentication (UL-C 16 bytes, EV1/NTAG 4 bytes)"),
      arg_lit0("l", NULL, "swap entered key's endianness"), arg_param_end};
  ...
  if (swap_endian && has_auth_key) {
    authkeyptr =
        SwapEndian64(authenticationkey, ak_len, (ak_len == 16) ? 8 : 4);
  }

  if (ul_auth_select(&card, tagtype, has_auth_key, authkeyptr, pack,
                     sizeof(pack)) == PM3_ESOFT) {
    return PM3_ESOFT;
  }
  ...
  if ((tagtype & UL_C)) {
    ...
    if ((tagtype & MAGIC)) {
      ...
      // also try to diversify default keys..  look into
      // CmdHF14AMfGenDiverseKeys
  ...
  // NTAG counters?
  if ((tagtype & (NTAG_213 | NTAG_213_F | NTAG_213_C | NTAG_213_TT | NTAG_215 |
                  NTAG_216))) {
  ...
  // Read signature
  if ((tagtype &
       (UL_EV1_48 | UL_EV1_128 | UL_EV1 | UL_NANO_40 | NTAG_213 | NTAG_213_F |
        NTAG_213_C | NTAG_213_TT | NTAG_215 | NTAG_216 | NTAG_216_F |
    ...
    if (status == 32) {
      ulev1_print_signature(tagtype, card.uid, ulev1_signature,
    ...
    if (!authlim && !has_auth_key) {
      ...
      if (len > -1) {
        PrintAndLogEx(SUCCESS,
                      "Found default password " _GREEN_("%s") " pack %02X %02X",
                      sprint_hex(key, 4), pack[0], pack[1]);
        goto out;
      }
      ...
      if (len < 1) {
        ...
        PrintAndLogEx(HINT, "Hint: try " _YELLOW_(
                                "`hf mfu pwdgen -r`") " to get see known pwd "
                                                      "gen algo suggestions");
      }
    } else {
      PrintAndLogEx(HINT, "Hint: try " _YELLOW_(
                              "`hf mfu pwdgen -r`") " to get see known pwd gen "
                                                    "algo suggestions");
    }
  }
out:
  ...
  if (locked) {
    ...
    PrintAndLogEx(
        HINT,
        "Hint: try " _YELLOW_(
            "`hf mfu pwdgen -r`") " to get see known pwd gen algo suggestions");
  }
  PrintAndLogEx(NORMAL, "");
  return PM3_SUCCESS;
}
...
static int CmdHF14AMfUWrBl(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "hf mfu wrbl", "Write a block. It autodetects card type.",
  ...
  void *argtable[] = {
      ...
      arg_str1("d", "data", "<hex>",
               "block data (4 or 16 hex bytes, 16 hex bytes will do a "
               "compatibility write)"),
      arg_param_end};
  ...
  if (datalen != 16 && datalen != 4) {
    PrintAndLogEx(WARNING, "Wrong data length. Expect 16 or 4, got %d",
                  datalen);
    return PM3_EINVARG;
  }
  ...
  if (blockno <= 3)
    PrintAndLogEx(INFO, "Special block: %0d (0x%02X) [ %s]", blockno, blockno,
                  sprint_hex(data, datalen));
  else
    PrintAndLogEx(INFO, "Block: %0d (0x%02X) [ %s]", blockno, blockno,
                  sprint_hex(data, datalen));

  if (ak_len) {
    PrintAndLogEx(INFO, "Using %s " _GREEN_("%s"),
                  (ak_len == 16) ? "3des" : "pwd",
                  sprint_hex(authenticationkey, ak_len));
  }
  ...
  clearCommandBuffer();
  if (datalen == 16) {
    SendCommandMIX(CMD_HF_MIFAREU_WRITEBL_COMPAT, blockno, keytype, 0, cmddata,
                   cmdlen);
  } else {
    SendCommandMIX(CMD_HF_MIFAREU_WRITEBL, blockno, keytype, 0, cmddata,
                   cmdlen);
  }
  PacketResponseNG resp;
  if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
...
static int CmdHF14AMfURdBl(const char *Cmd) {
  ...
  void *argtable[] = {
      ...
      arg_str0("k", "key", "<hex>",
               "key for authentication (UL-C 16 bytes, EV1/NTAG 4 bytes)"),
      arg_lit0("l", NULL, "swap entered key's endianness"),
      arg_int1("b", "block", "<dec>", "block number to write"), arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, false);
  ...
  clearCommandBuffer();
  SendCommandMIX(CMD_HF_MIFAREU_READBL, blockno, keytype, 0, authKeyPtr,
                 datalen);
  PacketResponseNG resp;
  if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
    uint8_t isOK = resp.oldarg[0] & 0xff;
    if (isOK) {
      ...
      PrintAndLogEx(INFO, "%02d/0x%02X | %s| %s\n", blockno, blockno,
                    sprint_hex(d, 4), sprint_ascii(d, 4));
    } else {
...
void printMFUdumpEx(mfu_dump_t *card, uint16_t pages, uint8_t startpage) {
  ...
  PrintAndLogEx(
      INFO, "-------------------------------------------------------------");
  PrintAndLogEx(INFO, "      Version | " _YELLOW_("%s"),
                sprint_hex(card->version, sizeof(card->version)));
  PrintAndLogEx(INFO, "        TBD 0 | %s",
                sprint_hex(card->tbo, sizeof(card->tbo)));
  PrintAndLogEx(INFO, "        TBD 1 | %s",
                sprint_hex(card->tbo1, sizeof(card->tbo1)));
  PrintAndLogEx(INFO, "    Signature | %s",
                sprint_hex(card->signature, sizeof(card->signature)));
  for (uint8_t i = 0; i < 3; i++) {
    PrintAndLogEx(INFO, "    Counter %d | %s", i,
                  sprint_hex(card->counter_tearing[i], 3));
    PrintAndLogEx(INFO, "    Tearing %d | %s", i,
                  sprint_hex(card->counter_tearing[i] + 3, 1));
  }

  PrintAndLogEx(INFO,
                "Max data page | " _YELLOW_("%d") " (" _YELLOW_("%d") " bytes)",
                card->pages - 1, card->pages * 4);
  ...
  for (uint8_t i = 0; i < pages; ++i) {
    if (i < 3) {
      PrintAndLogEx(INFO, "%3d/0x%02X | %s|   | %s", i + startpage,
                    i + startpage, sprint_hex(data + i * 4, 4),
    ...
    PrintAndLogEx(INFO, "%3d/0x%02X | %s| %s | %s", i + startpage,
                  i + startpage, sprint_hex(data + i * 4, 4),
                  (lckbit) ? _RED_("1") : "0", sprint_ascii(data + i * 4, 4));
  }
  PrintAndLogEx(INFO, "---------------------------------");
}
...
static int CmdHF14AMfUDump(const char *Cmd) {
  ...
  CLIParserInit(
      ...
      "hf mfu dump -p 10            -> start at page 10 and dump rest of "
      "blocks\n"
  ...
  void *argtable[] = {
      ...
      arg_lit0("l", NULL, "swap entered key's endianness"),
      arg_int0("p", "page", "<dec>",
               "manually set start page number to start from"),
  ...
  clearCommandBuffer();
  SendCommandMIX(CMD_HF_MIFAREU_READCARD, start_page, pages, keytype,
                 authKeyPtr, ak_len);
  ...
  if (!GetFromDevice(BIG_BUF, data, buffer_size, startindex, NULL, 0, NULL,
                     2500, false)) {
  ...
  if (!(tagtype & UL_C || tagtype & UL || tagtype & MY_D_MOVE ||
        tagtype & MY_D_MOVE_LEAN)) {
    // attempt to read pack
    uint8_t get_pack[] = {0, 0};
    if (ul_auth_select(&card, tagtype, true, authKeyPtr, get_pack,
                       sizeof(get_pack)) != PM3_SUCCESS) {
      // reset pack
      get_pack[0] = 0;
      get_pack[1] = 0;
    }
    ...
    if (has_auth_key) {
      ...
      ul_auth_select(&card, tagtype, has_auth_key, authKeyPtr, dummy_pack,
                     sizeof(dummy_pack));
    } else {
      ul_select(&card);
    }
    ...
    if ((tagtype & (NTAG_213 | NTAG_213_F | NTAG_213_C | NTAG_213_TT |
                    NTAG_215 | NTAG_216))) {
      n = 2;
    }
  ...
  if (!is_partial && pages == card_mem_size && has_auth_key) {
    ...
    // NOTE: default entry is bigendian (unless swapped), sprint_hex outputs
    // little endian
  ...
  memcpy(dump_file_data.signature, get_signature,
         sizeof(dump_file_data.signature));
  memcpy(dump_file_data.counter_tearing, get_counter_tearing,
         sizeof(dump_file_data.counter_tearing));
  ...
  if (is_partial)
    PrintAndLogEx(WARNING, "Partial dump created. (%d of %d blocks)", pages,
                  card_mem_size);
...
static int CmdHF14AMfURestore(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mfu restore", "Restore dumpfile onto card.",
                "hf mfu restore -f myfile -s                 -> user specified "
                "filename and special write\n"
                "hf mfu restore -f myfile -k AABBCCDD -s     -> user specified "
                "filename, special write and use key\n"
                "hf mfu restore -f myfile -k AABBCCDD -ser   -> user specified "
                "filename, special write, use key, ...");
  ...
  void *argtable[] = {
      ...
      arg_lit0("r", NULL,
               "use the password found in dumpfile to configure tag. "
               "requires " _YELLOW_("'-e'") " parameter to work"),
...
#define MFU_NTAG_SPECIAL_SIGNATURE 0xF2
    ...
    if (has_key || read_key) {
      ...
      clearCommandBuffer();
      SendCommandMIX(CMD_HF_MIFAREU_WRITEBL, MFU_NTAG_SPECIAL_PWD, keytype, 0,
                     data, sizeof(data));
    ...
    clearCommandBuffer();
    SendCommandMIX(CMD_HF_MIFAREU_WRITEBL, MFU_NTAG_SPECIAL_PACK, keytype, 0,
                   data, sizeof(data));
    wait4response(MFU_NTAG_SPECIAL_PACK);

    // Signature
    for (uint8_t s = MFU_NTAG_SPECIAL_SIGNATURE, i = 0;
         s < MFU_NTAG_SPECIAL_SIGNATURE + 8; s++, i += 4) {
    ...
    // Version
    for (uint8_t s = MFU_NTAG_SPECIAL_VERSION, i = 0;
         s < MFU_NTAG_SPECIAL_VERSION + 2; s++, i += 4) {
...
static int CmdHF14AMfUeLoad(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "hf mfu eload", "It loads emul dump from the file `filename.eml`",
  ...
  PrintAndLogEx(
      HINT,
      "Hint: See " _YELLOW_(
          "`script run data_mfu_bin2eml`") " to convert the .bin to .eml");
...
static int CmdHF14AMfUSim(const char *Cmd) {
  ...
  CLIParserInit(
      ...
      "hf mfu sim -t 7 --uid 1122344556677 -n 5   -> AMIIBO (NTAG 215),  pack "
      "0x8080");
  ...
  void *argtable[] = {
      ...
      arg_int0("n", "num", "<dec>",
               "Exit simulation after <numreads> blocks have been read by "
               "reader. 0 = infinite"),
...
static int CmdHF14AMfUCAuth(const char *Cmd) {
  ...
  if (isok)
    PrintAndLogEx(SUCCESS, "Authentication success. 3des key: " _GREEN_("%s"),
                  sprint_hex_inrow(authKeyPtr, 16));
  else
...
static int CmdHF14AMfUCSetPwd(const char *Cmd) {
  ...
  if (WaitForResponseTimeout(CMD_ACK, &resp, 1500)) {
    if ((resp.oldarg[0] & 0xff) == 1) {
      PrintAndLogEx(INFO, "Ultralight-C new key: %s",
                    sprint_hex(key, sizeof(key)));
    } else {
      PrintAndLogEx(WARNING, "Failed writing at block %u",
...
static int CmdHF14AMfUCSetUid(const char *Cmd) {
  ...
  if (oldconfig_bcc != 2) {
    config.forcebcc = 2;
    SendCommandNG(CMD_HF_ISO14443A_SET_CONFIG, (uint8_t *)&config,
                  sizeof(hf14a_config));
  }
...
static int CmdHF14AMfUGenDiverseKeys(const char *Cmd) {
  ...
  void *argtable[] = {arg_param_begin,
                      ...
                      arg_lit0("r", NULL, "read UID from tag"), arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
  ...
  if (read_tag) {
    // read uid from tag
    clearCommandBuffer();
    SendCommandMIX(CMD_HF_ISO14443A_READER, ISO14A_CONNECT | ISO14A_NO_RATS, 0,
                   0, NULL, 0);
    PacketResponseNG resp;
    WaitForResponse(CMD_ACK, &resp);
    iso14a_card_select_t card;
    ...
    if (card.uidlen != 4 && card.uidlen != 7) {
      PrintAndLogEx(WARNING, "Wrong sized UID, expected 4|7 bytes got %d",
                    card.uidlen);
      return PM3_ESOFT;
    }
    ulen = card.uidlen;
  ...
  uint8_t masterkey[] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77,
  ...
  PrintAndLogEx(SUCCESS, "Masterkey......... %s",
                sprint_hex(masterkey, sizeof(masterkey)));
  PrintAndLogEx(SUCCESS, "UID............... %s", sprint_hex(uid, ulen));
  PrintAndLogEx(SUCCESS, "block............. %0d", block);
  PrintAndLogEx(SUCCESS, "Mifare key........ %s",
                sprint_hex(mifarekeyA, sizeof(mifarekeyA)));
  ...
  PrintAndLogEx(SUCCESS, "\n-- DES version");
  PrintAndLogEx(SUCCESS, "MIFARE dkeyA...... %s",
                sprint_hex(dkeyA, sizeof(dkeyA)));
  PrintAndLogEx(SUCCESS, "MIFARE dkeyB...... %s",
                sprint_hex(dkeyB, sizeof(dkeyB)));
  PrintAndLogEx(SUCCESS, "MIFARE ABA........ %s",
                sprint_hex(dmkey, sizeof(dmkey)));
  PrintAndLogEx(SUCCESS, "MIFARE PWD........ %s",
                sprint_hex(newpwd, sizeof(newpwd)));

  mbedtls_des3_free(&ctx_des3);

  mbedtls_aes_context ctx_aes;
  uint8_t aes_iv[16] = {0x00};
  uint8_t aes_masterkey[] = {0x00, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
                             0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F};
  uint8_t aes_input[16] = {0x01, 0x04, 0x2A, 0x2E, 0x19, 0x70, 0x1C, 0x80,
                           0x01, 0x04, 0x2A, 0x2E, 0x19, 0x70, 0x1C, 0x80};
  uint8_t aes_output[16] = {0x00};
  mbedtls_aes_setkey_enc(&ctx_aes, aes_masterkey, 128);
  mbedtls_aes_crypt_cbc(&ctx_aes, MBEDTLS_AES_ENCRYPT, 16, aes_iv, aes_input,
                        aes_output);
  mbedtls_aes_free(&ctx_aes);

  PrintAndLogEx(SUCCESS, "\n-- AES version");
  PrintAndLogEx(SUCCESS, "MIFARE AES mk..... %s",
                sprint_hex(aes_masterkey, sizeof(aes_masterkey)));
  PrintAndLogEx(SUCCESS, "MIFARE Div........ %s",
                sprint_hex(aes_output, sizeof(aes_output)));
...
static int CmdHF14AMfUPwdGen(const char *Cmd) {
  ...
  void *argtable[] = {arg_param_begin,
                      ...
                      arg_lit0("t", NULL, "selftest"), arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
  ...
  PrintAndLogEx(INFO, "------+----------+-----");
  PrintAndLogEx(INFO, " EV1  | %08X | %04X", ul_ev1_pwdgenA(uid),
                ul_ev1_packgenA(uid));
  PrintAndLogEx(INFO, " Ami  | %08X | %04X", ul_ev1_pwdgenB(uid),
                ul_ev1_packgenB(uid));
  PrintAndLogEx(INFO, " LD   | %08X | %04X", ul_ev1_pwdgenC(uid),
                ul_ev1_packgenC(uid));
  PrintAndLogEx(INFO, " XYZ  | %08X | %04X", ul_ev1_pwdgenD(uid),
                ul_ev1_packgenD(uid));
  PrintAndLogEx(INFO, "------+----------+-----");
...
static int CmdHF14AMfuOtpTearoff(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "hf mfu otptear", "Tear-off test against OTP block",
      "hf mfu otptear -b 3\n"
      "hf mfu otptear -b 3 -i 100 -s 1000\n"
      "hf mfu otptear -b 3 -i 1 -e 200\n"
      "hf mfu otptear -b 3 -i 100 -s 200 -e 2500 -d FFFFFFFF -t EEEEEEEE\n"
      "hf mfu otptear -b 3 -i 100 -s 200 -e 2500 -d FFFFFFFF -t EEEEEEEE -m "
      "00000000    -> quite when OTP is reset");
  ...
  void *argtable[] = {
      ...
      arg_str0("m", "match", "<hex>",
               "exit criteria, if block matches this value (4 bytes)"),
      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
  ...
  PrintAndLogEx(INFO, "Target inital block data : %s",
                sprint_hex_inrow(teardata, 4));
  PrintAndLogEx(INFO, "Target write block data  : %s",
                sprint_hex_inrow(teardata + 4, 4));
  PrintAndLogEx(INFO, "----------------------------------------------------");
  uint8_t isOK;
  ...
  while (current <= (end - steps)) {
    ...
    PrintAndLogEx(INFO, "Using tear-off delay " _GREEN_("%" PRIu32) " us",
                  current);
    ...
    if (got_pre && got_post) {
      ...
      char prestr[20] = {0};
      snprintf(prestr, sizeof(prestr), "%s",
               sprint_hex_inrow(pre, sizeof(pre)));
      char poststr[20] = {0};
      snprintf(poststr, sizeof(poststr), "%s",
               sprint_hex_inrow(post, sizeof(post)));

      if (memcmp(pre, post, sizeof(pre)) == 0) {

        PrintAndLogEx(INFO, "Current %02d (0x%02X) %s", blockno, blockno,
                      poststr);
      } else {
        ...
        if (current == start) {
          ...
          PrintAndLogEx(
              INFO,
              _CYAN_("Tear off occured") " : %02d (0x%02X) %s vs " _RED_("%s"),
              blockno, blockno, prestr, poststr);
          ...
          if (phase_clear > -1 && phase_newwr == -1 &&
              current > (phase_clear + 100))
            phase_newwr = current;
        }
      }
    ...
    if (start != end) {
      ...
      if (lock_on == false) {
        if (++retries == 20) {
          ...
          retries = 0;
          PrintAndLogEx(INFO,
                        _CYAN_("Retried %u times, increased delay with 1us"),
                        retries);
        }
      }
    }
  ...
  PrintAndLogEx(INFO, "----------------------------------------------------");
  if (phase_clear > -1) {
    PrintAndLogEx(INFO, "New phase boundary around " _YELLOW_("%d") " us",
                  phase_clear);
  }
  if (phase_newwr > -1) {
    PrintAndLogEx(INFO, "New phase boundary around " _YELLOW_("%d") " us",
...
static int CmdHF14AMfuEv1CounterTearoff(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf mfu countertear",
                  ...
                  "hf mfu countertear\n"
                  "hf mfu countertear -s 200 -l 2500      -> target counter 0,
start delay 200\n" "hf mfu countertear -i 2 -s 200 -l 400  -> target counter 0,
start delay 200\n"
                 );
    ...
    void *argtable[] = {
        ...
        arg_int0("i", "inc", "<dec>", "time interval to increase in each
iteration - default 10 us"), arg_int0("l", "limit", "<dec>", "test upper limit
time - default 3000 us"), arg_int0("s", "start", "<dec>", "test start time -
default 0 us"), arg_int0(NULL, "fix", "<dec>", "test fixed loop delay"),
    ...
    uint32_t inital_value = ( inital_cnt[0] | inital_cnt[1] << 8 | inital_cnt[2]
<< 16 );;

    PrintAndLogEx(INFO, "----------------- " _CYAN_("MFU Ev1 Counter Tear off")
" ---------------------"); PrintAndLogEx(INFO, "Target counter no     [ "
_GREEN_("%u") " ]", counter); PrintAndLogEx(INFO, "       counter value  [ "
_GREEN_("%s") " ]", sprint_hex_inrow(inital_cnt, sizeof(inital_cnt)));
    PrintAndLogEx(INFO, "     anti-tear value  [ " _GREEN_("%02X") " ]",
inital_tear[0]); PrintAndLogEx(INFO, "       increase value [ " _GREEN_("%s") "
]", sprint_hex_inrow(newvalue, newvaluelen)); PrintAndLogEx(INFO,
"----------------------------------------------------");
    ...
    while (actual_time <= (time_limit - interval)) {
        ...
        PrintAndLogEx(INPLACE, "Using tear-off delay " _GREEN_("%" PRIu32) " us
(attempt %u)", actual_time, loop);
        ...
        if ( tlen == sizeof(tear) ) {
            ...
            PrintAndLogEx(NORMAL, "");
            PrintAndLogEx(FAILED, "BEFORE, failed to read ANTITEAR,  exiting...
%d", tlen); break;
        }
        ...
        clearCommandBuffer();
        PacketResponseNG resp;
        SendCommandNG(CMD_HF_MFU_COUNTER_TEAROFF, (uint8_t*)&payload,
sizeof(payload)); if (!WaitForResponseTimeout(CMD_HF_MFU_COUNTER_TEAROFF, &resp,
2000)) { PrintAndLogEx(WARNING, "\ntear off command failed"); continue;
        }
        ...
        char prestr[20] = {0};
        snprintf(prestr, sizeof(prestr), "%s", sprint_hex_inrow(pre,
sizeof(pre))); char poststr[20] = {0}; snprintf(poststr, sizeof(poststr), "%s",
sprint_hex_inrow(post, sizeof(post)));
        ...
        if (memcmp(pre, post, sizeof(pre)) != 0) {
            ...
            if (inital_value != a ) {
                ...
                if ( inital_value != b )
                    PrintAndLogEx(INFO, "pre %08x, post %08x != inital %08x  |
tear:  0x%02X  == 0x%02X", a, b, inital_value, pre_tear, post_tear); else
                    PrintAndLogEx(INFO, "pre %08x != inital and post %08x ==
inital %08x |  tear:  0x%02X  == 0x%02X", a, b, inital_value, pre_tear,
post_tear); } else {
...
post_tear);
            ...
            if ( b == 0 ) {
                PrintAndLogEx(INFO, _CYAN_("Tear off occured  (ZEROS value!) ->
") "%s vs " _GREEN_("%s") "  Tear status:  0x%02X == 0x%02X   ( %s )" , prestr
                    , poststr
                    , pre_tear
                    , post_tear
            ...
            if ( a > b ) {
                PrintAndLogEx(INFO, _CYAN_("Tear off occured  " _RED_("( LESS
)") " ->  ") "%s vs " _GREEN_("%s") "  Tear status:  0x%02X == 0x%02X   ( %s )"
                    , prestr
                    , poststr
                    , pre_tear
                    , post_tear
                ...
                PrintAndLogEx(INFO, "New increase value " _YELLOW_("%s"),
sprint_hex_inrow(newvalue, newvaluelen)); continue; } else  {

                PrintAndLogEx(NORMAL, "");
                PrintAndLogEx(INFO, _CYAN_("Tear off occured  (+1)  (too late)
->  ") "%s vs %s   Tear:  0x%02X == 0x%02X   ( %s )" , prestr , poststr ,
pre_tear , post_tear , post_tear_check ? _GREEN_("OK") : _RED_("DETECTED")
                );
                ...
                if ( wr_value != 0 ) {
                    ...
                    if ( b >= (inital_value + (2 * wr_value))) {
                        PrintAndLogEx(INFO, "Large " _YELLOW_("( JUMP )") "
detected");
                ...
                PrintAndLogEx(INFO, "New increase value " _YELLOW_("%s"),
sprint_hex_inrow(newvalue, newvaluelen));
            ...
            PrintAndLogEx(NORMAL, "");
            PrintAndLogEx(INFO, _CYAN_("Status:  same value!   ->  ") "%s == %s
Tear:  0x%02X == 0x%02X   ( %s )" , prestr , poststr , pre_tear , post_tear ,
post_tear_check ? _GREEN_("OK") : _RED_("DETECTED")
            );
    ...
    PrintAndLogEx(INFO, "hf 14a raw -s -c 3900              -->  read counter
0"); PrintAndLogEx(INFO, "hf 14a raw -s -c 3e00              -->  read tearing
0"); PrintAndLogEx(NORMAL, ""); char read_cnt_str[30]; sprintf(read_cnt_str, "hf
14a raw -s -c 39%02x", counter); CommandReceived(read_cnt_str); char
read_tear_str[30]; sprintf(read_tear_str, "hf 14a raw -s -c 3e%02x", counter);
...
static int CmdHF14MfuNDEF(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mfu ndef", "Prints NFC Data Exchange Format (NDEF)",
  ...
  if (swapEndian && hasAuthKey)
    p_key = SwapEndian64(key, keylen, (keylen == 16) ? 8 : 4);

  // Select and Auth
  if (ul_auth_select(&card, tagtype, hasAuthKey, p_key, pack, sizeof(pack)) ==
      PM3_ESOFT)
    return PM3_ESOFT;
  ...
  for (uint8_t i = 0; i < ARRAYLEN(UL_TYPES_ARRAY); i++) {
    if (tagtype & UL_TYPES_ARRAY[i]) {
      ...
      if (maxsize != (UL_MEMORY_ARRAY[i] * 4)) {
        PrintAndLogEx(INFO, "Tag reported size vs NDEF reported size mismatch. "
  ...
  if (jooki) {
    ...
    while (jooki) {
      if ((*jooki) != '=')
        ...
        strncpy(s, jooki, 16);
        PrintAndLogEx(
            HINT, "Use `" _YELLOW_("hf jooki decode -d %s") "` to decode", s);
        break;
      }
    }
  }
...
static int CmdHF14AMfuEView(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hf mfu eview", "It displays emulator memory",
                "hf mfu eview");
...
static command_t CommandTable[] = {
    ...
    {"-----------", CmdHelp, IfPm3Iso14443a,
     "----------------------- " _CYAN_("recovery") " -----------------------"},
    {"keygen", CmdHF14AMfUGenDiverseKeys, AlwaysAvailable,
     "Generate 3DES MIFARE diversified keys"},
    {"pwdgen", CmdHF14AMfUPwdGen, AlwaysAvailable,
     "Generate pwd from known algos"},
    {"otptear", CmdHF14AMfuOtpTearoff, IfPm3Iso14443a,
     "Tear-off test on OTP bits"},
    //    {"tear_cnt", CmdHF14AMfuEv1CounterTearoff,     IfPm3Iso14443a,
    //    "Tear-off test on Ev1/NTAG Counter bits"},
    {"-----------", CmdHelp, IfPm3Iso14443a,
     "----------------------- " _CYAN_(
         "operations") " -----------------------"},
    {"cauth", CmdHF14AMfUCAuth, IfPm3Iso14443a,
     "Authentication - Ultralight-C"},
    {"dump", CmdHF14AMfUDump, IfPm3Iso14443a,
