...
static void lookupChipID(uint32_t iChipID, uint32_t mem_used) {
  ...
  PrintAndLogEx(NORMAL,
                "  --= Nonvolatile program memory: " _YELLOW_(
                    "%uK") " bytes %s ( " _YELLOW_("%2.0f%%") " used )",
                mem_avail, asBuff,
                mem_avail == 0 ? 0.0f
                               : (float)mem_used / (mem_avail * 1024) * 100);
  ...
  PrintAndLogEx(NORMAL, "  --= Second nonvolatile program memory size: %s",
                asBuff);
}
...
static int CmdDetectReader(const char *Cmd) {
  ...
  void *argtable[] = {
      arg_param_begin, arg_lit0("L", "LF", "detect low frequence 125/134 kHz"),
      arg_lit0("H", "HF", "detect high frequence 13.56 MHZ"), arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdFPGAOff(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hw fpgaoff", "Turn of fpga and antenna field",
                "hw fpgaoff\n");
...
static int CmdLCD(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hw lcd", "Send command/data to LCD",
                "hw lcd -r AA -c 03    -> sends 0xAA three times");

  void *argtable[] = {arg_param_begin, arg_int1("r", "raw", "<hex>", "data "),
...
static int CmdLCDReset(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hw lcdreset", "Hardware reset LCD", "hw lcdreset\n");
...
static int CmdReset(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hw reset", "Reset the Proxmark3 device.", "hw reset");
...
/*
 * Sets the divisor for LF frequency clock: lets the user choose any LF
 * frequency below 600kHz.
 */
static int CmdSetDivisor(const char *Cmd) {
  ...
  if (arg < 19) {
    PrintAndLogEx(
        ERR, "Divisor must be between" _YELLOW_("19") " and " _YELLOW_("255"));
    return PM3_EINVARG;
  }
  // 12 000 000 (12MHz)
  clearCommandBuffer();
  SendCommandNG(CMD_LF_SET_DIVISOR, (uint8_t *)&arg, sizeof(arg));
  PrintAndLogEx(SUCCESS, "Divisor set, expected " _YELLOW_("%.1f") " kHz",
                ((double)12000 / (arg + 1)));
  return PM3_SUCCESS;
}
...
static int CmdSetMux(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hw setmux", "Set the ADC mux to a specific value",
...
static int CmdStandalone(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hw standalone", "Start standalone mode",
                "hw standalone       -> start \n"
...
static int CmdTune(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hw tune", "Measure antenna tuning", "hw tune");
...
int handle_tearoff(tearoff_params_t *params, bool verbose) {
  ...
  if (resp.status == PM3_SUCCESS) {
    if (params->delay_us > 0 && verbose)
      PrintAndLogEx(INFO,
                    "Tear-off hook configured with delay of " _GREEN_("%i us"),
                    params->delay_us);
...
static int CmdTearoff(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hw tearoff",
                "Configure a tear-off hook for the next write command "
                "supporting tear-off\n"
                "After having been triggered by a write command, the tear-off "
                "hook is deactivated\n"
                "Delay (in us) must be between 1 and 43000 (43ms). Precision "
                "is about 1/3us.",
                "hw tearoff --delay 1200 --> define delay of 1200us\n"
                "hw tearoff --on --> (re)activate a previously defined delay\n"
                "hw tearoff --off --> deactivate a previously activated but "
                "not yet triggered hook\n");

  void *argtable[] = {arg_param_begin,
                      arg_int0(NULL, "delay", "<dec>",
                               "Delay in us before triggering tear-off, must "
                               "be between 1 and 43000"),
...
static int CmdTia(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hw tia",
                "Trigger a Timing Interval Acquisition to re-adjust the "
                "RealTimeCounter divider",
                "hw tia");
...
static int CmdPing(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "hw ping", "Test if the Proxmark3 is responsive",
                "hw ping\n"
                "hw ping --len 32");
  ...
  if (WaitForResponseTimeout(CMD_PING, &resp, 1000)) {
    if (len) {
      ...
      PrintAndLogEx((error) ? ERR : SUCCESS,
                    "Ping response " _GREEN_("received") " and content is %s",
                    error ? _RED_("NOT ok") : _GREEN_("OK"));
    } else {
...
static int CmdConnect(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "hw connect",
                ...
                "Baudrate here is only for physical UART or UART-BT, NOT for "
                "USB-CDC or blue shark add-on",
                "hw connect -p " SERIAL_PORT_EXAMPLE_H "\n"
                "hw connect -p " SERIAL_PORT_EXAMPLE_H " -b 115200");

  void *argtable[] = {
      arg_param_begin,
      arg_str0("p", "port", NULL,
               "Serial port to connect to, else retry the last used one"),
      arg_u64_0("b", "baud", "<dec>", "Baudrate"), arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
  ...
  if (session.pm3_present &&
      (TestProxmark(session.current_device) != PM3_SUCCESS)) {
    PrintAndLogEx(ERR,
...
static command_t CommandTable[] = {
    ...
    {"status", CmdStatus, IfPm3Present,
     "Show runtime status information about the connected Proxmark3"},
    {"tearoff", CmdTearoff, IfPm3Present,
     "Program a tearoff hook for the next command supporting tearoff"},
    {"tia", CmdTia, IfPm3Present,
     "Trigger a Timing Interval Acquisition to re-adjust the RealTimeCounter "
     "divider"},
    {"tune", CmdTune, IfPm3Present, "Measure antenna tuning"},
    {"version", CmdVersion, IfPm3Present,
     "Show version information about the connected Proxmark3"},
    {NULL, NULL, NULL, NULL}};
...
#define PM3HOSTARCH " ARCH:unknown"
  ...
  if (oneliner) {
    ...
    FormatVersionInformation(temp, sizeof(temp),
  ...
  if (WaitForResponseTimeout(CMD_VERSION, &resp, 1000)) {
    ...
    if (IfPm3Rdv4Fw()) {
      ...
      PrintAndLogEx(NORMAL, "  device.................... %s",
                    (is_genuine_rdv4) ? _GREEN_("RDV4")
                                      : _RED_("device / fw mismatch"));
      PrintAndLogEx(NORMAL, "  firmware.................. %s",
                    _YELLOW_("RDV4"));
      PrintAndLogEx(NORMAL, "  external flash............ %s",
                    IfPm3Flash() ? _GREEN_("present") : _YELLOW_("absent"));
      PrintAndLogEx(NORMAL, "  smartcard reader.......... %s",
                    IfPm3Smartcard() ? _GREEN_("present") : _YELLOW_("absent"));
      PrintAndLogEx(NORMAL, "  FPC USART for BT add-on... %s",
                    IfPm3FpcUsartHost() ? _GREEN_("present")
                                        : _YELLOW_("absent"));
    } else {
      PrintAndLogEx(NORMAL, "  firmware.................. %s",
                    _YELLOW_("PM3 GENERIC"));
    ...
    if (IfPm3FpcUsartDevFromUsb()) {
      PrintAndLogEx(NORMAL, "  FPC USART for developer... %s",
                    _GREEN_("present"));
    }
