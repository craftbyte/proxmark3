...
int lfsim_wait_check(uint32_t cmd) {
  ...
  PrintAndLogEx(
      INFO, "Press " _GREEN_("<Enter>") " or pm3-button to abort simulation");
...
static int CmdLFTune(const char *Cmd) {
  ...
  char q_str[60];
  snprintf(q_str, sizeof(q_str),
           "Frequency divisor. %d -> 134 kHz, %d -> 125 kHz", LF_DIVISOR_134,
           LF_DIVISOR_125);
  void *argtable[] = {arg_param_begin,
                      arg_u64_0("n", "iter", "<dec>",
                                "number of iterations (default: 0=infinite)"),
  ...
  PrintAndLogEx(
      INFO,
      "Measuring LF antenna at " _YELLOW_("%.2f") " kHz, click " _GREEN_(
          "pm3 button") " or press " _GREEN_("Enter") " to exit",
      LF_DIV2FREQ(divisor));
  ...
  if (!WaitForResponseTimeout(CMD_MEASURE_ANTENNA_TUNING_LF, &resp, 1000)) {
    PrintAndLogEx(
        WARNING,
        "Timeout while waiting for Proxmark LF initialization, aborting");
    return PM3_ETIMEOUT;
  }
  ...
  for (uint8_t i = 0; iter == 0 || i < iter; i++) {
    ...
    if (!WaitForResponseTimeout(CMD_MEASURE_ANTENNA_TUNING_LF, &resp, 1000)) {
      ...
      PrintAndLogEx(WARNING,
                    "Timeout while waiting for Proxmark LF measure, aborting");
      break;
    }
  ...
  if (!WaitForResponseTimeout(CMD_MEASURE_ANTENNA_TUNING_LF, &resp, 1000)) {
    PrintAndLogEx(WARNING,
                  "Timeout while waiting for Proxmark LF shutdown, aborting");
    return PM3_ETIMEOUT;
  }
...
int CmdLFCommandRead(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "lf cmdread",
                "Modulate LF reader field to send command before read. All "
                "periods in microseconds.\n"
                " - use " _YELLOW_("`lf config`") _CYAN_(" to set parameters"),
                "lf cmdread -d 50 -z 116 -o 166 -e W3000 -c W00110             "
                "              --> probing for Hitag 1/S\n"
                "lf cmdread -d 50 -z 116 -o 166 -e W3000 -c W11000             "
                "              --> probing for Hitag 2\n"
                "lf cmdread -d 50 -z 116 -o 166 -e W3000 -c W11000 -q -s 2000 "
                "-@             --> probing for Hitag 2, oscilloscope style\n"
                "lf cmdread -d 48 -z 112 -o 176 -e W3000 -e S240 -e E336 -c "
                "W0S00000010000E  --> probing for Hitag (us)\n");

  char div_str[70] = {0};
  sprintf(div_str, "Extra symbol definition and duration (up to %i)",
          LF_CMDREAD_MAX_EXTRA_SYMBOLS);

  void *argtable[] = {
      arg_param_begin,
      arg_u64_0("d", "duration", "<us>",
                "delay OFF period, (0 for bitbang mode)"),
  ...
  for (; i < extra_arg_len;) {
    ...
    if (index_extra < LF_CMDREAD_MAX_EXTRA_SYMBOLS - 1) {
      ...
      PrintAndLogEx(WARNING,
                    "Too many extra symbols, please define up to %i symbols",
                    LF_CMDREAD_MAX_EXTRA_SYMBOLS);
    }
  }
  ...
  if (payload.delay == 0) {
    if (payload.period_0 < 7 || payload.period_1 < 7) {
      PrintAndLogEx(WARNING,
                    "periods cannot be less than 7us in bit bang mode");
      return PM3_EINVARG;
    }
  }
  ...
  PrintAndLogEx(DEBUG, "-------------------");
  PrintAndLogEx(DEBUG, "delay: %u ,  zero %u , one %u , samples %u",
                payload.delay, payload.period_0, payload.period_1,
                payload.samples);
  ...
  for (i = 0; i < LF_CMDREAD_MAX_EXTRA_SYMBOLS; i++) {
    ...
    PrintAndLogEx(DEBUG, "  %c - %u", payload.symbol_extra[i],
                  payload.period_extra[i]);
  }
  ...
  do {
    clearCommandBuffer();
    SendCommandNG(CMD_LF_MOD_THEN_ACQ_RAW_ADC, (uint8_t *)&payload,
                  payload_header_size + cmd_len);

    PacketResponseNG resp;

    i = 10;
    // 20sec wait loop
    while (!WaitForResponseTimeout(CMD_LF_MOD_THEN_ACQ_RAW_ADC, &resp, 2000) &&
           i != 0) {
      if (verbose) {
...
#define LONG_WAIT 100
  ...
  for (bit = 0; bit < 64; bit++) {
    ...
    if (sum > 0 && bits[bit] != 1)
      PrintAndLogEx(WARNING, "oops1 at %d", bit);

    if (sum < 0 && bits[bit] != 0)
      PrintAndLogEx(WARNING, "oops2 at %d", bit);
  }
...
int lf_config(sample_config *config) {
  ...
  clearCommandBuffer();
  if (config != NULL)
    SendCommandNG(CMD_LF_SAMPLING_SET_CONFIG, (uint8_t *)config,
...
int CmdLFConfig(const char *Cmd) {
  ...
  CLIParserInit(
      ...
      " - use " _YELLOW_("`lf read`")
          _CYAN_(" performs a read (active field)\n") _CYAN_(" - use ")
              _YELLOW_("`lf sniff`")
                  _CYAN_(" performs a sniff (no active field)"),
      "lf config                    --> shows current config\n"
      "lf config -b 8 --125         --> samples at 125 kHz, 8 bps\n"
      "lf config -b 4 --134 --dec 3 --> samples at 134 kHz, averages three "
      "samples into one, stored with a resolution of 4 bits per sample\n"
      "lf config --trig 20 -s 10000 --> trigger sampling when above 20, skip "
      "10 000 first samples after triggered\n");

  char div_str[70] = {0};
  sprintf(div_str, "Manually set freq divisor. %d -> 134 kHz, %d -> 125 kHz",
          LF_DIVISOR_134, LF_DIVISOR_125);
  ...
  void *argtable[] = {
      ...
      arg_int0("a", "avg", "<0|1>",
               "averaging - if set, will average the stored sample value when "
               "decimating (default 1)"),
      arg_int0("b", "bps", "<1-8>",
               "sets resolution of bits per sample (default 8)"),
      arg_int0(NULL, "dec", "<1-8>",
               "sets decimation. A value of N saves only 1 in N samples "
               "(default 1)"),
      ...
      arg_int0("s", "skip", "<dec>",
               "sets a number of samples to skip before capture (default 0)"),
      arg_int0("t", "trig", "<0-128>",
               "sets trigger threshold. 0 means no threshold"),
      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
  ...
  sample_config config = {.decimation = -1,
                          .bits_per_sample = -1,
                          .averaging = -1,
                          .divisor = -1,
                          .trigger_threshold = -1,
  ...
  if (reset) {
    ...
    config.averaging = 1, config.divisor = LF_DIVISOR_125;
...
int CmdLFRead(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "lf read",
                ...
                " - use " _YELLOW_("`lf config`")
                    _CYAN_(" to set parameters.\n") _CYAN_(" - use ")
                        _YELLOW_("`data plot`") _CYAN_(" to look at it"),
  ...
  void *argtable[] = {
      ...
      arg_u64_0("s", "samples", "<dec>", "number of samples to collect"),
      arg_lit0("v", "verbose", "verbose output"),
      arg_lit0("@", NULL, "continuous reading mode"), arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
...
int CmdLFSniff(const char *Cmd) {
  ...
  void *argtable[] = {
      ...
      arg_u64_0("s", "samples", "<dec>", "number of samples to collect"),
      arg_lit0("v", "verbose", "verbose output"),
      arg_lit0("@", NULL, "continuous sniffing mode"), arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, true);
  uint32_t samples = arg_get_u32_def(ctx, 1, 0);
  bool verbose = arg_get_lit(ctx, 2);
  bool cm = arg_get_lit(ctx, 3);
...
int lfsim_upload_gb(void) {
  ...
  for (uint16_t i = 0; i < GraphTraceLen; i += PM3_CMD_DATA_SIZE - 3) {
    ...
    SendCommandNG(CMD_LF_UPLOAD_SIM_SAMPLES, (uint8_t *)&payload_up,
                  sizeof(struct pupload));
...
int CmdLFfskSim(const char *Cmd) {
  ...
  CLIParserInit(
      &ctx, "lf simfsk",
      "Simulate FSK tag from demodbuffer or input. There are about four FSK "
      "modulations to know of.\n"
      ...
      "lf simfsk -c 40 --high 8 --low 5 -d 010203  --> FSK1  rf/40  data "
      "010203\n"
      "lf simfsk -c 40 --high 5 --low 8 -d 010203  --> FSK1a rf/40  data "
      "010203\n"
      "lf simfsk -c 64 --high 10 --low 8 -d 010203 --> FSK2  rf/64  data "
      "010203\n"
      "lf simfsk -c 64 --high 8 --low 10 -d 010203 --> FSK2a rf/64  data "
      "010203\n\n"
      "lf simfsk -c 50 --high 10 --low 8 -d 1D5559555569A9A555A59569        "
      "--> simulate HID Prox tag manually\n"
      "lf simfsk -c 50 --high 10 --low 8 --stt -d 011DB2487E8D811111111111  "
      "--> simulate AWID tag manually");

  void *argtable[] = {
      arg_param_begin,
      arg_u64_0("c", "clk", "<dec>",
                "manually set clock - can autodetect if using DemodBuffer "
                "(default 64)"),
      arg_u64_0(NULL, "low", "<dec>", "manually set larger Field Clock"),
      arg_u64_0(NULL, "high", "<dec>", "manually set smaller Field Clock"),
      arg_lit0(NULL, "stt",
               "TBD! - STT to enable a gap between playback repetitions "
               "(default: no gap)"),
  ...
  if (bs_len == 0) {
    ...
    // might be able to autodetect FC and clock from Graphbuffer if using demod
    // buffer will need clock, fchigh, fclow and bitstream
    ...
    PrintAndLogEx(DEBUG, "Detected rf/%u, High fc/%u, Low fc/%u, n %zu ", clk,
                  fchigh, fclow, DemodBufferLen);
  ...
  if (clk == 0) {
    clk = 50;
    PrintAndLogEx(DEBUG, "Autodetection of clock failed, falling back to rf/%u",
                  clk);
  }
  ...
  if (fchigh == 0) {
    fchigh = 10;
    PrintAndLogEx(DEBUG,
                  "Autodetection of larger clock failed, falling back to fc/%u",
                  fchigh);
  }
  ...
  if (fclow == 0) {
    fclow = 8;
    PrintAndLogEx(
        DEBUG, "Autodetection of smaller clock failed, falling back to fc/%u",
        fclow);
  }
  ...
  if (size > (PM3_CMD_DATA_SIZE - sizeof(lf_fsksim_t))) {
    PrintAndLogEx(WARNING,
                  "DemodBuffer too long for current implementation - length: "
                  "%zu - max: %zu",
  ...
  clearCommandBuffer();
  SendCommandNG(CMD_LF_FSK_SIMULATE, (uint8_t *)payload,
                sizeof(lf_fsksim_t) + size);
  free(payload);
  setClockGrid(clk, 0);
...
int CmdLFaskSim(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "lf simask", "Simulate ASK tag from demodbuffer or input",
      "lf simask --clk 32 --am -d 0102030405   --> simulate ASK/MAN rf/32\n"
      "lf simask --clk 32 --bi -d 0102030405   --> simulate ASK/BIPHASE "
      "rf/32\n\n"
      "lf simask --clk 64 --am -d ffbd8001686f1924               --> simulate "
      "a EM410x tag\n"
      "lf simask --clk 64 --am --stt -d 5649533200003F340000001B --> simulate "
      "a VISA2K tag");
  ...
  void *argtable[] = {
      ...
      arg_lit0(NULL, "stt",
               "add t55xx Sequence Terminator gap - default: no gaps (only "
               "manchester)"),
      arg_str0("d", "data", "<hex>", "data to sim - omit to use DemodBuffer"),
      arg_lit0("v", "verbose", "verbose output"),
      arg_param_end};
  ...
  if (bs_len == 0) {
    // Using data from DemodBuffer
    // might be able to autodetect FC and clock from Graphbuffer if using demod
    // buffer will need carrier, clock, and bitstream
    PrintAndLogEx(INFO, "No user supplied data, using Demodbuffer...");
  ...
  clearCommandBuffer();
  SendCommandNG(CMD_LF_ASK_SIMULATE, (uint8_t *)payload,
                sizeof(lf_asksim_t) + size);
  free(payload);
  setClockGrid(clk, 0);
...
int CmdLFpskSim(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "lf simpsk", "Simulate PSK tag from demodbuffer or input",
                "lf simpsk -1 --clk 40 --fc 4 -d 01020304   --> simulate PSK1 "
                "rf/40 psksub fc/4, data 01020304\n\n"
                "lf simpsk -1 --clk 32 --fc 2 -d a0000000bd989a11   --> "
                "simulate a indala tag manually");
  ...
  if (bs_len == 0) {
    ...
    PrintAndLogEx(DEBUG, "Detected rf/%u, fc/%u, n %zu ", clk, carrier,
                  DemodBufferLen);
  ...
  memcpy(payload->data, DemodBuffer, size);
  clearCommandBuffer();
  SendCommandNG(CMD_LF_PSK_SIMULATE, (uint8_t *)payload,
                sizeof(lf_psksim_t) + size);
  free(payload);
  setClockGrid(clk, 0);
...
int CmdLFSimBidir(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "lf simbidir",
                "Simulate LF tag with bidirectional data transmission between "
                "reader and tag",
                "lf simbidir");
...
int CmdVchDemod(const char *Cmd) {
    ...
    // iceman, using correlate as preamble detect seems way better than our
current memcompare
    ...
    // It does us no good to find the sync pattern, with fewer than 2048 samples
after it.
...
int CmdLFfind(const char *Cmd) {
  ...
  CLIParserInit(
      &ctx, "lf search",
      "Read and search for valid known tag. For offline mode, you can `data "
      "load` first then search.",
      "lf search       -> try reading data from tag & search for known tag\n"
      "lf search -1    -> use data from GraphBuffer & search for known tag\n"
      "lf search -u    -> try reading data from tag & search for known and "
      "unknown tag\n"
      "lf search -1u   -> use data from GraphBuffer & search for known and "
      "unknown tag\n");
  ...
  if (demodDestron(true) == PM3_SUCCESS) {
    PrintAndLogEx(SUCCESS,
                  "\nValid " _GREEN_(
                      "FDX-A FECAVA Destron ID") " found!"); // to do before HID
    goto out;
  }
  ...
  //    _GREEN_("Texas Instrument ID") " found!"); goto out;} if (demodFermax()
  //    == PM3_SUCCESS) { PrintAndLogEx(SUCCESS, "\nValid " _GREEN_("Fermax ID")
  //    " found!"); goto out;}
  ...
  if (search_unk) {
    ...
    int ans = AutoCorrelate(GraphBuffer, GraphBuffer, GraphTraceLen, 8000,
                            false, false);
    if (ans > 0) {

      PrintAndLogEx(INFO, "Possible auto correlation of %d repeating samples",
                    ans);
    ...
    if (ASKDemod_ext(0, 0, 0, 0, false, true, false, 1, &st) == PM3_SUCCESS) {
      ...
      PrintAndLogEx(INFO, "if it does not look right it could instead be "
                          "ASK/Biphase - try " _YELLOW_("'data rawdemod ab'"));
      goto out;
    }
    ...
    if (CmdPSK1rawDemod("") == PM3_SUCCESS) {
      ...
      PrintAndLogEx(
          INFO, "    Could also be PSK2 - try " _YELLOW_("'data rawdemod p2'"));
      PrintAndLogEx(INFO, "    Could also be PSK3 - [currently not supported]");
      PrintAndLogEx(
          INFO, "    Could also be  NRZ - try " _YELLOW_("'data rawdemod nr"));
      goto out;
    }
...
static command_t CommandTable[] = {
    ...
    {"jablotron", CmdLFJablotron, AlwaysAvailable,
     "{ Jablotron RFIDs...         }"},
    {"keri", CmdLFKeri, AlwaysAvailable, "{ KERI RFIDs...              }"},
    {"motorola", CmdLFMotorola, AlwaysAvailable,
     "{ Motorola RFIDs...          }"},
    {"nedap", CmdLFNedap, AlwaysAvailable, "{ Nedap RFIDs...             }"},
    {"nexwatch", CmdLFNEXWATCH, AlwaysAvailable,
     "{ NexWatch RFIDs...          }"},
    {"noralsy", CmdLFNoralsy, AlwaysAvailable,
     "{ Noralsy RFIDs...           }"},
    {"pac", CmdLFPac, AlwaysAvailable, "{ PAC/Stanley RFIDs...       }"},
    {"paradox", CmdLFParadox, AlwaysAvailable,
     "{ Paradox RFIDs...           }"},
    {"pcf7931", CmdLFPCF7931, AlwaysAvailable,
     "{ PCF7931 CHIPs...           }"},
    {"presco", CmdLFPresco, AlwaysAvailable, "{ Presco RFIDs...            }"},
    {"pyramid", CmdLFPyramid, AlwaysAvailable,
     "{ Farpointe/Pyramid RFIDs... }"},
    {"securakey", CmdLFSecurakey, AlwaysAvailable,
     "{ Securakey RFIDs...         }"},
    ...
    {"visa2000", CmdLFVisa2k, AlwaysAvailable,
     "{ Visa2000 RFIDs...          }"},
    {"-----------", CmdHelp, AlwaysAvailable,
     "--------------------- " _CYAN_("General") " ---------------------"},
    {"config", CmdLFConfig, IfPm3Lf,
     "Get/Set config for LF sampling, bit/sample, decimation, frequency"},
    {"cmdread", CmdLFCommandRead, IfPm3Lf,
     "Modulate LF reader field to send command before read"},
    {"read", CmdLFRead, IfPm3Lf, "Read LF tag"},
    {"search", CmdLFfind, AlwaysAvailable,
    ...
    //    {"simnrz",      CmdLFnrzSim,        IfPm3Lf,         "Simulate "
    //    _YELLOW_("NRZ") " tag"},
    {"simbidir", CmdLFSimBidir, IfPm3Lf,
     "Simulate LF tag (with bidirectional data transmission between reader and "
     "tag)"},
    ...
    //    {"vchdemod",    CmdVchDemod,        AlwaysAvailable, "Demodulate
    //    samples for VeriChip"},
    //    {"flexdemod",   CmdFlexdemod,       AlwaysAvailable, "Demodulate
    //    samples for Motorola FlexPass"},
    {NULL, NULL, NULL, NULL}};
