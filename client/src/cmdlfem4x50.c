...
static void prepare_result(const uint8_t *data, int fwr, int lwr,
                           em4x50_word_t *words) {
...
static void print_result(const em4x50_word_t *words, int fwr, int lwr) {
  ...
  for (int i = fwr; i <= lwr; i++) {
    ...
    PrintAndLogEx(INFO, " %2i | " _GREEN_("%s") "| %s| %s", i,
                  sprint_hex(words[i].byte, 4), r, s);
  }
  PrintAndLogEx(INFO, "----+-------------+-------------+--------------------");
}
...
static void print_info_result(uint8_t *data, bool verbose) {
  ...
  int fwwi =
      reflect8(words[EM4X50_PROTECTION].byte[FIRST_WORD_WRITE_INHIBITED]);
  ...
  PrintAndLogEx(INFO, "password check   %3s",
                (bpwc) ? _RED_("on") : _GREEN_("off"));
...
static int em4x50_load_file(const char *filename, uint8_t *data,
...
int CmdEM4x50ELoad(const char *Cmd) {
  ...
  if (em4x50_load_file(filename, data, DUMP_FILESIZE, &bytes_read) !=
      PM3_SUCCESS) {
  ...
  PrintAndLogEx(INFO, "Uploading dump " _YELLOW_("%s") " to emulator memory",
                filename);
...
int CmdEM4x50ESave(const char *Cmd) {
  ...
  uint8_t data[DUMP_FILESIZE] = {0x0};
  if (GetFromDevice(BIG_BUF_EML, data, DUMP_FILESIZE, 0, NULL, 0, NULL, 2500,
                    false) == false) {
...
int CmdEM4x50Login(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "lf em 4x50 login", "Login into EM4x50 tag.",
...
int CmdEM4x50Brute(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "lf em 4x50 brute",
                ...
                "lf em 4x50 brute --first 12330000 --last 12340000     -> "
                "tries pwds from 0x12330000 to 0x1234000000\n");
  ...
  PrintAndLogEx(INFO, "Trying %i passwords in range [0x%08x, 0x%08x]", no_iter,
  ...
  // print response
  if (resp.status == PM3_SUCCESS)
    PrintAndLogEx(SUCCESS, "Password " _GREEN_("found") ": 0x%08x",
...
int CmdEM4x50Chk(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "lf em 4x50 chk", "Dictionary attack against EM4x50.",
  ...
  if (status == PM3_SUCCESS) {
    PrintAndLogEx(SUCCESS, "Key " _GREEN_("found: %02x %02x %02x %02x"),
                  resp.data.asBytes[3], resp.data.asBytes[2],
                  resp.data.asBytes[1], resp.data.asBytes[0]);
  } else {
...
int read_em4x50_uid(void) {
  ...
  if (res == PM3_SUCCESS)
    PrintAndLogEx(INFO, " Serial: " _GREEN_("%s"),
                  sprint_hex(words[EM4X50_DEVICE_SERIAL].byte, 4));
  return res;
}
...
int em4x50_read(em4x50_data_t *etd, em4x50_word_t *out) {
  ...
  em4x50_word_t words[EM4X50_NO_WORDS];
  prepare_result(data, etd->addresses & 0xFF, (etd->addresses >> 8) & 0xFF,
                 words);
...
int CmdEM4x50Read(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "lf em 4x50 read", "Reads single EM4x50 block/word.",
                "lf em 4x50 rdbl -b 3\n"
                "lf em 4x50 rdbl -b 32 -p 12345678   -> reads block 32 with "
                "pwd 0x12345678\n");

  void *argtable[] = {
      arg_param_begin, arg_int1("b", "block", "<dec>", "block/word address"),
  ...
  if (pwd_len) {
    if (pwd_len != 4) {
      PrintAndLogEx(FAILED, "password length must be 4 bytes instead of %d",
                    pwd_len);
      return PM3_EINVARG;
    } else {
...
int CmdEM4x50Info(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "lf em 4x50 info", "Tag information EM4x50.",
                "lf em 4x50 info\n"
  ...
  do {
    ...
    if (now > 0) {
      ...
      for (int i = 0; i < now; i++) {
        ...
        PrintAndLogEx(INFO, _GREEN_(" %s") "| %s", sprint_hex(words[i].byte, 4),
                      r);
      }
...
int CmdEM4x50Dump(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "lf em 4x50 dump",
                "Reads all blocks/words from EM4x50 tag and saves dump in "
                "bin/eml/json format.",
                "lf em 4x50 dump\n"
...
int CmdEM4x50Write(const char *Cmd) {
  ...
  void *argtable[] = {
      ...
      arg_str0("p", "pwd", "<hex>", "password, 4 bytes, lsb"), arg_param_end};
  ...
  if (word_len != 4) {
    PrintAndLogEx(FAILED, "word/data length must be 4 bytes instead of %d",
                  word_len);
    return PM3_EINVARG;
  }
  ...
  PrintAndLogEx(
      HINT, "Try `" _YELLOW_("lf em 4x50 rdbl -a %u") "` - to read your data",
      addr);
...
int CmdEM4x50WritePwd(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "lf em 4x50 writepwd", "Writes EM4x50 password.",
  ...
  PrintAndLogEx(SUCCESS, "Writing new password %s (%s)",
                sprint_hex_inrow(npwd, sizeof(npwd)), _GREEN_("ok"));
  return PM3_SUCCESS;
}
...
int CmdEM4x50Restore(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "lf em 4x50 restore",
                ...
                "if used with -u,  the filetemplate `lf-4x50-UID-dump.bin` is "
                "used as filename",
                "lf em 4x50 restore -u 1b5aff5c           -> uses "
                "lf-4x50-1B5AFF5C-dump.bin\n"
                "lf em 4x50 restore -f mydump.eml\n"
                "lf em 4x50 restore -u 1b5aff5c -p 12345678\n"
                "lf em 4x50 restore -f mydump.eml -p 12345678\n");

  void *argtable[] = {
      arg_param_begin, arg_str0("u", "uid", "<hex>", "uid, 4 hex bytes, msb"),
      arg_str0("f", "filename", "<filename>", "dump filename (bin/eml/json)"),
      arg_str0("p", "pwd", "<hex>", "password, 4 hex bytes, lsb"),
      arg_param_end};
...
static command_t CommandTable[] = {
    ...
    {"reader", CmdEM4x50Reader, IfPm3EM4x50,
     "show standard read mode data of EM4x50"},
    {"restore", CmdEM4x50Restore, IfPm3EM4x50, "restore EM4x50 dump to tag"},
    {"sim", CmdEM4x50Sim, IfPm3EM4x50, "simulate EM4x50 tag"},
    {"eload", CmdEM4x50ELoad, IfPm3EM4x50,
     "upload dump of EM4x50 to emulator memory"},
    {"esave", CmdEM4x50ESave, IfPm3EM4x50, "save emulator memory to file"},
    {"eview", CmdEM4x50EView, IfPm3EM4x50,
     "view EM4x50 content in emulator memory"},
    {NULL, NULL, NULL, NULL}};
