...
static void scramble(uint8_t *arr, uint8_t len) {
  uint8_t lut[] = {
      };
...
int demodGallagher(bool verbose) {
  ...
  if (ans < 0) {
    if (ans == -1)
      ...
      PrintAndLogEx(DEBUG, "DEBUG: Error - GALLAGHER: Size not correct: %zu",
                    size);
    else
  ...
  // 24bit CN
  uint32_t cn =
      arr[0] << 16 | (arr[4] & 0x1F) << 11 | arr[2] << 3 | (arr[3] & 0xE0) >> 5;

  // 4bit issue level
  uint8_t il = arr[7] & 0x0F;

  PrintAndLogEx(SUCCESS,
                "GALLAGHER - Region: " _GREEN_("%u") " FC: " _GREEN_(
                    "%u") " CN: " _GREEN_("%u") " Issue Level: " _GREEN_("%u"),
                rc, fc, cn, il);
  PrintAndLogEx(SUCCESS, "   Displayed: " _GREEN_("%C%u"), rc + 'A', fc);
  PrintAndLogEx(SUCCESS, "   Raw: %08X%08X%08X", raw1, raw2, raw3);
  PrintAndLogEx(SUCCESS, "   CRC: %02X - %02X (%s)", crc, calc_crc,
                (crc == calc_crc) ? "ok" : "fail");
  return PM3_SUCCESS;
}
...
static int CmdGallagherReader(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "lf gallagher reader", "read a GALLAGHER tag",
...
static bool isValidGallagherParams(int8_t rc, int32_t fc, int32_t cn,
                                   int8_t il) {
  bool isValid = true;
...
static void setBitsInBlocks(uint32_t *blocks, uint8_t *pos, uint32_t data,
                            uint8_t data_len) {
...
static void createBlocks(uint32_t *blocks, uint8_t rc, uint16_t fc, uint32_t cn,
                         uint8_t il) {
...
static int CmdGallagherClone(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "lf gallagher clone",
      "clone a GALLAGHER tag to a T55x7, Q5/T5555 or EM4305/4469 tag.",
      "lf gallagher clone --raw 0FFD5461A9DA1346B2D1AC32\n"
      "lf gallagher clone --q5 --raw 0FFD5461A9DA1346B2D1AC32 -> encode for "
      "Q5/T5555 tag\n"
      "lf gallagher clone --em --raw 0FFD5461A9DA1346B2D1AC32 -> encode for "
      "EM4305/4469\n"
  ...
  if (region_code == -1 && facility_code == -1 && card_number == -1 &&
      issue_level == -1) {
    if (!use_raw) {
    ...
    if (!isValidGallagherParams(region_code, facility_code, card_number,
                                issue_level)) {
      return PM3_EINVARG;
    }
  }
  ...
  if (use_raw) {
    ...
    createBlocks(blocks + 1, region_code, facility_code, card_number,
                 issue_level);
  }

  // Pac - compat mode, NRZ, data rate 40, 3 data blocks
  blocks[0] = T55x7_MODULATION_MANCHESTER | T55x7_BITRATE_RF_32 |
              3 << T55x7_MAXBLOCK_SHIFT;
  char cardtype[16] = {"T55x7"};
  // Q5
  if (q5) {
    blocks[0] = T5555_FIXED | T5555_MODULATION_MANCHESTER |
...
static int CmdGallagherSim(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "lf gallagher sim",
      "Enables simulation of GALLAGHER card with specified card number.\n"
      "Simulation runs until the button is pressed or another USB command is "
      "issued.\n",
  ...
  // ASK/MAN sim.
  PrintAndLogEx(SUCCESS, "Simulating Gallagher - raw " _YELLOW_("%s"),
...
static command_t CommandTable[] = {
    ...
    {"demod", CmdGallagherDemod, AlwaysAvailable,
     "Demodulate an GALLAGHER tag from the GraphBuffer"},
    {"reader", CmdGallagherReader, IfPm3Lf,
     "Attempt to read and extract tag data from the antenna"},
...
int detectGallagher(uint8_t *dest, size_t *size) {
  if (*size < 96)
    return -1; // make sure buffer has data
  size_t startIdx = 0;
