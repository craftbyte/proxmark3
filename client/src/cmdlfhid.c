...
static int sendTry(uint8_t format_idx, wiegand_card_t *card, uint32_t delay,
                   bool verbose) {
  ...
  if (verbose) {
    PrintAndLogEx(
        INFO,
        "Trying FC: " _YELLOW_("%u") " CN: " _YELLOW_(
            "%" PRIu64) " Issue level: " _YELLOW_("%u") " OEM: " _YELLOW_("%u"),
        card->FacilityCode, card->CardNumber, card->IssueLevel, card->OEM);
  }
...
// HID Prox demod - FSK RF/50 with preamble of 00011101 (then manchester
// encoded) print full HID Prox ID and some bit format details if found
int demodHID(bool verbose) {
  (void)verbose; // unused so far

  // HID simulation etc uses 0/1 as signal data. This must be converted in order
  // to demod it back again
  if (isGraphBitstream()) {
  ...
  if (idx < 0) {
    ...
    if (idx == -1)
      ...
      PrintAndLogEx(
          DEBUG,
          "DEBUG: Error - " _RED_("HID error in Manchester data, size %zu"),
          size);
    else
      PrintAndLogEx(DEBUG, "DEBUG: Error - " _RED_("HID error demoding fsk %d"),
                    idx);
...
// this read is the "normal" read,  which download lf signal and tries to demod
// here.
static int CmdHIDReader(const char *Cmd) {
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "lf hid reader", "read a HID Prox tag",
...
static int CmdHIDWatch(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "lf hid watch",
                ...
                "By default, values are printed and logged until the button is "
                "pressed or another USB command is issued.\n",
                "lf hid watch");
...
static int CmdHIDSim(const char *Cmd) {
  ...
  void *argtable[] = {
      arg_param_begin,
      arg_str0("w", "wiegand", "<format>",
               "see " _YELLOW_("`wiegand list`") " for available formats"),
  ...
  char format[16] = {0};
  int format_len = 0;
  CLIParamStrToBuf(arg_get_str(ctx, 1), (uint8_t *)format, sizeof(format),
                   &format_len);
...
static int CmdHIDClone(const char *Cmd) {
  ...
  CLIParserInit(
      ...
      "lf hid clone -r 2006ec0c86                      -> write raw value (HID "
      "10301 26 bit)\n"
      "lf hid clone -r 2e0ec00c87                      -> write raw value (HID "
      "Corporate 35 bit)\n"
      "lf hid clone -r 01f0760643c3                    -> write raw value (HID "
      "P10001 40 bit)\n"
      "lf hid clone -r 01400076000c86                  -> write raw value (HID "
      "Corporate 48 bit)\n"
      "lf hid clone -w H10301 --fc 118 --cn 1603       -> write raw value (HID "
      "10301 26 bit)\n"
      "lf hid clone -w H10301 --fc 118 --cn 1603 --q5  -> HID 10301 26 bit, "
      "encode for Q5/T5555 tag\n"
      "lf hid clone -w H10301 --fc 118 --cn 1603 --em  -> HID 10301 26 bit, "
      "encode for EM4305/4469");
  ...
  if (raw_len == 0) {
    ...
    PrintAndLogEx(INFO, "Preparing to clone HID tag using raw " _YELLOW_("%s"),
                  raw);
  }
...
/*
    PrintAndLogEx(NORMAL, "HID | OEM | FC   | CN      |  Wiegand  |  HID
   Formatted"); PrintAndLogEx(NORMAL,
   "----+-----+------+---------+-----------+--------------------");
    PrintAndLogEx(NORMAL, " %u | %03u | %03u  | %" PRIu64 "  | %" PRIX64 "  | %"
   PRIX64, fmtlen[i], oem, fc, cardnum, wiegand, blocks
                     );
    }
...
static int CmdHIDBrute(const char *Cmd) {
  ...
  CLIParserInit(
      ...
      "This is a attack against reader. if cardnumber is given, it starts with "
      "it and goes up / down one step\n"
  ...
  void *argtable[] = {
      ...
      arg_str1("w", "wiegand", "<format>",
               "see " _YELLOW_("`wiegand list`") " for available formats"),
      arg_u64_0(NULL, "fc", "<dec>", "facility code"),
      arg_u64_0(NULL, "cn", "<dec>", "card number to start with"),
      arg_u64_0("i", "issue", "<dec>", "issue level"),
      arg_u64_0("o", "oem", "<dec>", "OEM code"),
      arg_u64_0("d", "delay", "<dec>",
                "delay betweens attempts in ms. Default 1000ms"),
      arg_lit0(
          NULL, "up",
          "direction to increment card number. (default is both directions)"),
      arg_lit0(
          NULL, "down",
          "direction to decrement card number. (default is both directions)"),
      arg_param_end};
  CLIExecWithReturn(ctx, Cmd, argtable, false);
  ...
  if (verbose) {
    ...
    case 0:
      PrintAndLogEx(
          INFO,
          "Brute-forcing direction: " _YELLOW_("BOTH") " delay " _YELLOW_("%d"),
          delay);
      break;
    case 1:
      PrintAndLogEx(
          INFO,
          "Brute-forcing direction: " _YELLOW_("UP") " delay " _YELLOW_("%d"),
          delay);
      break;
    case 2:
      PrintAndLogEx(
          INFO,
          "Brute-forcing direction: " _YELLOW_("DOWN") " delay " _YELLOW_("%d"),
          delay);
      break;
    default:
      break;
  ...
  PrintAndLogEx(INFO, "Press pm3-button to abort simulation or press " _GREEN_(
                          "`<enter>`") " to exit");
...
static command_t CommandTable[] = {
    ...
    {"demod", CmdHIDDemod, AlwaysAvailable,
