...
int CmdWiegandList(const char *Cmd) {
  ...
  CLIParserInit(&ctx, "wiegand info", "List available wiegand formats",
                "wiegand list");
...
int CmdWiegandEncode(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(
      &ctx, "wiegand encode", "Encode wiegand formatted number to raw hex",
  ...
  void *argtable[] = {
      ...
      arg_str0("w", "wiegand", "<format>",
               "see `wiegand list` for available formats"),
  ...
  int len = 0;
  char format[16] = {0};
  CLIParamStrToBuf(arg_get_str(ctx, 5), (uint8_t *)format, sizeof(format),
                   &len);
...
int CmdWiegandDecode(const char *Cmd) {
  ...
  CLIParserContext *ctx;
  CLIParserInit(&ctx, "wiegand decode", "Decode raw hex to wiegand format",
                "wiegand decode --raw 2006f623ae");

  void *argtable[] = {
      arg_param_begin, arg_lit0("p", "parity", "ignore invalid parity"),
      arg_strx1("r", "raw", "<hex>", "raw hex to be decoded"), arg_param_end};
...
static command_t CommandTable[] = {
    ...
    {"encode", CmdWiegandEncode, AlwaysAvailable,
     "Encode to wiegand raw hex (currently for HID Prox)"},
    {"decode", CmdWiegandDecode, AlwaysAvailable,
