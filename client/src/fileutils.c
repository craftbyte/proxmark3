 ...
 *
 * THIS CODE SHOULD NEVER BE USED TO INFRINGE PATENTS OR INTELLECTUAL PROPERTY
 *RIGHTS.
 *
 *****************************************************************************
 *
...
#else
#define make_dir(a) mkdir(a,0755) //note 0755 MUST have leading 0 for octal
linux file permissions #endif bool create_path(const char *dirname) {
    ...
    if (fileExists(dirname) == 0) {
        ...
        if (make_dir(dirname) != 0) {
           PrintAndLogEx(ERR, "could not create directory....
"_RED_("%s"),dirname); return false;
        }
    }
    return true;
}
...
bool setDefaultPath(savePaths_t pathIndex, const char *Path) {
  ...
  if (pathIndex < spItemCount) {
    ...
    if (Path != NULL) {
      session.defaultPaths[pathIndex] =
          (char *)realloc(session.defaultPaths[pathIndex], strlen(Path) + 1);
...
static char *filenamemcopy(const char *preferredName, const char *suffix) {
  ...
  if (suffix == NULL)
    return NULL;
  char *fileName = (char *)calloc(strlen(preferredName) + strlen(suffix) + 1,
                                  sizeof(uint8_t));
  if (fileName == NULL)
    return NULL;
...
char *newfilenamemcopy(const char *preferredName, const char *suffix) {
  ...
  char *fileName = (char *)calloc(
      p_namelen + strlen(suffix) + 1 + 10,
      sizeof(uint8_t)); // 10: room for filenum to ensure new filename
  if (fileName == NULL) {
    return NULL;
  }
...
int saveFile(const char *preferredName, const char *suffix, const void *data,
             size_t datalen) {
  ...
  if (!f) {
    PrintAndLogEx(WARNING, "file not found or locked. '" _YELLOW_("%s") "'",
                  fileName);
    free(fileName);
    return PM3_EFILE;
  }
  ...
  fclose(f);
  PrintAndLogEx(
      SUCCESS, "saved " _YELLOW_("%zu") " bytes to binary file " _YELLOW_("%s"),
      datalen, fileName);
  free(fileName);
  return PM3_SUCCESS;
}

int saveFileEML(const char *preferredName, uint8_t *data, size_t datalen,
                size_t blocksize) {
  ...
  PrintAndLogEx(
      SUCCESS,
      "saved " _YELLOW_("%" PRId32) " blocks to text file " _YELLOW_("%s"),
      blocks, fileName);
...
int saveFileJSON(const char *preferredName, JSONFileType ftype, uint8_t *data,
                 size_t datalen, void (*callback)(json_t *)) {
  return saveFileJSONex(preferredName, ftype, data, datalen, true, callback);
}
int saveFileJSONex(const char *preferredName, JSONFileType ftype, uint8_t *data,
                   size_t datalen, bool verbose, void (*callback)(json_t *)) {
  ...
  case jsfCardMemory: {
    ...
    for (size_t i = 0; i < (datalen / 16); i++) {
      ...
      if (mfIsSectorTrailer(i)) {
        ...
        snprintf(path, sizeof(path), "$.SectorKeys.%d.AccessConditions",
                 mfSectorNum(i));
        JsonSaveBufAsHexCompact(root, path, &data[i * 16 + 6], 4);

        snprintf(path, sizeof(path),
                 "$.SectorKeys.%d.AccessConditionsText.block%zu",
                 mfSectorNum(i), i - 3);
        ...
        snprintf(path, sizeof(path),
                 "$.SectorKeys.%d.AccessConditionsText.UserData",
                 mfSectorNum(i));
  ...
  case jsfMfuMemory: {
    ...
    JsonSaveBufAsHexCompact(root, "$.Card.Version", tmp->version,
                            sizeof(tmp->version));
    ...
    JsonSaveBufAsHexCompact(root, "$.Card.Signature", tmp->signature,
  ...
  case jsfIclass: {
    ...
    JsonSaveBufAsHexCompact(root, "$.Card.Configuration", (uint8_t *)&hdr->conf,
                            sizeof(hdr->conf));

    uint8_t pagemap = get_pagemap(hdr);
    if (pagemap == PICOPASS_NON_SECURE_PAGEMODE) {
      picopass_ns_hdr *ns_hdr = (picopass_ns_hdr *)data;
      JsonSaveBufAsHexCompact(root, "$.Card.AIA", ns_hdr->app_issuer_area,
                              sizeof(ns_hdr->app_issuer_area));
    } else {
      JsonSaveBufAsHexCompact(root, "$.Card.Epurse", hdr->epurse,
                              sizeof(hdr->epurse));
      JsonSaveBufAsHexCompact(root, "$.Card.Kd", hdr->key_d,
                              sizeof(hdr->key_d));
      JsonSaveBufAsHexCompact(root, "$.Card.Kc", hdr->key_c,
                              sizeof(hdr->key_c));
      JsonSaveBufAsHexCompact(root, "$.Card.AIA", hdr->app_issuer_area,
  ...
  if (res) {
    PrintAndLogEx(FAILED, "error: can't save the file: " _YELLOW_("%s"),
                  fileName);
    retval = 200;
    goto out;
  }
...
int saveFileWAVE(const char *preferredName, int *data, size_t datalen) {
  ...
  struct wave_info_t wave_info = {
      .signature = "RIFF",
      .filesize = sizeof(wave_info) - sizeof(wave_info.signature) -
                  sizeof(wave_info.filesize) + datalen,
      .type = "WAVE",
      .format.tag = "fmt ",
      .format.size = sizeof(wave_info.format) - sizeof(wave_info.format.tag) -
                     sizeof(wave_info.format.size),
      .format.codec = 1, // PCM
      .format.nb_channel = 1,
  ...
  PrintAndLogEx(
      SUCCESS, "saved " _YELLOW_("%zu") " bytes to wave file " _YELLOW_("'%s'"),
      2 * datalen, fileName);
...
int saveFilePM3(const char *preferredName, int *data, size_t datalen) {
  ...
  PrintAndLogEx(SUCCESS,
                "saved " _YELLOW_("%zu") " bytes to PM3 file " _YELLOW_("'%s'"),
                datalen, fileName);
...
int createMfcKeyDump(const char *preferredName, uint8_t sectorsCnt,
                     sector_t *e_sector) {
  ...
  fflush(f);
  fclose(f);
  PrintAndLogEx(SUCCESS, "Found keys have been dumped to " _YELLOW_("%s"),
                fileName);
  PrintAndLogEx(
      INFO,
      "FYI! --> " _YELLOW_("0xFFFFFFFFFFFF") " <-- has been inserted for "
                                             "unknown keys where " _YELLOW_(
                                                 "res") " is " _YELLOW_("0"));
  free(fileName);
  return PM3_SUCCESS;
}

int loadFile(const char *preferredName, const char *suffix, void *data,
             size_t maxdatalen, size_t *datalen) {
  ...
  if (bytes_read > maxdatalen) {
    PrintAndLogEx(WARNING,
                  "Warning, bytes read exceed calling array limit. Max bytes "
                  "is %zu bytes",
                  maxdatalen);
  ...
  PrintAndLogEx(
      SUCCESS,
      "loaded " _YELLOW_("%zu") " bytes from binary file " _YELLOW_("%s"),
      bytes_read, fileName);
...
int loadFile_safe(const char *preferredName, const char *suffix, void **pdata,
                  size_t *datalen) {
  return loadFile_safeEx(preferredName, suffix, pdata, datalen, true);
}
int loadFile_safeEx(const char *preferredName, const char *suffix, void **pdata,
                    size_t *datalen, bool verbose) {
...
int loadFileEML(const char *preferredName, void *data, size_t *datalen) {
  ...
  PrintAndLogEx(
      SUCCESS,
      "loaded " _YELLOW_("%zu") " bytes from text file " _YELLOW_("%s"),
      counter, fileName);
...
int loadFileJSON(const char *preferredName, void *data, size_t maxdatalen,
                 size_t *datalen, void (*callback)(json_t *)) {
  return loadFileJSONex(preferredName, data, maxdatalen, datalen, true,
                        callback);
}
int loadFileJSONex(const char *preferredName, void *data, size_t maxdatalen,
                   size_t *datalen, bool verbose, void (*callback)(json_t *)) {
  ...
  *datalen = 0;
  ...
  if (!root) {
    PrintAndLogEx(ERR, "ERROR: json " _YELLOW_("%s") " error on line %d: %s",
                  preferredName, error.line, error.text);
    retval = PM3_ESOFT;
    goto out;
  }
  ...
  if (!json_is_object(root)) {
    PrintAndLogEx(ERR,
                  "ERROR: Invalid json " _YELLOW_(
                      "%s") " format. root must be an object.",
                  preferredName);
    retval = PM3_ESOFT;
    goto out;
  }
...
int loadFileDICTIONARY(const char *preferredName, void *data, size_t *datalen,
                       uint8_t keylen, uint32_t *keycnt) {
  // t5577 == 4bytes
  // mifare == 6 bytes
  // mf plus == 16 bytes
  // iclass == 8 bytes
  ...
  return loadFileDICTIONARYEx(preferredName, data, 0, datalen, keylen, keycnt,
                              0, NULL, true);
}

int loadFileDICTIONARYEx(const char *preferredName, void *data,
                         size_t maxdatalen, size_t *datalen, uint8_t keylen,
                         uint32_t *keycnt, size_t startFilePosition,
                         size_t *endFilePosition, bool verbose) {
  ...
  char *path;
  if (searchFile(&path, DICTIONARIES_SUBDIR, preferredName, ".dic", false) !=
      PM3_SUCCESS)
    return PM3_EFILE;
  ...
  if (verbose)
    PrintAndLogEx(
        SUCCESS,
        "loaded " _GREEN_("%2d") " keys from dictionary file " _YELLOW_("%s"),
        vkeycnt, path);
...
int loadFileDICTIONARY_safe(const char *preferredName, void **pdata,
                            uint8_t keylen, uint32_t *keycnt) {
...
static int convert_plain_mfu_dump(uint8_t **dump, size_t *dumplen,
                                  bool verbose) {
  ...
  if (verbose) {
    PrintAndLogEx(
        SUCCESS,
        "plain mfu dump format was converted to " _GREEN_("%d") " blocks",
        mfu->pages + 1);
  }
...
static int convert_old_mfu_dump(uint8_t **dump, size_t *dumplen, bool verbose) {
  ...
  */
  ...
  mfu_dump_t *mfu_dump =
      (mfu_dump_t *)calloc(sizeof(mfu_dump_t), sizeof(uint8_t));
  if (mfu_dump == NULL) {
    return PM3_EMALLOC;
  }
  ...
  memcpy(mfu_dump->signature, old_mfu_dump->signature,
  ...
  memcpy(mfu_dump->data + (mfu_dump->pages * 4 + MFU_DUMP_PREFIX_LENGTH),
         old_mfu_dump->pack, 2);

  if (verbose) {
    PrintAndLogEx(
        SUCCESS,
        "old mfu dump format was converted to " _GREEN_("%d") " blocks",
        mfu_dump->pages + 1);
  }
...
static int filelist(const char *path, const char *ext, uint8_t last,
                    bool tentative, uint8_t indent, uint16_t strip) {
  ...
  if (n == -1) {
    ...
    if (tentative == false) {
      ...
      PrintAndLogEx(NORMAL, "%s── "_GREEN_("%s"), last ? "└" : "├",
                    &path[strip]);
    }
    return PM3_EFILE;
  }
  ...
  for (uint16_t i = 0; i < n; i++) {
    ...
    if (is_directory(tmp_fullpath)) {
      ...
      strncat(newpath, namelist[i]->d_name,
              sizeof(newpath) - strlen(newpath) - 1);
      strncat(newpath, "/", sizeof(newpath) - strlen(newpath) - 1);

      filelist(newpath, ext, last + ((i == n - 1) << (indent + 1)), tentative,
               indent + 1, strlen(path));
    } else {
      ...
      if ((ext == NULL) || (ext && (str_endswith(namelist[i]->d_name, ext)))) {
        ...
        PrintAndLogEx(NORMAL, "%s── %-21s", i == n - 1 ? "└" : "├",
                      namelist[i]->d_name);
      }
    }
    free(namelist[i]);
...
int searchAndList(const char *pm3dir, const char *ext) {
  ...
  if (get_my_executable_directory() != NULL) {
    char script_directory_path[strlen(get_my_executable_directory()) +
                               strlen(pm3dir) + 1];
  ...
  if (user_path != NULL) {
    char script_directory_path[strlen(user_path) + strlen(PM3_USER_DIRECTORY) +
                               strlen(pm3dir) + 1];
  ...
  if (exec_path != NULL) {
    char script_directory_path[strlen(exec_path) + strlen(PM3_SHARE_RELPATH) +
                               strlen(pm3dir) + 1];
...
static int searchFinalFile(char **foundpath, const char *pm3dir,
                           const char *searchname, bool silent) {
  if ((foundpath == NULL) || (pm3dir == NULL) || (searchname == NULL))
    return PM3_ESOFT;
  // explicit absolute (/) or relative path (./) => try only to match it
  // directly
  ...
  const char *user_path = get_my_user_directory();
  if (user_path != NULL) {
    char *path = calloc(strlen(user_path) + strlen(PM3_USER_DIRECTORY) +
                            strlen(pm3dir) + strlen(filename) + 1,
                        sizeof(char));
    if (path == NULL)
      goto out;
  ...
  const char *exec_path = get_my_executable_directory();
  if ((exec_path != NULL) && ((strcmp(DICTIONARIES_SUBDIR, pm3dir) == 0) ||
    ...
    char *path =
        calloc(strlen(exec_path) + strlen(pm3dir) + strlen(filename) + 1,
               sizeof(char));
    if (path == NULL)
      goto out;
  ...
  if ((exec_path != NULL) && ((strcmp(TRACES_SUBDIR, pm3dir) == 0) ||
                              (strcmp(FIRMWARES_SUBDIR, pm3dir) == 0) ||
                              (strcmp(BOOTROM_SUBDIR, pm3dir) == 0) ||
                              (strcmp(FULLIMAGE_SUBDIR, pm3dir) == 0))) {
    const char *above = "../";
    char *path = calloc(strlen(exec_path) + strlen(above) + strlen(pm3dir) +
                            strlen(filename) + 1,
                        sizeof(char));
    if (path == NULL)
      goto out;
  ...
  // try pm3 dirs in pm3 installation dir (install mode)
  if (exec_path != NULL) {
    char *path = calloc(strlen(exec_path) + strlen(PM3_SHARE_RELPATH) +
                            strlen(pm3dir) + strlen(filename) + 1,
                        sizeof(char));
    if (path == NULL)
      goto out;
...
int searchFile(char **foundpath, const char *pm3dir, const char *searchname,
               const char *suffix, bool silent) {
