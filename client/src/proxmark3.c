...
static void showBanner(void) {
  ...
  //    https://www.patreon.com/iceman1001/"); PrintAndLogEx(NORMAL, " on paypal
  //    - https://www.paypal.me/iceman1001"); PrintAndLogEx(NORMAL, "\nMonero:
...
static int check_comm(void) {
  ...
  if (IsCommunicationThreadDead() && session.pm3_present) {
    PrintAndLogEx(INFO, "Running in " _YELLOW_("OFFLINE") " mode. Use "_YELLOW_(
...
void
  ...
  if (session.incognito) {
    ...
    if (searchHomeFilePath(&session.history_path, NULL, PROXHISTORY, true) !=
        PM3_SUCCESS) {
...
#if defined(_WIN32)
      //        SetConsoleCtrlHandler((PHANDLER_ROUTINE)terminate_handler,
      //        true);
#else
      ...
      if (execCommand) {
        ...
        if (stdinOnPipe) {
          ...
          if ((ret = getline(&cmd, &len, stdin)) < 0) {
            // TODO this happens also when kbd_enter_pressed() is used, with a
            // key pressed or not
...
static void show_help(bool showFullHelp, char *exec_name) {
  ...
  if (showFullHelp) {
    ...
    PrintAndLogEx(NORMAL, "      -m/--markdown                       dump all "
                          "interactive help at once in markdown syntax");
    PrintAndLogEx(NORMAL,
    ...
    PrintAndLogEx(
        NORMAL, "      --unlock-bootloader                 Enable flashing of "
                "bootloader area *DANGEROUS* (need --flash or --flash-info)");
    ...
    PrintAndLogEx(NORMAL,
                  "      %s " SERIAL_PORT_EXAMPLE_H
                  " -l hf_read            -- execute lua script " _YELLOW_(
                      "`hf_read`") " and quit client",
                  exec_name);
    PrintAndLogEx(NORMAL,
                  "      %s " SERIAL_PORT_EXAMPLE_H
...
static int flash_pm3(char *serial_port_name, uint8_t num_files,
                     char *filenames[FLASH_MAX_FILES], bool can_write_bl) {
  ...
  if (OpenProxmark(&session.current_device, serial_port_name, true, 60, true,
                   FLASHMODE_SPEED)) {
  ...
  for (int i = 0; i < num_files; ++i) {
    ret =
        flash_load(&files[i], filepaths[i], can_write_bl, max_allowed * ONE_KB);
    if (ret != PM3_SUCCESS) {
      goto finish;
    }
    PrintAndLogEx(NORMAL, "");
...
int main(int argc, char *argv[]) {
  ...
  if (!session.pm3_present)
    PrintAndLogEx(INFO,
                  "Running in " _YELLOW_("OFFLINE") " mode. Check " _YELLOW_(
                      "\"%s -h\"") " if it's not what you want.\n",
                  exec_name);

  // ascii art only in interactive client
  if (!script_cmds_file && !script_cmd && session.stdinOnTTY &&
      session.stdoutOnTTY && !flash_mode)
    showBanner();
