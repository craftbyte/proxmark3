...
void FillFileNameByUID(char *filenamePrefix, const uint8_t *uid,
                       const char *ext, const int uidlen) {
...
void hex_to_buffer(const uint8_t *buf, const uint8_t *hex_data,
                   const size_t hex_len, const size_t hex_max_len,
                   const size_t min_str_len, const size_t spaces_between,
                   bool uppercase) {
...
void print_buffer(const uint8_t *data, const size_t len, int level) {
  ...
  for (i = 0; i < len; i += 16) {
    ...
    hex_to_buffer((uint8_t *)(buf + strlen(buf)), data + i, 16,
                  (sizeof(buf) - strlen(buf) - 1), 0, 1, true);
    snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "| %s",
             sprint_ascii(data + i, 16));
  ...
  if (mod) {
    memset(buf, 0, sizeof(buf));
    sprintf(buf, "%*s%02x: ", (level * 4), " ", i);
    hex_to_buffer((uint8_t *)(buf + strlen(buf)), data + i, mod,
                  (sizeof(buf) - strlen(buf) - 1), 0, 1, true);
...
char *sprint_hex_inrow_ex(const uint8_t *data, const size_t len,
                          const size_t min_str_len) {
  static char buf[UTIL_BUFFER_SIZE_SPRINT] = {0};
  hex_to_buffer((uint8_t *)buf, data, len, sizeof(buf) - 1, min_str_len, 0,
                true);
  return buf;
}
...
char *sprint_hex_inrow_spaces(const uint8_t *data, const size_t len,
                              size_t spaces_between) {
  static char buf[UTIL_BUFFER_SIZE_SPRINT] = {0};
  hex_to_buffer((uint8_t *)buf, data, len, sizeof(buf) - 1, 0, spaces_between,
                true);
  return buf;
}

char *sprint_bin_break(const uint8_t *data, const size_t len,
  ...
  if (breaks > 0 && len % breaks != 0)
    rowlen = (len + (len / breaks) > MAX_BIN_BREAK_LENGTH)
                 ? MAX_BIN_BREAK_LENGTH
                 : len + (len / breaks);

  // PrintAndLogEx(NORMAL, "(sprint_bin_break) rowlen %d", rowlen);

  static char buf[MAX_BIN_BREAK_LENGTH]; // 3072 + end of line characters if
                                         // broken at 8 bits
  // clear memory
...
/*
void sprint_bin_break_ex(uint8_t *src, size_t srclen, char *dest , uint8_t
breaks) { if ( src == NULL ) return; if ( srclen < 1 ) return;
    ...
    if (breaks==0)
        ...
        rowlen = ( len+(len/breaks) > MAX_BIN_BREAK_LENGTH ) ?
MAX_BIN_BREAK_LENGTH : len+(len/breaks);
    ...
    for (out_index=0; out_index < rowlen-1; out_index++) {
        ...
        // check if a line break is needed and we have room to print it in our
array if ( (breaks > 0) && !((in_index+1) % breaks) && (out_index+1 != rowlen) )
{
...
char *sprint_ascii_ex(const uint8_t *data, const size_t len,
                      const size_t min_str_len) {
  static char buf[UTIL_BUFFER_SIZE_SPRINT];
  char *tmp = buf;
  memset(buf, 0x00, UTIL_BUFFER_SIZE_SPRINT);
...
int hex_to_bytes(const char *hexValue, uint8_t *bytesValue,
...
uint8_t *SwapEndian64(const uint8_t *src, const size_t len,
  ...
  for (uint8_t block = 0; block < (uint8_t)(len / blockSize); block++) {
    for (size_t i = 0; i < blockSize; i++) {
      tmp[i + (blockSize * block)] =
          src[(blockSize - 1 - i) + (blockSize * block)];
    }
  }
  return buf;
}

// takes a uint8_t src array, for len items and reverses the byte order in
// blocksizes (8,16,32,64), returns: the dest array contains the reordered src
// array.
void SwapEndian64ex(const uint8_t *src, const size_t len,
                    const uint8_t blockSize, uint8_t *dest) {
  for (uint8_t block = 0; block < (uint8_t)(len / blockSize); block++) {
    for (size_t i = 0; i < blockSize; i++) {
      dest[i + (blockSize * block)] =
          src[(blockSize - 1 - i) + (blockSize * block)];
    }
  }
}
...
int param_gethex_ex(const char *line, int paramnum, uint8_t *data,
                    int *hexcnt) {
  int bg, en, i;
  uint32_t temp;
...
int param_gethex_to_eol(const char *line, int paramnum, uint8_t *data,
                        int maxdatalen, int *datalen) {
  int bg, en;
  uint32_t temp;
  char buf[5] = {0};
...
int hextobinarray(char *target, char *source) {
  ...
  while (length--) {
    ...
    if (x >= '0' && x <= '9')
      ...
      PrintAndLogEx(
          INFO,
          "(hextobinarray) discovered unknown character %c %d at idx %d of %s",
...
int binarraytohex(char *target, const size_t targetlen, char *source,
                  size_t srclen) {
  uint8_t i = 0, x = 0;
  ...
  while (length--) {
    ...
    if (x >= '0' && x <= '1')
      ...
      PrintAndLogEx(WARNING,
                    "(binstring2binarray) discovered unknown character %c %d "
                    "at idx %d of %s",
                    x, x, (int16_t)(source - start), start);
      return 0;
    }
...
// add HID parity to binary array: EVEN prefix for 1st half of ID, ODD suffix
// for 2nd half
...
// add HID parity to binary array: ODD prefix for 1st half of ID, EVEN suffix
// for 2nd half
void wiegand_add_parity_swapped(uint8_t *target, uint8_t *source,
                                uint8_t length) {
...
char *str_dup(const char *src) { return str_ndup(src, strlen(src)); }
 ...
 */
int hexstring_to_u96(uint32_t *hi2, uint32_t *hi, uint32_t *lo,
                     const char *str) {
  uint32_t n = 0, i = 0;
 ...
 */
int binstring_to_u96(uint32_t *hi2, uint32_t *hi, uint32_t *lo,
                     const char *str) {
  uint32_t n = 0, i = 0;
