...
void crc_init_ref(crc_t *crc, int order, uint32_t polynom,
                  uint32_t initial_value, uint32_t final_xor, bool refin,
                  bool refout) {
...
void crc_init(crc_t *crc, int order, uint32_t polynom, uint32_t initial_value,
              uint32_t final_xor) {
  crc->order = order;
...
// width=8 poly=0x1d, init=0xc7 (0xe3 - WRONG! but it mentioned in MAD
// datasheet) refin=false  refout=false  xorout=0x00 name="CRC-8/MIFARE-MAD"
...
// width=4  poly=0xC, reversed poly=0x7  init=0x5   refin=true  refout=true
// xorout=0x0000  check=  name="CRC-4/LEGIC"
...
// width=8  poly=0x63, reversed poly=0x8D  init=0x55  refin=true  refout=true
// xorout=0x0000  check=0xC6  name="CRC-8/LEGIC" the CRC needs to be reversed
// before returned.
...
// width=8  poly=0x7, init=0x2C  refin=false  refout=false  xorout=0x0000
// check=0 name="CRC-8/CARDX"
